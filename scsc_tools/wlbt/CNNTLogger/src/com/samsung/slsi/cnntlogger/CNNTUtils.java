...
public class CNNTUtils {
    ...
    public boolean mIsSystemPartition;

    CNNTUtils(Context context) {
        mContext = context;
        mIsSystemPartition = isSystemPermission();
    }

    protected boolean removeDirectory(final String path, boolean inclroot) {
        File file = new File(path);
        if (file == null) {
            Log.d(CmdDefine.LOGTAG, path + " don't exist");
            return false;
        }
    ...
    protected String getSystemLoggingPath() {
        if (mIsSystemPartition) {
            return CmdDefine.systemDirPath;
        } else {
    ...
    protected String getSystemSdcardPath() {
        if (mIsSystemPartition) {
            return CmdDefine.SDCARD_SYSTEM_DEFAULT_DIR;
        } else {
    ...
    private boolean isSystemPermission() {
        File file = new File("/vendor/bin/wland");
        Log.d(CmdDefine.LOGTAG, "isVendorPermission :" + file.exists());
    ...
    public String getMxLogProperty() {
        if (mIsSystemPartition) {
            return "persist.wlbtlog.mxlog";
        } else {
            return "persist.vendor.wlbtlog.mxlog";
        }
    }

    public String getUdiLogProperty() {
        if (mIsSystemPartition) {
            return "persist.wlbtlog.udilog";
        } else {
            return "persist.vendor.wlbtlog.udilog";
        }
    }
}
