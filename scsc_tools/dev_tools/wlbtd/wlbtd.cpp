/*
 * Copyright (C) 2021 Samsung Electronics Co. Ltd.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#define LOG_TAG "WLBTD"
//#define LOG_NDEBUG 0
#if __linux__ && !__ANDROID__
#include <unistd.h>
#define ALOGD(...) do { printf(__VA_ARGS__); puts(""); } while (0)
#define ALOGE(...) do { printf(__VA_ARGS__); puts(""); } while (0)
#define ALOGI(...) do { printf(__VA_ARGS__); puts(""); } while (0)
#define PROP_VALUE_MAX	60
static int __system_property_get (const char *p, char *res) { return 0; }
static int __system_property_set (const char *p, const char *res) { return 0; }
#endif

#include <linux/netlink.h>
#include <netlink/socket.h>
#include <netlink/msg.h>
#include <netlink/genl/genl.h>
#include <netlink/genl/ctrl.h>
#if __ANDROID__
#include <log/log.h>
#endif
#include <stdbool.h>
#include <sys/mman.h>
#include <sys/types.h>      /* needed to use pid_t, etc. */
#include <sys/wait.h>       /* needed to use waitpid() */
#if __ANDROID__
#include <sys/system_properties.h>
#endif
#include <sys/time.h>
#include <sys/stat.h>
#include <stdlib.h>
#include <signal.h>
#include <errno.h>
#include "wlbtd.h"
#include "wlbtd_sable.h"

/* WLBTD version
 * =============
 * v3.15- Add back error code in sable file
 * v3.14- Support dump over MMAP in EVENT_CHIPSET_LOGGING
 * v3.13- Append Error code in Sable file
 * v3.12- EVENT_CHIPSET_LOGGING
 * v3.11- Remove sleep in wlbtd sable init function
 * v3.10- Implement queue to buffer sable requests
 * v3.9 - Use w+ with fopen when writing files.
 * v3.8 - Fix memory leaks
 * v3.7 - Add support to dump the last fw panic in DRAM.
 * v3.6 - Fix synchronisation in sable tar generation.
 * v3.5 - Use consistent response codes across triggers.
 * v3.4 - Make max_file_per_trigger value configurable
 * v3.3 - Add termination signal handling
 * v3.2 - set vendor.wlbtd.board_id property
 * v3.1 - use response_codes to convey status to kernel driver
 * v3.0 - parse .memdump.info
 * v2.0 - SABLE - EVENT_SABLE
 * v1.0 - pre-SABLE EVENT_SCSC, EVENT_SYSTEM_PROPERTY, EVENT_WRITE_FILE
 * */
#define WLBTD_VERSION "3.15"
#define TAR_FILES_PER_TRIGGER "2"
#define MEMDUMP_FILE_NO_MOREDUMP	0
#define MEMDUMP_FILE_RECOVERY_ON	2
#define MEMDUMP_FILE_PANIC_KERNEL	3
#define MEMDUMP_FILE_OTHER		4
static struct nl_sock *sk = NULL;
static int group;
static int family;
static int memdump_file_contents = -1;
char *retain_filepath = NULL;
#define BIT(x) (1 << (x))
#define MMAP_LOG BIT(2)
#define FILE_CREATE BIT(1)
#define MORE_DATA BIT(0)

const char *get_trigger_str(int trigger_int) {
	switch (trigger_int) {
	case 1: return "scsc_log_fw_panic";
	case 2: return "scsc_log_user";
	case 3: return "scsc_log_fw";
	case 4: return "scsc_log_dumpstate";
	case 5: return "scsc_log_host_wlan";
	case 6: return "scsc_log_host_bt";
	case 7: return "scsc_log_host_common";
	case 8: return "scsc_log_sys_error";
	default:
	case 0:
		return "scsc_log_unknown";
	}
}

static int file_exists(const char* filename) {
	struct stat buffer;
	int exist = stat(filename, &buffer);
	if (exist == 0)
		return 1;
	else {
		return 0;
	}
}

static int process_cmd_system_property_get(struct nl_msg *msg)
{
	/* The total bytes copied will be no greater than PROP_VALUE_MAX */
	char value[PROP_VALUE_MAX];
	int error, length;
	struct nl_msg *ack_msg;
	struct nlmsghdr *nl_hdr;
	struct genlmsghdr *genl_hdr;
	struct nlattr *attrs[__ATTR_MAX];

	nl_hdr = nlmsg_hdr(msg);
	genl_hdr = genlmsg_hdr(nl_hdr);
	error = genlmsg_parse(nl_hdr, 0, attrs, __ATTR_MAX - 1, NULL);
	if (error) {
		ALOGE("genlmsg_parse failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	ALOGI("ATTR_STR: len:%u type:%u data:%s\n", attrs[1]->nla_len, attrs[1]->nla_type, (char *)nla_data(attrs[1]));
	length = __system_property_get((char *)nla_data(attrs[1]), value);
	if (length <= 0) {
		ALOGE("__system_property_get failed: length=%d", length);
		return -1;
	}

	ALOGI("__system_property_get: length=%d value=%s", length, value);

	ALOGI("Sending EVENT_SYSTEM_PROPERTY cmd reply.");
	ack_msg = nlmsg_alloc();
	if (ack_msg) {
		genlmsg_put(ack_msg,
			NL_AUTO_PORT,
			NL_AUTO_SEQ,
			family, 0, 0,
			genl_hdr->cmd/*EVENT_SYSTEM_PROPERTY*/,
			0);
		nla_put_string(ack_msg, ATTR_STR, value);
		error = nl_send_auto(sk, ack_msg);
		if (error < 0)
			ALOGE("nl_send_auto failed !");
		nlmsg_free(ack_msg);
		ALOGI("sent (%d byte) reply msg to kernel.", error);
	} else {
		ALOGE("nlmsg_alloc failed");
	}
	return 0;
}

int process_cmd_write_file(struct nl_msg *msg)
{
	struct nlmsghdr *nl_hdr;
	struct genlmsghdr *genl_hdr;
	struct nlattr *attrs[__ATTR_MAX];
	int error = -1;
	struct nl_msg *ack_msg;
	char *filepath;
	char *content;
	char msg_str[MAX_LOG_STR_KERNEL];

	nl_hdr = nlmsg_hdr(msg);
	genl_hdr = genlmsg_hdr(nl_hdr);

	error = genlmsg_parse(nl_hdr, 0, attrs, __ATTR_MAX - 1, NULL);
	if (error) {
		ALOGE("genlmsg_parse failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	filepath = (char *)nla_data(attrs[3]);
	content = (char *)nla_data(attrs[4]);

	FILE *fp = fopen(filepath, "w+");
	if (fp == NULL)
		ALOGE("failed to open file");
	else
	{
		error = fprintf(fp, "%s", content);
		if (ferror(fp)) {
			ALOGE("writing %s failed.", filepath);
			snprintf(msg_str, MAX_LOG_STR_KERNEL, "writing %s failed.", filepath);

		} else {
			error = fflush(fp);
			ALOGI("wrote file %s", filepath);
			snprintf(msg_str, MAX_LOG_STR_KERNEL, "written %s", filepath);
		}
		fclose(fp);
	}

	ack_msg = nlmsg_alloc();
	if (ack_msg) {
		genlmsg_put(ack_msg,
				NL_AUTO_PORT,
				NL_AUTO_SEQ,
				family, 0, 0,
				genl_hdr->cmd/*EVENT_WRITE_FILE*/,
				0);
		nla_put_string(ack_msg, ATTR_PATH, msg_str);
		error = nl_send_auto(sk, ack_msg);
		if (error < 0)
			ALOGE("nl_send_auto failed !");
		nlmsg_free(ack_msg);
		ALOGI("sent (%d byte) ack msg to kernel.", error);
	} else {
		/* not sending ack back to kernel so there will be completion timeout */
		ALOGE("nlmsg_alloc failed");
	}

	return 0;
}

#define LOG_WLAN_MMMAP_DEV_ENTRY "/dev/scsc_wlan_mmap_0"

int process_cmd_chipset_logging_mmap(int length)
{
	int log_fd, dev_fd, errn;
	struct tm *lt = NULL;
	char filepath[64];
	char msg_str[MAX_LOG_STR_KERNEL];
	time_t rawtime;

	ALOGI("Memmap %s for %d bytes\n", LOG_WLAN_MMMAP_DEV_ENTRY, length);

	dev_fd = open(LOG_WLAN_MMMAP_DEV_ENTRY, O_RDWR);
	if (dev_fd < 0) {
		ALOGE("failed %d to open %s\n", errno, LOG_WLAN_MMMAP_DEV_ENTRY);
		snprintf(msg_str, MAX_LOG_STR_KERNEL, "failed %d to open %s",
			 errno, LOG_WLAN_MMMAP_DEV_ENTRY);

		send_response_to_kernel(EVENT_CHIPSET_LOGGING, msg_str, 0);
		return -1;
	}

	char *addr = (char *)mmap(NULL, length, PROT_READ | PROT_WRITE,
				  MAP_SHARED, dev_fd, 0);
	if (addr == nullptr) {
		ALOGE("mmap failed %d to open %s\n", errno, LOG_WLAN_MMMAP_DEV_ENTRY);
		snprintf(msg_str, MAX_LOG_STR_KERNEL, "failed %d to open %s",
			 errno, LOG_WLAN_MMMAP_DEV_ENTRY);
		close(dev_fd);
		send_response_to_kernel(EVENT_CHIPSET_LOGGING, msg_str, 0);
		return -1;
	}

	ALOGI("Create a file at path /data/vendor/log/wifi/");

	rawtime = time(NULL);

	if (rawtime == -1) {
		ALOGE("error in fetching time!");
	} else {
		lt = localtime(&rawtime);
	}

	if (lt != NULL) {
		sprintf(filepath,"/data/vendor/log/wifi/debug_dump_%04d%02d%02d%02d%02d%02d",
			lt->tm_year + 1900,
			lt->tm_mon + 1,
			lt->tm_mday,
			lt->tm_hour,
			lt->tm_min,
			lt->tm_sec);
	} else {
		ALOGE("error in fetching local time, creating file without timestamp");
		sprintf(filepath, "/data/vendor/log/wifi/debug_dump");
	}

	ALOGI("filepath = %s", filepath);
	log_fd = open(filepath, O_WRONLY | O_CREAT, 0644);
	if (log_fd < 0) {
		ALOGE("failed %d to open %s\n", errno, filepath);
		snprintf(msg_str, MAX_LOG_STR_KERNEL, "failed %d to open %s",
			 errno, filepath);
		close(dev_fd);
		munmap((void *)addr, length);
		send_response_to_kernel(EVENT_CHIPSET_LOGGING, msg_str, 0);
		return -1;
	}

	/* write sable file */
	write(log_fd, addr, length);
	ALOGD("%s written", filepath);
	close(dev_fd);
	munmap((void *)addr, length);
	close(log_fd);

	snprintf(msg_str, MAX_LOG_STR_KERNEL, "%s written", filepath);
	ALOGI("sending ack to kernel, msg_str = %s", msg_str);
	send_response_to_kernel(EVENT_CHIPSET_LOGGING, msg_str, 0);
	return 0;
}

int process_cmd_chipset_logging(struct nl_msg *msg)
{
	struct nlmsghdr *nl_hdr;
	struct genlmsghdr *genl_hdr;
	struct nlattr *attrs[__ATTR_MAX];
	int error = -1;
	char filepath[64];
	char *content;
	char msg_str[MAX_LOG_STR_KERNEL];
	time_t rawtime;
	struct tm *lt = NULL;
	uint8_t flags = 0;
	int length = 0;

	nl_hdr = nlmsg_hdr(msg);
	genl_hdr = genlmsg_hdr(nl_hdr);

	error = genlmsg_parse(nl_hdr, 0, attrs, __ATTR_MAX - 1, NULL);
	if (error) {
		ALOGE("genlmsg_parse failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	content = (char *)nla_data(attrs[ATTR_CONTENT]);

	flags = nla_get_u8(attrs[ATTR_INT8]);
	ALOGI("flags from driver = 0x%x", flags);

	length = nla_get_u32(attrs[ATTR_INT]);
	ALOGI("length from driver = %d", length);

	if (flags & MMAP_LOG) {
		/* Proces request using mmap */
		ALOGI("MMAP used for chip loggging");
		error = process_cmd_chipset_logging_mmap(length);
		return error;
	}

	if (flags & FILE_CREATE){
		ALOGI("Create a file at path /data/vendor/log/wifi/");

		rawtime = time(NULL);

		if (rawtime == -1) {
			ALOGE("error in fetching time!");
		} else {
			lt = localtime(&rawtime);
		}

		if (lt != NULL) {
			sprintf(filepath,"/data/vendor/log/wifi/debug_dump_%04d%02d%02d%02d%02d%02d",
			        lt->tm_year + 1900,
			        lt->tm_mon + 1,
			        lt->tm_mday,
			        lt->tm_hour,
			        lt->tm_min,
			        lt->tm_sec);
		} else {
			ALOGE("error in fetching local time, creating file without timestamp");
			sprintf(filepath, "/data/vendor/log/wifi/debug_dump");
		}

		ALOGI("filepath = %s", filepath);

		FILE *fp = fopen(filepath, "wb");
		if (fp == NULL) {
			ALOGE("Failed to create debug_dump file");
			snprintf(msg_str, MAX_LOG_STR_KERNEL, "Failed to create file");
		} else {
			fclose(fp);
			ALOGI("File creation successful");
		}

		/* Retain file path */
		retain_filepath = (char *)malloc(64);

		if (retain_filepath) {
			strcpy(retain_filepath, filepath);
			ALOGI("file path retained = %s", retain_filepath);
		} else {
			ALOGE("Failed to retain filepath");
			snprintf(msg_str, MAX_LOG_STR_KERNEL, "mem alloc failed for retain_filepath");
		}
	}

	if (retain_filepath != NULL) {
		FILE *fp = fopen(retain_filepath, "ab");
		if (fp == NULL) {
			ALOGE("failed to append file");
			snprintf(msg_str, MAX_LOG_STR_KERNEL, "failed to append file: %s", retain_filepath);

			/* free the retained file path */
			ALOGI("Failed to open file, clear retain_filepath");
			free(retain_filepath);
			retain_filepath = NULL;
		} else {
			fseek(fp, 0, SEEK_END);
			fwrite(content, length, 1, fp);
			if (ferror(fp)) {
				ALOGE("append %s failed", retain_filepath);
				snprintf(msg_str, MAX_LOG_STR_KERNEL, "append %s failed", retain_filepath);

				/* free the retained file path now */
				ALOGI("Append file failed, clear retain_filepath");
				free(retain_filepath);
				retain_filepath = NULL;
			} else {
				error = fflush(fp);
				ALOGI("wrote file %s", filepath);
				snprintf(msg_str, MAX_LOG_STR_KERNEL, "File is written");
			}
			fclose(fp);
		}
	} else {
		ALOGE("Unable to write file, error in retain_filepath");
		snprintf(msg_str, MAX_LOG_STR_KERNEL, "Unable to write file, retain_filepath error");
	}

	if ((flags & MORE_DATA) == 0) {
		ALOGI("sending ack to kernel, msg_str = %s", msg_str);
		send_response_to_kernel(EVENT_CHIPSET_LOGGING, msg_str, 0);

		/* free the retained file path now */
		ALOGI("Clear retain_filepath after sending ack to kernel");
		free(retain_filepath);
		retain_filepath = NULL;
	}
	return 0;
}

void send_response_to_kernel(enum events e, const char *msg_str, int status)
{
	int error = -1;
	struct nl_msg *ack_msg = nlmsg_alloc();
	if (ack_msg) {
		genlmsg_put(ack_msg, NL_AUTO_PORT, NL_AUTO_SEQ, family, 0, 0, e, 0);
		nla_put_string(ack_msg, ATTR_STR, msg_str);
		if (e == EVENT_SABLE)
			nla_put_u16(ack_msg, ATTR_INT, status);
		else
			nla_put_u32(ack_msg, ATTR_INT, status);
		error = nl_send_auto(sk, ack_msg);
		if (error < 0)
			ALOGE("nl_send_auto failed !");
		nlmsg_free(ack_msg);
		ALOGI("sent (%d byte) ack msg to kernel.", error);
	} else {
		/* not sending ack back to kernel so there will be completion timeout */
		ALOGE("nlmsg_alloc failed");
	}

}

void parse_memdump_file_wlbtd(void) {
	FILE *fp = nullptr;
	int c = 0;
	char android_version[PROP_VALUE_MAX];
	int len, disable_recovery = 1;
	char memdump_file[64];

	len = __system_property_get("ro.build.version.release", android_version);

	if (atoi(android_version) >= 9)
		snprintf(memdump_file, sizeof(memdump_file), "%s","/sys/wifi/memdump");
	else
		snprintf(memdump_file, sizeof(memdump_file), "%s","/data/misc/conn/.memdump.info");

	if(file_exists(memdump_file)) {
		fp = fopen(memdump_file, "r");
		if (fp != NULL) {
			c = fgetc(fp);
			switch (c) {
			case '3' :
				memdump_file_contents = MEMDUMP_FILE_PANIC_KERNEL;
				disable_recovery = 1;
				break;
			case '0':
				memdump_file_contents = MEMDUMP_FILE_NO_MOREDUMP;
				disable_recovery = 0;
				break;
			case '2':
				memdump_file_contents = MEMDUMP_FILE_RECOVERY_ON;
				disable_recovery = 0;
				break;
			default:
				memdump_file_contents = c - '0';
				disable_recovery = 1;
				break;
			}

			ALOGI("memdump_file_contents %d Auto-recovery: %s", memdump_file_contents, disable_recovery ? "off" : "on");
			fclose(fp);
		} else
			ALOGE("failed to open %s", memdump_file);
	} else
		ALOGE("%s not found", memdump_file);
}

static void check_setup()
{
	char maxTarFiles[PROP_VALUE_MAX];

	__system_property_set("vendor.wlbtd.version", WLBTD_VERSION);

	if (!__system_property_get("vendor.wlbtd.tar_files_per_trigger", maxTarFiles))
	{
		int retCode = __system_property_set("vendor.wlbtd.tar_files_per_trigger", TAR_FILES_PER_TRIGGER);

		if (retCode < 0)
			ALOGE("Failed to set property value for vendor.wlbtd.tar_files_per_trigger. ErrorCode = %d", retCode);
		else
			ALOGI("Successfully set Max Sable files per trigger value to %s", TAR_FILES_PER_TRIGGER);
	}
	else
	{
		ALOGI("Max Sable files per trigger value = %s", maxTarFiles);
	}

	parse_memdump_file_wlbtd();
}

int wlbtd_popen(const char *command)
{
	pid_t pid, w;
	int error = -1;
	int status = -1;
	struct timeval t0, t1;
	double diff;

	/* fork and run script in child process otherwise wlbtd service gets killed */
	ALOGI("%s start", __func__);
	gettimeofday(&t0, 0);
	pid = fork();
	if (pid < 0) {
		ALOGE("fork() failed.");
		return error;
	} else if (pid == 0) {
		/* child */
#if __linux__ && !__ANDROID__
		ALOGI("Execute command %s", command);
		error = system(command);
#else
		error = execl("/vendor/bin/sh", "/vendor/bin/sh", "-c", command, (char *)0);
		/* execl only returns -1 on error, so following code should not execute
		 * on success
		 */
#endif
		if (error == -1 ) {
			ALOGE("%s execl failed error %d", __func__, error);
			return error;
		}
	} else {
		do {
			/* parent
			 * wait for child to terminate and send ack to kernel that script is done.
			 *
			 * When WIFEXITED() is nonzero, WEXITSTATUS() evaluates to the low-order 8 bits
			 * of the status argument that the child passed to the exit() or _exit() function,
			 * or the value the child process returned from main().
			 */
			w = waitpid(pid, &status, WUNTRACED | WCONTINUED);
			if (w == -1) {
				ALOGE("%s waitpid failed", __func__);
				return error;
			}
		}while (!WIFEXITED(status));
	}
	gettimeofday(&t1, 0);
	diff = (t1.tv_sec - t0.tv_sec) * 1000.0f + (t1.tv_usec - t0.tv_usec) / 1000.0f;
	ALOGI("%s end. done in %dms", __func__, (int)diff);
	return 0;
}

int parse_and_queue_sable_msg(struct nl_msg *msg)
{
	struct nlmsghdr *nl_hdr;
	struct nlattr *attrs[__ATTR_MAX];
	int error = -1;
	char logmsg[MAX_LOG_STR_KERNEL];
	sable_req *req = NULL;

	nl_hdr = nlmsg_hdr(msg);
	error = genlmsg_parse(nl_hdr, 0, attrs, __ATTR_MAX - 1, NULL);
	if (error) {
		ALOGE("genlmsg_parse failed : %s (%d)", nl_geterror(error), error);
		send_response_to_kernel(EVENT_SABLE, "genlmsg_parse failed", SCSC_WLBTD_ERR_PARSE_FAILED);
		return error;
	}

	if (!file_exists(log_collection_script)){
		ALOGE("%s not found. SABLE tar will not be generated", log_collection_script);
		send_response_to_kernel(EVENT_SABLE, "mx_log_collection.sh not found",
                        (nla_get_u8(attrs[5]) == SCSC_LOG_FW_PANIC) ? SCSC_WLBTD_FW_PANIC_ERR_SCRIPT_FILE_NOT_FOUND : SCSC_WLBTD_OTHER_ERR_SCRIPT_FILE_NOT_FOUND);
		return -ENOENT;
	}

	req = (sable_req *) malloc(sizeof(sable_req));
	if (req == NULL) {
		snprintf(logmsg, MAX_LOG_STR_KERNEL, "Memory allocation for sable_req failed\n");
		send_response_to_kernel(EVENT_SABLE, logmsg, SCSC_WLBTD_OTHER_IGNORE_TRIGGER);
		return -ENOMEM;
	}

	req->error_code    = nla_get_u16(attrs[2]);
	req->trigger_uint8 = nla_get_u8(attrs[5]);
	req->trigger_str   = get_trigger_str((int)req->trigger_uint8);
	error = wlbtd_sable_queue_req(req, logmsg, MAX_LOG_STR_KERNEL);
	if (error) {
		ALOGI("Sable request for error code %d ignored\n", req->error_code);
		free(req);
		send_response_to_kernel(EVENT_SABLE, logmsg, SCSC_WLBTD_OTHER_IGNORE_TRIGGER);
		return -1;
	}

	ALOGI("Sable request for error code %d queued successfully\n", req->error_code);
	return 0;
}

static int process_cmd_scsc(struct nl_msg *msg)
{
	struct nlmsghdr *nl_hdr;
	struct nlattr *attrs[__ATTR_MAX];
	int error, status = -1;
	char *script;
	char command[64];

	nl_hdr = nlmsg_hdr(msg);
	error = genlmsg_parse(nl_hdr, 0, attrs, __ATTR_MAX - 1, NULL);
	if (error) {
		ALOGE("genlmsg_parse failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	/* Remember: attrs[0] is a throwaway. */

	if (attrs[1])
		ALOGD("ATTR_STR: len:%u type:%u data:%s\n",
				attrs[1]->nla_len,
				attrs[1]->nla_type,
				(char *)nla_data(attrs[1]));
	else
		ALOGD("ATTR_STR: null\n");

	if (attrs[2])
		ALOGD("ATTR_INT: len:%u type:%u data:%u\n",
				attrs[2]->nla_len,
				attrs[2]->nla_type,
				*((__u32 *)nla_data(attrs[2])));
	else
		ALOGD("ATTR_INT: null\n");

	script = (char *)nla_data(attrs[1]);

	/* moredump script calls moredump.bin and mx_logger_dump.sh internally */
	if (!strcmp(script, "moredump")) {
		if (!file_exists("/vendor/bin/moredump")){
			ALOGE("/vendor/bin/moredump not found. Moredump will not be generated");
			if (file_exists("/vendor/bin/mx_logger_dump.sh"))
				strncpy(script, "mx_logger_dump.sh", sizeof("mx_logger_dump.sh")+1);
			else {
				ALOGE("/vendor/bin/mx_logger_dump.sh not found. Wifi disconnect logs will not be generated");
				send_response_to_kernel(EVENT_SCSC, "moredump & mx_logger_dump.sh not found", SCSC_WLBTD_OTHER_ERR_SCRIPT_FILE_NOT_FOUND);
				return status;
			}
		}
		if (!file_exists("/vendor/bin/moredump.bin")){
			ALOGE("/vendor/bin/moredump.bin not found. Moredump will not be generated");
			if (file_exists("/vendor/bin/mx_logger_dump.sh"))
				strncpy(script ,"mx_logger_dump.sh", sizeof("mx_logger_dump.sh")+1);
			else {
				ALOGE("/vendor/bin/mx_logger_dump.sh not found. Wifi disconnect logs will not be generated");
				send_response_to_kernel(EVENT_SCSC, "moredump.bin & mx_logger_dump.sh not found", SCSC_WLBTD_OTHER_ERR_SCRIPT_FILE_NOT_FOUND);
				return status;
			}
		}
		if (!file_exists("/vendor/bin/mx_logger_dump.sh"))
			ALOGE("/vendor/bin/mx_logger_dump.sh not found. Wifi disconnect logs will not be generated");

	}

	if (!strcmp(script, "mx_logger_dump.sh")) {
		if (!file_exists("/vendor/bin/mx_logger_dump.sh")){
			ALOGE("/vendor/bin/mx_logger_dump.sh not found. Wifi disconnect logs will not be generated");
			send_response_to_kernel(EVENT_SCSC, "mx_logger_dump.sh not found", SCSC_WLBTD_OTHER_ERR_SCRIPT_FILE_NOT_FOUND);
			return status;
		}
	}

	parse_memdump_file_wlbtd();
	if (!strcmp(script, "moredump") && (memdump_file_contents == MEMDUMP_FILE_NO_MOREDUMP)) {
		send_response_to_kernel(EVENT_SCSC, " memdump_file_contents == 0, no moredump generated. Ignoring trigger ...", SCSC_WLBTD_OTHER_IGNORE_TRIGGER);
		return 0;
	}

	snprintf(command, 64, "/vendor/bin/%s", script);

	ALOGI("%s: %s - start", __func__, script);

	status = wlbtd_popen(command);
	if (status == 0) {
		ALOGI("%s: %s - end", __func__, script);
		send_response_to_kernel(EVENT_SCSC, strcat(script, " done ack !"), SCSC_WLBTD_OTHER_SBL_GENERATED);
	} else {
		ALOGE("%s: %s failed to run. status %d", __func__, script, status);
		send_response_to_kernel(EVENT_SCSC, strcat(script, " failed to run"), SCSC_WLBTD_FW_PANIC_ERR_TAR);
	}

	return 0;
}

/*
 * This function will be called for each valid netlink message received
 * in nl_recvmsgs_default()
 */
static int rcv_msg_cb(struct nl_msg *msg, void *arg)
{
	struct nlmsghdr *nl_hdr;
	struct genlmsghdr *genl_hdr;

	(void)arg;

	nl_hdr = nlmsg_hdr(msg);
	genl_hdr = genlmsg_hdr(nl_hdr);

	if (genl_hdr->cmd == EVENT_SABLE) {
		ALOGI("Received EVENT_SABLE v%s\n", WLBTD_VERSION);
		return parse_and_queue_sable_msg(msg);
	} else if (genl_hdr->cmd == EVENT_SYSTEM_PROPERTY) {
		ALOGI("Received EVENT_SYSTEM_PROPERTY\n");
		return process_cmd_system_property_get(msg);
	} else if (genl_hdr->cmd == EVENT_SCSC) {
		ALOGI("Received EVENT_SCSC\n");
		return process_cmd_scsc(msg);
	} else if (genl_hdr->cmd == EVENT_WRITE_FILE) {
		ALOGI("Received EVENT_WRITE_FILE\n");
		return process_cmd_write_file(msg);
	} else if (genl_hdr->cmd == EVENT_CHIPSET_LOGGING) {
		ALOGI("Received EVENT_CHIPSET_LOGGING\n");
		return process_cmd_chipset_logging(msg);
	}
	else {
		ALOGI("The message type is not supported; ignoring.\n");
		send_response_to_kernel(EVENT_SCSC, "message type not supported", 0);
		return 0;
	}

	return 0;
}


/*
 * Write the board_id from sysfs into an Android property
 */
#define WLBTD_BOARD_ID_FILE		"/sys/devices/system/chip-id/board_id"
#define WLBTD_BOARD_ID_PROPERTY		"vendor.wlbtd.board_id"
#define WLBTD_BOARD_PROP_LENGTH		128
static void get_board_id()
{
	FILE *fd = fopen(WLBTD_BOARD_ID_FILE, "r");
	char value[WLBTD_BOARD_PROP_LENGTH];
	size_t n;
	int r;

	if (!fd) {
		ALOGE("open board_id file %s failed", WLBTD_BOARD_ID_FILE);
		return;
	}

	n = fscanf(fd, "%127s", value);
	if (n > 0) {
		ALOGI("read board_id '%s' from %s", value, WLBTD_BOARD_ID_FILE);
		r = __system_property_set(WLBTD_BOARD_ID_PROPERTY, value);
		if (r)
			ALOGE("__system_property_set %s: failed %d", WLBTD_BOARD_ID_PROPERTY, r);
		else
			ALOGI("property_set: %s to %s", WLBTD_BOARD_ID_PROPERTY, value);
	}
	fclose(fd);
}

static void clean_resources(void)
{
        ALOGI("Cleaning resources\n");
        ALOGI("Sending message to wlbtd sable to stop ... \n");
        wlbtd_sable_destroy();
        ALOGI("Sending error to kernel to abort current transaction\n");
        send_response_to_kernel(EVENT_SABLE, "WLBTD process terminated. Abort current transaction", SCSC_WLBTD_ERR_PARSE_FAILED);
        /* Free memory allocated by netlink socket */
        if (sk)
                /* The socket is closed automatically if a `struct nl_sock` object is freed using `nl_socket_free() */
                nl_socket_free(sk);
}

static void wlbtd_signal_handler(int signum)
{
	switch(signum)
	{
		case SIGTERM:
		case SIGINT:
		case SIGABRT:
		case SIGQUIT:
		case SIGKILL:
		case SIGHUP:
			ALOGI("Received termination signal %d\n", signum);
			clean_resources();
			exit(0);
		default:
			ALOGE("WLBTD: Caught wrong signal\n");
	};
}

static int do_things(void)
{
	int error = -1;

	struct sigaction sa;
        memset(&sa, '\0', sizeof(sa));

	/* Socket allocation */
	sk = nl_socket_alloc();
	if (!sk) {
		ALOGE("nl_socket_alloc failed");
		return error;
	}

	nl_socket_disable_seq_check(sk);
	nl_socket_disable_auto_ack(sk);

	error = nl_socket_modify_cb(sk, NL_CB_VALID, NL_CB_CUSTOM, rcv_msg_cb, NULL);
	if (error) {
		ALOGE("nl_socket_modify_cb failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	error = genl_connect(sk);
	if (error) {
		ALOGE("genl_connect failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	/* Find the multicast group identifier and register ourselves to it. */
	group = genl_ctrl_resolve_grp(sk, "scsc_mdp_family", "scsc_mdp_grp");
	if (group < 0) {
		ALOGE("genl_ctrl_resolve_grp failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	family = genl_ctrl_resolve(sk, "scsc_mdp_family");
	if (family < 0) {
		ALOGE("genl_ctrl_resolve failed : %s (%d)", nl_geterror(error), error);
		return error;
	}

	ALOGI("add membership to group %u family %u", group, family);

	error = nl_socket_add_memberships(sk, group, 0);
	if (error) {
		ALOGE("nl_socket_add_memberships() failed  : %s (%d)", nl_geterror(error), error);
		return error;
	}

	check_setup();

	get_board_id();

	ALOGI("Registering termination signal handler\n");

        sa.sa_handler=wlbtd_signal_handler;
        sigemptyset(&sa.sa_mask);
        sigaction(SIGTERM, &sa, NULL);
        sigaction(SIGINT,  &sa, NULL);
        sigaction(SIGABRT, &sa, NULL);
        sigaction(SIGQUIT, &sa, NULL);
        sigaction(SIGKILL, &sa, NULL);
        sigaction(SIGHUP,  &sa, NULL);


	ALOGI("calling nl_recvmsgs_default");

	/* Finally, receive the messages */
	while (1)
		nl_recvmsgs_default(sk);

	return 0;
}

int main(void)
{
	int error;

	wlbtd_sable_init();
	error = do_things();
	clean_resources();
	ALOGI("%s: end error = %d",__func__, error);

	return error;
}
