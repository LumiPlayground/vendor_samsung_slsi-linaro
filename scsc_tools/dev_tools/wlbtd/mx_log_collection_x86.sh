...
if [[ ${memdump_file_val} != "0" ]]; then
	...
	if grep -q -E -i "lassen|leman|nacho|neus|orange" /proc/driver/mxman_info/mx_release; then
		moredump ${moredumpdir}/moredump_${DATE_TAG}.cmm -xml_path ${xml_dir} -log_strings ${log_strings} -dev 2>/dev/null >/dev/null
	else
		# p-series chip log_strings are in the FW image
		moredump ${moredumpdir}/moredump_${DATE_TAG}.cmm -xml_path ${xml_dir} -firmware_binary ${base_dir}/${fw_var}.bin -dev 2>/dev/null >/dev/null
	fi
	script_status=$?
...
echo "Linux: tar_files_per_trigger : ${max_logs}" >> ${status_file} 2>&1
echo "Linux: WLBTD version Linux : ${wlbtd_version}" >> ${status_file} 2>&1
