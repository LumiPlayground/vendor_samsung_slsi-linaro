...
/* START [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  pthread_mutex_init(&p_os->gki_sync_mutex, nullptr);
#endif
/* END [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
}
...
/* START [17112201S] Patch for reducing NFC Off operation time (100ms => 20ms)*/
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("GKI_shutdown() : Enter. task_id =  %d", task_id);
      while ((gki_cb.com.OSWaitEvt[task_id - 1] != 0) && (++i < 10))
        usleep(20 * 1000);
/* END [17112201S] Patch for reducing NFC Off operation time (100ms => 20ms)*/
#else
      while ((gki_cb.com.OSWaitEvt[task_id - 1] != 0) && (++i < 10))
        usleep(100 * 1000);
#endif
...
/* START [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  pthread_mutex_lock(&gki_cb.os.gki_sync_mutex);
  /* Destroy mutex and condition variable objects */
  DLOG_IF(ERROR,nfc_debug_enabled)
    << StringPrintf("destroy start GKI_mutex : %p", &gki_cb.os.GKI_mutex);
  pthread_mutex_destroy(&gki_cb.os.GKI_mutex);
  DLOG_IF(ERROR,nfc_debug_enabled)
    << StringPrintf("destroy end GKI_mutex : %p", &gki_cb.os.GKI_mutex);
/* END [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
#else
  /* Destroy mutex and condition variable objects */
  pthread_mutex_destroy(&gki_cb.os.GKI_mutex);
...
/* START [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  pthread_mutex_unlock(&gki_cb.os.gki_sync_mutex);
  DLOG_IF(INFO, nfc_debug_enabled)
    << StringPrintf("GKI_shutdown: exit");
#endif
/* END [20011300][NFC-2823] Sync GKI_run and GKI_shutdown */
}
...
/* START [S180410] GKI critical section problem (CQ2063)*/
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    pthread_mutex_lock(&p_os->gki_timer_mutex);
    *p_run_cond = GKI_TIMER_TICK_RUN_COND;
/* END [S180410] GKI critical section problem (CQ2063)*/
#else
    *p_run_cond = GKI_TIMER_TICK_RUN_COND;
    pthread_mutex_lock(&p_os->gki_timer_mutex);
#endif
...
/* START [S180410] GKI critical section problem (CQ2063)*/
    ...
    if (GKI_TIMER_TICK_EXIT_COND != *p_run_cond) {
      if (GKI_TIMER_TICK_STOP_COND == *p_run_cond) {
