...
/* START [20200812][NFC2955] to avoid the crash */
bool NfcAdaptation::isHalShutdown = false;
/* END [20200812][NFC2955] to avoid the crash */
...
/* START_SLSI [S15052703] Get a firmware version information */
tNFC_FW_VERSION NfcAdaptation::fw_version;
/* END_SLSI [S15052703] Get a firmware version information */

/* START_SLSI [S14111804] Patch for prcoessing the first core_init response */
static bool firstInitRsp = true;
/* END_SLSI [S14111804] Patch for prcoessing the first core_init response */
#endif

bool isShuttingdown() {
  char val[PROPERTY_VALUE_MAX] = {0};
  property_get("sys.powerctl", val, "");
  ALOGE("sys.powerctl = %s", val);
...
/* START [S15110301] Add the Mifare CLT config for N81 or N7 */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    nfa_proprietary_cfg.pro_protocol_clt = NfcConfig::getUnsigned("NFA_PROPRIETARY_CFG_CLT",0x90);
#endif
/* END [S15110301] Add the Mifare CLT config for N81 or N7 */
...
/*******************************************************************************
**
** Function:    NfcAdaptation::isShutDown
**
** Description: .
**
** Returns:     bool.
**
*******************************************************************************/
bool NfcAdaptation::isShutDown() { return isHalShutdown; }
#endif
/* END [20200812][NFC2955] to avoid the crash */
...
/* START_SLSI [S14111804] Patch for prcoessing the first core_init response */
    ...
    case HAL_NFC_OPEN_CPLT_EVT:
      ...
      {
        uint8_t nci_reset[] = {0x20, 0x00, 0x01, 0x01};
        DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: Send CORE_RESET_CMD", func);
...
/* START [181106] Patch for supporting NCI v2.0 */
void setFWversionInfo(uint8_t* p_ver)
{
  if(p_ver != NULL)
  {
    NfcAdaptation::fw_version.major_version = (p_ver[0] >> 4) & 0xF;
    NfcAdaptation::fw_version.minor_version = p_ver[0] & 0xF;
    NfcAdaptation::fw_version.build_info_high = p_ver[1];
    NfcAdaptation::fw_version.build_info_low = p_ver[2];
  }
}
/* END[181106] Patch for supporting NCI v2.0 */
...
/* START_SLSI [S14111804] Patch for prcoessing the first core_init response */
  ...
  if (data_len < 3 || p_data == NULL) return;

  uint8_t mt = p_data[0] & 0xE0;
  uint8_t gid = p_data[0] & 0x0F;
  uint8_t oid = p_data[1];
  uint8_t status = p_data[3];
/* END_SLSI [S14111804] Patch for prcoessing the first core_init response */

/* START [181106] Patch for supporting NCI v2.0 */
  static int iNciVersion = NCI_VERSION_1_0;         // 0x10 : NCI 1.0, 0x20 : NCI2.0

  uint8_t nci_init_cmd_nci2[] = {0x20, 0x01, 0x02, 0x00, 0x00};
  uint8_t nci_init_cmd_nci1[] = {0x20, 0x01, 0x00};

/* START_SLSI [S15052703] Get a firmware version information */
  uint8_t* p_ver = &p_data[(p_data[2] + 3) - 4];
  if (mt == 0x40)  // NCI_RSP
  {
    if (gid == 0x00 && oid == 0x00)  // CORE_RESET_RSP
    {
      iNciVersion = NCI_VERSION_2_0;
      if(data_len == 6 && status == 0x00) // NCI 1.0 Case
      {
        iNciVersion = NCI_VERSION_1_0;
        HalWrite(3, nci_init_cmd_nci1);  // CORE_INIT_CMD
      }
    }
    else if (gid == 0x00 && oid == 0x01)  // CORE_INIT_RSP
    {
      if (iNciVersion == NCI_VERSION_1_0 && status == 0x00) //when NCI 2.0, extract fw version from RESET_NTF
      {
        setFWversionInfo(p_ver);
      }
      DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: is first init rsp: %d", func, firstInitRsp);
      if (firstInitRsp && status == 0x00)  // STATUS_OK
      {
        HalCoreInitialized(data_len, p_data);  // POST INIT
        firstInitRsp = false;
      }
    }
  }
...
/* START_SLSI [S15052702] Notify firmware downloading status */
  else if (mt == 0x60)  // NCI_NTF
  {
    if (gid == 0x00 && oid == 0x00) //CORE_RESET_NTF
    {
      if (iNciVersion == NCI_VERSION_2_0)
      {
        setFWversionInfo(p_ver);
        HalWrite(5, nci_init_cmd_nci2);  // CORE_INIT_CMD (NCI 2.0)
      }
    }
    else if (gid == 0x0f && oid == 0xff)
    {
      DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: (checkFirmware) Firmware download status=%d", __FUNCTION__,
            status);
      //notifyDownloadFirmwareStatus(status);
    }
  }
/* END[181106] Patch for supporting NCI v2.0 */
/* END_SLSI [S15052702] Notify firmware downloading status */
#endif
}
