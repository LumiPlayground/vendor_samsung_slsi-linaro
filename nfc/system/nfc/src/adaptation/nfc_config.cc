...
std::string findConfigPath() {
  ...
  // [sec] Search for libnfc-sec.conf
  f_path = searchConfigPath("libnfc-sec.conf");
  if (!f_path.empty()) {
    LOG(INFO) << StringPrintf("findConfigPath : load libnfc-sec.conf");
    return f_path;
  }
...
void NfcConfig::loadConfig() {
  ...
  char PATH_FACTORY_RF_CONF[60] = "/vendor/etc/libnfc-nci-factory.conf";
  ...
  struct stat file_stat1;
    if (stat(PATH_FACTORY_RF_CONF, &file_stat1) == 0) {
      LOG(ERROR) << StringPrintf("in factory mode,config file exist and load factory config file!!");
      config_.setFactoryConf(true);
      config_.parseFromFile(PATH_FACTORY_RF_CONF);
      config_.setFactoryConf(false);
    } else {
      LOG(ERROR) << StringPrintf("in factory mode,config file isn't exist config file!!");
    }
  }
