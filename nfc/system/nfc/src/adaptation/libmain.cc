...
std::string getFilenameForBlock2(const unsigned block) {
  std::string bin = "nfaStorage2.bin";
  return StringPrintf("%s/%s%u", nfc_storage_path.c_str(), bin.c_str(), block);
}
#endif
...
**
*******************************************************************************/
extern bool nfa_nv_co_read2(uint8_t* pBuffer, uint16_t nbytes, uint8_t block,
                            uint8_t binType) {
  bool ret = TRUE;
  std::string filename;
  if (binType != 0x02)
    filename = getFilenameForBlock(block);
  else
    filename = getFilenameForBlock2(block);
...
**
*******************************************************************************/
extern bool nfa_nv_co_write2(const uint8_t* pBuffer, uint16_t nbytes,
                             uint8_t block, uint8_t binType) {
  bool ret = TRUE;
  std::string filename;
  if (binType != 0x02)
    filename = getFilenameForBlock(block);
  ...
  int fileStream = open(filename.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
  if (fileStream >= 0) {
