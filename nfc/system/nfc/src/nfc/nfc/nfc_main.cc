...
/* START [190514][NFC-2622] - Patch for memory leak */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  do {
  p_msg = (tNFC_HAL_EVT_MSG*)GKI_getbuf(sizeof(tNFC_HAL_EVT_MSG));
    if (p_msg == NULL)
      LOG(ERROR) << StringPrintf("No buffer / retry");
  } while (p_msg == NULL); //retry
...
/* END [190514][NFC-2622] - Patch for memory leak */
#else
  p_msg = (tNFC_HAL_EVT_MSG*)GKI_getbuf(sizeof(tNFC_HAL_EVT_MSG));
  if (p_msg != nullptr) {
    /* Initialize NFC_HDR */
...
tNFC_STATUS NFC_Deactivate(tNFC_DEACT_TYPE deactivate_type) {
  ...
  if (nfc_cb.nfc_state == NFC_STATE_OPEN) {
    ...
    if ((p_cb->act_protocol == NCI_PROTOCOL_NFC_DEP) &&
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("DTA_TIMEOUT_MARGIN: %d", DTA_TIMEOUT_MARGIN);
      nfc_cb.flags |= NFC_FL_DEACTIVATING;
