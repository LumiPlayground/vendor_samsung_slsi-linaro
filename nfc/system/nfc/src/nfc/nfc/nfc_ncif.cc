...
/* START_SLSI [S14111902] Get a firmware version information (S14111902 , J15052703) */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
tNFC_FW_VERSION nfc_fw_version;
#endif
/* END_SLSI [S14111902] Get a firmware version information (S14111902 , J15052703) */
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  if (NfcAdaptation::GetInstance().isShutDown() == true) {
    LOG(ERROR) << StringPrintf("already shutdown");
    return;
  }
#endif
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  if (nfc_cb.nfc_state == NFC_STATE_W4_HAL_CLOSE) {
    LOG(ERROR) << StringPrintf("nfc_ncif_proc_deactivate: Unexpected call");
  } else {
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  LOG(ERROR) << StringPrintf(
      "nfc_ncif_proc_deactivate - evt_data.deactivate.reason %d, nfc_cb.deact_reason = %d",
...
**
*******************************************************************************/
void nfc_extract_FWVersion(uint8_t* p_buf) {
  // Get Version ptr
  // p_buf[2] + 3 = lengh of p_buf = 0x16, ((p_buf[2]+2) -4) = starting ptr
  // of version info.
  uint8_t* p_ver = &p_buf[(p_buf[2] + 3) - 4];
  ...
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "FW Version : %x.%x.%d", nfc_fw_version.major_version,
...
**
*******************************************************************************/
tNFC_FW_VERSION NFC_getFWVersion() { return nfc_fw_version; }
#endif
/* END_SLSI [S14111902] Get a firmware version information (S14111902 , J15052703) */
...
/* START [S14111906] if NCI_PROTOCOL_15693, the status byte needs to be removed */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
        ...
        if ( (NFC_GetNCIVersion() == NCI_VERSION_1_0) &&
          ...
          data_cevt.status = *(p + p_evt->offset + p_evt->len);
          if (data_cevt.status != NFC_STATUS_OK) {
            (*p_cb->p_cback)(p_cb->conn_id, NFC_ERROR_CEVT,
                             (tNFC_CONN*)&data_cevt);
            GKI_freebuf(p_evt);
            p_evt = nullptr;
            return;
...
#endif
/* END [S14111906] if NCI_PROTOCOL_15693, the status byte needs to be removed*/
      }
      tNFC_CONN nfc_conn;
