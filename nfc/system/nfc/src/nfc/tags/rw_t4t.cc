...
/* START [19040807] Add a log for reporting T4T error */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    LOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_t4t_sm_detect_ndef (): status=0x%02X", NFC_STATUS_CMD_NOT_CMPLTD);
#endif
/* END [19040807] Add a log for reporting T4T error */
    rw_t4t_handle_error(NFC_STATUS_CMD_NOT_CMPLTD, *(p - 2), *(p - 1));
    return;
  }
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
          rw_t4t_handle_error (NFC_STATUS_WRONG_SIZE_BAD_RESP, 0, 0);
#else
          rw_t4t_handle_error(NFC_STATUS_BAD_RESP, 0, 0);
#endif
...
/* START [J16031100][NFC-2691] - Abnormal T4T Read Patch */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "sub_state (0x%x) , apdu_len (0x%x)", p_t4t->sub_state, p_r_apdu->len);
  if ( (p_t4t->state == RW_T4T_STATE_DETECT_NDEF) &&
      (p_t4t->sub_state == RW_T4T_SUBSTATE_WAIT_SELECT_APP) &&
      (p_r_apdu->len < T4T_RSP_STATUS_WORDS_SIZE) ) {
    LOG(ERROR) << StringPrintf("%s Got error reponse during NDEF checking", __func__);
    rw_t4t_handle_error(NFC_STATUS_CMD_NOT_CMPLTD, 0, 0);
    GKI_freebuf(p_r_apdu);
    return;
  }
#endif
/* END [J16031100][NFC-2691] - Abnormal T4T Read Patch */
...
tNFC_STATUS RW_T4tPresenceCheck(uint8_t option) {
  ...
  /* If command is pending, assume tag is still present */
    ...
    /* [SEC] READ_BINARY will be converted to ISO_DEP_NAK at firmware side
       Because ISO_DEP_NAK is supported on NCI 2.0
       Check also PRESENCE_CHECK_METHOD2_EN_IDX on RF option
     */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    else if (option == RW_T4T_CHK_READ_BINARY_CH3) {
