...
/* START [NSM750] Patch for NFC stuck during detect T3T Ndef */
      if ((p_cb->flags & (RW_T3T_FL_W4_PRESENCE_CHECK_POLL_RSP |
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("Reset wait flag : flag = 0x%x", p_cb->flags);
        p_cb->flags = 0x00;
      }
/* END [NSM750] Patch for NFC stuck during detect T3T Ndef */
...
void rw_t3t_message_set_block_list(tRW_T3T_CB* p_cb, uint8_t** p,
  ...
  *p_msg_num_services = num_services;
  ...
  for (i = 0; i < num_blocks; i++) {
    ...
    /* START_SLSI [S14111903] Add the block_extend field for T3T tag */
...
#endif
    {
      /* END_SLSI [S14111903] Add the block_extend field for T3T tag */
