...
/* START [S14111901] Get a detail_status from NDEF_DETECT_EVT */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    conn_evt_data.ndef_detect.detail_status = NFA_STATUS_OK;
#endif
/* END [S14111901] Get a detail_status from NDEF_DETECT_EVT */
...
/* START [S14111901] Get a detail_status from NDEF_DETECT_EVT */
      ...
      LOG_IF(INFO, nfc_debug_enabled) << StringPrintf
        ("%s: conn_evt_data.ndef_detect.detail_status = 0x%X",
        __func__, conn_evt_data.ndef_detect.detail_status);
#endif
/* END [S14111901] Get a detail_status from NDEF_DETECT_EVT */
...
/* START [S18101001]  Patch for supporting CHK_RB_CH0, CHK_RB_CH3 and CHK_RESET*/
     ...
     case NFA_RW_PRES_CHK_RB_CH3:
        ...
        break;
/* END [S18101001]  Patch for supporting CHK_RB_CH0, CHK_RB_CH3 and CHK_RESET*/
...
/* START [S18060801] - Recovery Mifare CheckNdef Timeout */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
/*******************************************************************************
**
** Function         nfa_rw_raw_data_timeout
**
** Description
**
** Returns
**
*******************************************************************************/
bool nfa_rw_raw_data_timeout(__attribute__((unused)) tNFA_RW_MSG* p_data) {
    tNFA_CONN_EVT_DATA evt_data;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_rw_raw_data_timeout");
    evt_data.data.status = NFA_STATUS_TIMEOUT;
...
#endif
/* END [S18060801] - Recovery Mifare CheckNdef Timeout */
...
/* START [S16062000] Release GKI_freebuf for unsupported tag protocols */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  else if (event == NFC_DATA_CEVT) {
    p_msg = (NFC_HDR*)p_data->data.p_data;
    if (p_msg != nullptr) GKI_freebuf(p_msg);
  }
#endif
/* END [S16062000] Release GKI_freebuf for unsupported tag protocols */
...
/* START [S18060801] - Recovery Mifare CheckNdef Timeout */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      nfa_sys_start_timer(&nfa_rw_cb.tle, NFA_RW_RAW_DATA_TIMEOUT_EVT, 1000); //1000ms
#endif
/* END [S18060801] - Recovery Mifare CheckNdef Timeout */
      /* Clear the busy flag */
      nfa_rw_cb.flags &= ~NFA_RW_FL_API_BUSY;
