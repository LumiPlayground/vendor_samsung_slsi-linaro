      ...
      if ((dest_gate == NFA_HCI_IDENTITY_MANAGEMENT_GATE) ||
        ...
        if ((pgate = nfa_hciu_find_gate_by_gid(dest_gate)) != nullptr) {
          ...
          if (nfa_hciu_check_pipe_between_gates(dest_gate, source_host, source_gate)) {
            if ((dest_gate == NFA_HCI_CONNECTIVITY_GATE
                  && dest_gate == source_gate
                  && source_host == 0x03)
                || (dest_gate == 0xf0
                  && dest_gate == source_gate
                  && source_host == 0x03)) {
              ...
              LOG_IF(INFO, nfc_debug_enabled)
                  << StringPrintf("HCI Patch - Start [0x%02X] pipe re-assign.", dest_gate);
              ...
              for (xx = 0, pp = nfa_hci_cb.cfg.dyn_pipes; xx < NFA_HCI_MAX_PIPE_CB; xx++, pp++) {
                if ((pp->pipe_id != 0)
                  ...
                  && (pp->local_gate == dest_gate)
                  && (pp->dest_host == source_host)
                  && (pp->dest_gate == source_gate)) {
                  LOG_IF(INFO, nfc_debug_enabled)
                      << StringPrintf("HCI Patch - Pipe re-assigned. removed - 0x%02X, re-assign - 0x%02X", pp->pipe_id, pipe);
                  remove_pipe = pp->pipe_id;
                }
              }
              if (remove_pipe != -1)
                response = nfa_hciu_release_pipe(remove_pipe);
            }
          }
#endif
