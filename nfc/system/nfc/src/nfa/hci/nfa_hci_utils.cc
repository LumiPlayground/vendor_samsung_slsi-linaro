...
/* START [160503001S] Configure HCI setting for wired C/E by eSE chip type (Oberthure or Gemalto)*/
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("HCI Patch - p_gate->gate_owner(0x%02X)", p_gate->gate_owner);

      // Patch for no  SecureElement object at this time.
      if (p_gate->gate_owner == 0x00)
        p_gate->gate_owner = NFA_HANDLE_GROUP_HCI;
#endif
/* END [160503001S] Configure HCI setting for wired C/E by eSE chip type (Oberthure or Gemalto)*/
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  nfa_hci_cb.dh_is_occurred_clear_all_pipe = true;
#endif
