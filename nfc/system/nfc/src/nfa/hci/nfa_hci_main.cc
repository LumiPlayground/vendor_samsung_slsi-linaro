...
/* START [P1604040001] - Support Dual-SIM solution */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
static int nPreferredSimSlot = 0x01;
#endif
/* END [P1604040001] - Support Dual-SIM solution */
/*****************************************************************************
**  Static Functions
...
/*******************************************************************************
**
** Function         nfa_hci_setPreferredSimSlot
**
** Description
**
** Returns          sim slot number
**
*******************************************************************************/
int nfa_hci_setPreferredSimSlot(uint16_t sim_slot) {
   DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_setPreferredSimSlot () : SIM Slot = 0x%x",
                   sim_slot);

  nPreferredSimSlot = sim_slot;
  return nPreferredSimSlot;
}
#endif
/* END [P1604040001] - Support Dual-SIM solution */
...
/* START [P1604040001] - Support Dual-SIM solution */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  nfa_nv_co_read2((uint8_t*)&nfa_hci_cb.cfg, sizeof(nfa_hci_cb.cfg),
                  DH_NV_BLOCK, (uint8_t)nPreferredSimSlot);
/* END [P1604040001] - Support Dual-SIM solution */
#else
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    bool ret = true;
    ret = nfa_nv_co_write2((uint8_t*)&nfa_hci_cb.cfg, sizeof(nfa_hci_cb.cfg),
                           DH_NV_BLOCK, (uint8_t)nPreferredSimSlot);

    /* Does need to copy nv_storage fiile. */
    if (nPreferredSimSlot == 0x02) {
      if ((nfa_hci_cb.dh_is_occurred_clear_all_pipe) ||
          (open("/data/nfc/nfaStorage.bin1", O_RDONLY) < 0)) {
        nfa_hci_cb.dh_is_occurred_clear_all_pipe = false;
        ret =
            nfa_nv_co_write2((uint8_t*)&nfa_hci_cb.cfg, sizeof(nfa_hci_cb.cfg),
                             DH_NV_BLOCK, (uint8_t)0x01);
      }
    } else {
      if ((nfa_hci_cb.dh_is_occurred_clear_all_pipe) ||
          (open("/data/nfc/nfaStorage2.bin1", O_RDONLY) < 0)) {
        nfa_hci_cb.dh_is_occurred_clear_all_pipe = false;
        ret =
            nfa_nv_co_write2((uint8_t*)&nfa_hci_cb.cfg, sizeof(nfa_hci_cb.cfg),
                             DH_NV_BLOCK, (uint8_t)0x02);
      }
    }
/* END [P1604040001] - Support Dual-SIM solution */
#else
