...
#endif
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
bool is_wait_ee_discovery = true;
#endif
...
/* START [S17010103] ISO 7816 - Add Aid Default Route */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    NFA_PROTOCOL_MASK_ISO_DEP, NFA_PROTOCOL_MASK_NFC_DEP, NFA_PROTOCOL_MASK_ISO7816};
#else
    NFA_PROTOCOL_MASK_ISO_DEP, NFA_PROTOCOL_MASK_NFC_DEP};
#endif
/* END [S17010103] ISO 7816 - Add Aid Default Route */

const uint8_t nfa_ee_proto_list[NFA_EE_NUM_PROTO] = {
    NFC_PROTOCOL_T1T, NFC_PROTOCOL_T2T, NFC_PROTOCOL_T3T, NFC_PROTOCOL_ISO_DEP,
/* START [S15110302] Add ISO7816 parameter for AID default routing */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    NFC_PROTOCOL_NFC_DEP, NFC_PROTOCOL_ISO7816};
#else
    NFC_PROTOCOL_NFC_DEP};
#endif
/* END [S15110302] Add ISO7816 parameter for AID default routing */

/* START [S14111810] Get LMRT table size for AID */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
uint8_t NFA_REMOVE_ALL_AID[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
#endif
/* END [S14111810] Get LMRT table size for AID */
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    if (power_cfg & NCI_ROUTE_PWR_STATE_ON) { // [SEC] Set power sub state on NCI1.0
#else
...
/* START [18072401S] sys_code not used in NCI1.0 */
  if (NFC_GetNCIVersion() == NCI_VERSION_2_0) {
    lmrt_size += p_cb->size_sys_code;
  }
/* END [18072401S] sys_code not used in NCI1.0 */
/* START [NFC3123] Prevent OOB when nfa_ee_cb.cur_ee is zero */
  if (nfa_ee_cb.cur_ee > 0) p_cb = &nfa_ee_cb.ecb[nfa_ee_cb.cur_ee - 1];
/* END [NFC3123] Prevent OOB when nfa_ee_cb.cur_ee is zero */
...
static void nfa_ee_add_tech_route_to_ecb(tNFA_EE_ECB* p_cb, uint8_t* pp,
  ...
  for (int xx = 0; xx < NFA_EE_NUM_TECH; xx++) {
    ...
    if (power_cfg) {
/* START [19101801][NFC-2738] Patch for blocked EE for NCI v2.0 */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      uint8_t tech_tag = NFC_ROUTE_TAG_TECH | nfa_ee_cb.route_block_control;
      add_route_tech_proto_tlv(&pp, tech_tag, p_cb->nfcee_id,
                               power_cfg, nfa_ee_tech_list[xx]);
/* END [19101801][NFC-2738] Patch for blocked EE for NCI v2.0 */
#else
...
static void nfa_ee_add_proto_route_to_ecb(tNFA_EE_ECB* p_cb, uint8_t* pp,
  ...
  for (int xx = 0; xx < NFA_EE_NUM_PROTO; xx++) {
    ...
    if (p_cb->proto_battery_off & nfa_ee_proto_mask_list[xx])
      power_cfg |= NCI_ROUTE_PWR_STATE_BATT_OFF;
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE) // [SEC] Set power sub state
    if (p_cb->proto_screen_lock & nfa_ee_proto_mask_list[xx])
      power_cfg |= NCI_ROUTE_PWR_STATE_SCREEN_ON_LOCK();
    if (p_cb->proto_screen_off & nfa_ee_proto_mask_list[xx])
      power_cfg |= NCI_ROUTE_PWR_STATE_SCREEN_OFF_UNLOCK();
...
int nfa_ee_find_max_aid_cfg_len(void) {
  ...
  if (max_lmrt_size) {
    if (NFA_GetNCIVersion() == NCI_VERSION_1_0) {
      return max_lmrt_size - NFA_EE_MAX_PROTO_TECH_EXT_ROUTE_LEN_1_0;
    } else if (NFA_GetNCIVersion() == NCI_VERSION_2_0) {
      return max_lmrt_size - NFA_EE_MAX_PROTO_TECH_EXT_ROUTE_LEN_2_0;
    } else {
      LOG(ERROR) << StringPrintf("Unknown NCI Version!");
      return max_lmrt_size - NFA_EE_MAX_PROTO_TECH_EXT_ROUTE_LEN_1_0;
    }
  } else {
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE) /* START_LSI [S14111810] */
  /*Clear All AIDs*/
    ...
    for (xx = 0; xx < NFA_EE_MAX_EE_SUPPORTED; xx++, p_cb++) {
      ...
      p_cb->aid_entries = 0;
      p_cb->size_aid = 0; /* START_SLSI [S14120501] */
      nfa_ee_cb.ee_cfged |= nfa_ee_ecb_to_mask(p_cb);
    }
    ...
    p_cb->size_aid = 0; /* START_SLSI [S14120501] */
                        /*
                            Temporarily add callback to update routing
                            Need to source change by NXP
                        */
    p_cb->ecb_flags |= NFA_EE_ECB_FLAGS_AID;
...
void nfa_ee_nci_disc_ntf(tNFA_EE_MSG* p_data) {
  ...
  if ((nfa_ee_cb.cur_ee == nfa_ee_max_ee_cfg) &&
      (nfa_ee_cb.em_state == NFA_EE_EM_STATE_INIT_DONE)) {
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    if (is_wait_ee_discovery == true) {
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("Do not expire EE_DISCV timer during creation pipe");
      is_wait_ee_discovery = false;
      return;
    }
#endif
    if (nfa_ee_cb.discv_timer.in_use) {
...
void nfa_ee_nci_action_ntf(tNFA_EE_MSG* p_data) {
  ...
  memcpy(evt_data.buffer_t, p_cbk->buffer_test, sizeof(p_cbk->buffer_test));
...
tNFA_STATUS nfa_ee_route_add_one_ecb(tNFA_EE_ECB* p_cb, int* p_max_len,
  ...
  nfa_ee_check_set_routing(p_cb->size_mask_proto+p_cb->size_mask_tech, p_max_len, ps, p_cur_offset);
...
/* START_SLSI [S14111806] Add power sub-state for configuring LMRT */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    if (p_cb->tech_screen_off & nfa_ee_tech_mask_list[xx])
      power_cfg |= NCI_ROUTE_PWR_STATE_SCREEN_OFF_UNLOCK();
    if (p_cb->tech_screen_lock & nfa_ee_tech_mask_list[xx])
    ...
    if (p_cb->tech_screen_off_lock & nfa_ee_tech_mask_list[xx])
      power_cfg |= NCI_ROUTE_PWR_STATE_SCREEN_OFF_LOCK();
#endif
/* END_SLSI [S14111806] Add power sub-state for configuring LMRT */
...
/* END_SLSI [S14111806] Add power sub-state for configuring LMRT */
  ...
  *ps = num_tlv;
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("ee_cfg_sts:0x%02x lmrt_size:%d", nfa_ee_cb.ee_cfg_sts,
                   tlv_size);
...
/* START [S19040501] Modify nfa_ee_lmrt_to_nfcc() function for NCI v2.0 and NCI v1.0 */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
  if ((NFC_GetNCIVersion() == NCI_VERSION_2_0)) {
#endif
/* END [S19040501] Modify nfa_ee_lmrt_to_nfcc() function for NCI v2.0 and NCI v1.0 */
  int xx;
  tNFA_EE_ECB* p_cb;
  uint8_t* p = nullptr;
  bool more = true;
      ...
      *p = 0;
      ...
      if (status == NFA_STATUS_OK) {
        ...
        for (xx = 0; (xx < nfa_ee_cb.cur_ee) && more; xx++, p_cb++) {
          ...
          if (p_cb->ee_status == NFC_NFCEE_STATUS_ACTIVE) {
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfcee_id:0x%x, last_active: 0x%x", p_cb->nfcee_id,
                             last_active);
