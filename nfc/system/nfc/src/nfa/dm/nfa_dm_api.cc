...
**
*******************************************************************************/
tNFA_STATUS NFA_SetPreferredSimSlot(uint16_t nPreferredSimSlot) {
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_SetPreferredSimSlot () nPreferredSimSlot=%d",
                 nPreferredSimSlot);
...
tNFA_STATUS NFA_SetListenTechMask(tNFA_TECHNOLOGY_MASK tech_mask) {
  tNFA_DM_API_SET_P2P_LISTEN_TECH* p_msg;
  ...
  p_msg = (tNFA_DM_API_SET_P2P_LISTEN_TECH*)GKI_getbuf(
        sizeof(tNFA_DM_API_SET_P2P_LISTEN_TECH));
  if (p_msg != nullptr) {
    p_msg->hdr.event = NFA_DM_API_LISTEN_TECH_MASK_EVT;
    p_msg->tech_mask = tech_mask;
