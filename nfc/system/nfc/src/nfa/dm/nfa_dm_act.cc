...
static uint16_t sPreferredSimSlot = SIM_SLOT_N1;
static bool nfa_dm_VS_data(uint8_t oid, uint8_t* p_raw_data, uint16_t data_len,
...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
   DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_set_init_nci_params (): Select SIM Slot =%d",
                   sPreferredSimSlot);
  uint8_t vs_cmd[] = {0x01};
  vs_cmd[0] = (uint8_t)sPreferredSimSlot;
...
static bool nfa_dm_VS_data(uint8_t oid, uint8_t* p_raw_data, uint16_t data_len,
  ...
  p_data = (NFC_HDR*)GKI_getpoolbuf(NFC_NCI_POOL_ID);
  if (p_data) {
    p_data->offset = sizeof(tNFA_DM_API_SEND_VSC) - NFC_HDR_SIZE;
    p = (uint8_t*)(p_data + 1) + p_data->offset;
    memcpy(p, p_raw_data, data_len);
    p_data->len = data_len;

     DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("Send data (0x%x)", data_len);

    status = NFC_SendVsCommand(oid, p_data, p_cback);

    if (NFC_STATUS_OK != status)
       DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: data sending failed", __FUNCTION__);
  }
...
bool nfa_dm_act_set_listen_tech_mask(tNFA_DM_MSG* p_data) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("tech_mask = %d", p_data->set_p2p_listen_tech.tech_mask);

  nfa_dm_set_listen_tech_mask(p_data->set_p2p_listen_tech.tech_mask);
  nfa_dm_conn_cback_event_notify(NFA_SET_LISTEN_TECH_MASK_EVT, nullptr);
...
/*******************************************************************************
**
** Function         nfa_dm_act_set_preferred_simslot
**
** Description      set preferred sim slot
**
** Returns          TRUE
**
*******************************************************************************/
bool nfa_dm_act_set_preferred_simslot(uint16_t nPreferredSimSlot) {
  sPreferredSimSlot = nPreferredSimSlot;
  nfa_hci_setPreferredSimSlot(sPreferredSimSlot);
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "nfa_dm_act_set_preferred_simslot() : sPreferredSimSlot = %d",
      sPreferredSimSlot);
