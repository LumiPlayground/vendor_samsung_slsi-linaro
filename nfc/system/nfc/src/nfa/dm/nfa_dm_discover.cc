...
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      int DTA_TIMEOUT_MARGIN = 0;
      if (nfa_dta_cb.dta_flags & 0x01) { // NFA_DTA_FL_ENABLED = 0x01
        DTA_TIMEOUT_MARGIN = 3;
      }
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("DTA_TIMEOUT_MARGIN2 :%d", DTA_TIMEOUT_MARGIN);
      nfa_sys_start_timer(&nfa_dm_cb.disc_cb.tle, 0,
                          (DTA_TIMEOUT_MARGIN * 1000)+
...
void nfa_dm_start_rf_discover(void) {
  ...
  if (nfa_dm_cb.disc_cb.listen_tech_mask_entry.in_use) {
    bool is_p2p_enable = false;
    if (dm_disc_mask & NFA_DM_DISC_MASK_LF_NFC_DEP)
      is_p2p_enable = true;
    dm_disc_mask &= ~(NFA_DM_DISC_MASK_LA_ISO_DEP | NFA_DM_DISC_MASK_LB_ISO_DEP |
                      NFA_DM_DISC_MASK_LF_T3T | NFA_DM_DISC_MASK_LF_NFC_DEP);
    dm_disc_mask |= nfa_dm_cb.disc_cb.listen_tech_mask_entry.selected_disc_mask;
    if (is_p2p_enable && (dm_disc_mask & NFA_DM_DISC_MASK_LF_T3T))
      dm_disc_mask |= NFA_DM_DISC_MASK_LF_NFC_DEP;
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("listen tech mask is enabled, dm_disc_mask = 0x%x", dm_disc_mask);
  }
#endif
...
/* START [S18112701] - Update deactivate reason */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      LOG(ERROR) << StringPrintf(
          "NFA_DM_RF_DEACTIVATE_NTF p_data->nfc_discover.deactivate.reason = %d",
          p_data->nfc_discover.deactivate.reason);
#endif
/* END [S18112701] - Update deactivate reason */
...
static void nfa_dm_disc_sm_listen_active(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    case NFA_DM_RF_DEACTIVATE_CMD:
      ...
      break;
    case NFA_DM_RF_DEACTIVATE_RSP:
/* START [S14111811] Patch for RF_DEACTIVATE_RSP error response at listen mode */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
      if (p_data->nfc_discover.status != NCI_STATUS_OK) {
        nfa_dm_cb.disc_cb.disc_flags &=
            ~(NFA_DM_DISC_FLAGS_W4_RSP | NFA_DM_DISC_FLAGS_W4_NTF);
...
#endif
/* END [S14111811] Patch for RF_DEACTIVATE_RSP error response at listen mode */
      nfa_dm_cb.disc_cb.disc_flags &= ~NFA_DM_DISC_FLAGS_W4_RSP;
      if (!(nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_NTF)) {
        /* it's race condition. received deactivate NTF before receiving RSP */
...
#ifdef NFC_SEC_NOT_OPEN_INCLUDED
void nfa_dm_set_listen_tech_mask(tNFA_TECHNOLOGY_MASK tech_mask) {
  tNFA_DM_DISC_TECH_PROTO_MASK listen_tech_mask = 0;

  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("tech_mask = 0x%x", tech_mask);

  nfa_dm_cb.disc_cb.listen_tech_mask_entry.in_use = true;
  if (tech_mask & NFA_TECHNOLOGY_MASK_A)
    listen_tech_mask |= NFA_DM_DISC_MASK_LA_ISO_DEP;
  if (tech_mask & NFA_TECHNOLOGY_MASK_B)
    listen_tech_mask |= NFA_DM_DISC_MASK_LB_ISO_DEP;
  if (tech_mask & NFA_TECHNOLOGY_MASK_F)
    listen_tech_mask |= NFA_DM_DISC_MASK_LF_T3T;

  nfa_dm_cb.disc_cb.listen_tech_mask_entry.selected_disc_mask = listen_tech_mask;
}
#endif
