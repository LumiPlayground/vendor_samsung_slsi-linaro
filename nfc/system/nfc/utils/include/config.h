...
class ConfigFile {
 public:
  ConfigFile(){
    is_factory_conf = false; // [SEC] Factory config
  }
  ...
  void updateConfig(const std::string& config, ConfigValue& value);
  ...
  void setFactoryConf(const bool); // [SEC] Factory config

 private:
  ConfigValue& getValue(const std::string& key);

  std::map<std::string, ConfigValue> values_;
  bool is_factory_conf; // [SEC] Factory config
};
