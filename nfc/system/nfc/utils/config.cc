...
void ConfigFile::updateConfig(const std::string& key, ConfigValue& value) {
  std::map<std::string, ConfigValue>::iterator it;
  it = values_.find(key);
  if (it == values_.end()) {
    values_.emplace(key, value);
    LOG(INFO) << "Can't find key, added it : " << key << "'";
  } else {
    values_.erase(it);
    values_.emplace(key, value);
...
void ConfigFile::setFactoryConf(bool factory) {
  is_factory_conf = factory;
}