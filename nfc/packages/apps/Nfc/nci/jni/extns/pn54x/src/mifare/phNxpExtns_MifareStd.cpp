...
/* START [2020021302] - Patch for Mifare Classic Tag with Size Error (Empty Size Tag) */
    if(NdefInfo.NdefLength > NdefInfo.NdefActualSize)
    {
        uint32_t max_size = NdefInfo.NdefLength;
        uint32_t real_size = NdefInfo.NdefActualSize;
        DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("Mifare Tag Size Err: max = 0x%x, real = 0x%x", max_size, real_size);
    }
...
/* START [18072301] Patch for S.LSI's NFC Chip */
    for(unsigned int i=0; i<len; i++)
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: pdata[%d]: 0x%02X", __func__,i,p_data[i]);
...
Mfc_WrNdefEnd:
  ...
  DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: exit, status: 0x%02X", __func__, status);
  return status;
}
/*******************************************************************************
**
...
void Mfc_DeactivateCbackSelect(void) {
  ...
  if (NFA_STATUS_OK !=
      (status = NFA_Select(0x01,
                NFC_GetNCIVersion() == NCI_VERSION_1_0 ? phNciNfc_e_RfProtocolsMifCProtocol : 0x81,
...
NFCSTATUS Mfc_Transceive(uint8_t* p_data, uint32_t len) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: enter, p_data[0]: 0x%02X",__func__,p_data[0]);
...
/* START [J17111001] - To do authentication without checkNdef */
...
/* END [J17111001] */
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: enter, NdefMap->Cmd:0x%02X, phNfc_emifareRaw",
                            __func__,(unsigned int)NdefMap->Cmd.MfCmd);
...
static NFCSTATUS phNciNfc_RecvMfResp(phNciNfc_Buff_t* RspBuffInfo,
  ...
  if (NULL == RspBuffInfo) {
    ...
    if (((PHNCINFC_EXTNID_SIZE + PHNCINFC_EXTNSTATUS_SIZE) >
      ...
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: RecvdExtnRspId = 0x%x", __func__, RecvdExtnRspId);
...
/* END [2020121001][GSI_BUG] Fix Mifare tag write error */
        ...
        default:
        {
            DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: [NFC] no data case", __func__);
            status = NFCSTATUS_FAILED;
        } break;
      }
    }
...
NFCSTATUS Mfc_RecvPacket(uint8_t* buff, uint8_t buffSz) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: NFC status: 0x%02X", __func__, status);
  ...
  if (true == gphNxpExtns_Context.writecmdFlag &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: gphNxpExtns_Context.writecmdFlag", __func__);
    ...
    if (pcmd_buff != NULL) {
      ...
      pcmd_buff = NULL;
             ...
             (NFCSTATUS_SUCCESS == status)) {
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: gphNxpExtns_Context.incrdecflag", __func__);
    pcmd_buff = (uint8_t*)malloc((uint32_t)MAX_BUFF_SIZE);
    if (NULL == pcmd_buff) {
      return NFCSTATUS_FAILED;
    }
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: gphNxpExtns_Context.default", __func__);
...
static NFCSTATUS phNciNfc_MfCreateAuthCmdHdr(
  ...
  if (PHNCINFC_AUTHENTICATION_KEYB == tTranscvInfo.tSendData.pBuff[0]) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s:AUTH_KEYB:0x61, pbuff[2]: 0x%02X", __func__, tTranscvInfo.tSendData.pBuff[2]);
  }
  ...
  DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s:tSendData.pbuff[0]:0x%02X", __func__, tTranscvInfo.tSendData.pBuff[0]);
  ...
  *buffSz = 0x03;
  if (bKey & PH_NCINFC_MIFARECLASSIC_EMBEDDED_KEY) {
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: Embedded key", __func__);
...
static NFCSTATUS phLibNfc_MifareMap(
  ...
  switch (pTransceiveInfo->cmd.MfCmd) {
    case phNfc_eMifareRead16: {
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s:read", __func__);
    ...
    case phNfc_eMifareWrite16: {
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s:write", __func__);
      if ((NULL != pTransceiveInfo->sSendData.buffer) &&
          (0 != (pTransceiveInfo->sSendData.length))) {
    ...
    case phNfc_eMifareAuthentB: {
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s:Authenticate A|B", __func__);
      if ((NULL != pTransceiveInfo->sSendData.buffer) &&
          (0 != (pTransceiveInfo->sSendData.length)) &&
    ...
    case phNfc_eMifareRaw: {
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s:MifareRaw: do nothing", __func__);
    } break;
...
NFCSTATUS phFriNfc_ExtnsTransceive(phNfc_sTransceiveInfo_t* pTransceiveInfo,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: enter, pTans->cmd:0x%02X,length:%d",__func__,pTransceiveInfo->cmd.MfCmd,SendLength);
  if ((Cmd.MfCmd == phNfc_eMifareAuthentA) ||
      (Cmd.MfCmd == phNfc_eMifareAuthentB)) {
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: Cmd.MfCmd=0x%02X",__func__,Cmd.MfCmd);
