...
static NFCSTATUS phNxpExtns_ProcessSysMessage(phLibNfc_Message_t* msg) {
  ...
  if (gphNxpExtns_Context.Extns_status == EXTNS_STATUS_CLOSE) {
    LOG(ERROR) << StringPrintf ("%s: EXTNS_STATUS_CLOSE", __func__);
    return NFCSTATUS_FAILED;
  }
  ...
  switch (msg->eMsgType) {
    case PH_NXPEXTNS_RX_DATA:
      ...
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: status: 0x%02X", __func__, status);
      break;
    ...
    case PH_NXPEXTNS_MIFARE_READ_ONLY:
      DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf ("%s: enter, MIFARE_READ_ONLY", __func__);
...
NFCSTATUS EXTNS_GetPresenceCheckStatus(void) {
  ...
  if (sem_destroy(&gAuthCmdBuf.semPresenceCheck)) {
    LOG(ERROR) << StringPrintf(
        "%s: Failed to destroy check Presence semaphore (errno=%d)", __func__, errno);
  }
