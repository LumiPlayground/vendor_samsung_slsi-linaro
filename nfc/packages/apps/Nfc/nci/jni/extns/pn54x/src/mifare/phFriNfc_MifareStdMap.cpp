...
/* START [18072301] Patch for S.LSI's NFC Chip */
//#include <phNxpLog.h>
...
/* END [18072301] Patch for S.LSI's NFC Chip */
...
NFCSTATUS phFriNfc_MifareStdMap_WrNdef(phFriNfc_NdefMap_t* NdefMap,
  ...
  *NdefMap->WrNdefPacketLength = PH_FRINFC_MIFARESTD_VAL0;
  ...
  if ((NdefMap->CardState == PH_NDEFMAP_CARD_STATE_INVALID) ||
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: Offset == PH_FRINFC_NDEFMAP_SEEK_CUR", __func__);
    if (((Offset == PH_FRINFC_NDEFMAP_SEEK_CUR) &&
         (NdefMap->StdMifareContainer.ReadWriteCompleteFlag ==
          PH_FRINFC_MIFARESTD_FLAG1)) ||
  ...
  if ((Status & PHNFCSTBLOWER) == (NFCSTATUS_SUCCESS & PHNFCSTBLOWER)) {
    switch (NdefMap->State) {
      case PH_FRINFC_NDEFMAP_STATE_CHK_NDEF_COMP:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: CHK_NDEF_COMP", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_READ:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_READ", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_WRITE:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: SATE_WRITE", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_AUTH:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_AUTH", __func__);
        NdefMap->StdMifareContainer.FirstReadFlag = PH_FRINFC_MIFARESTD_FLAG0;
      ...
      case PH_FRINFC_NDEFMAP_STATE_RD_ACS_BIT:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_RF_ACS_BIT", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_WR_NDEF_LEN:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_WR_NDEF_LEN", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_RD_TO_WR_NDEF_LEN:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_RD_TO_WR_NDEF_LEN", __func__);
        CRFlag = PH_FRINFC_MIFARESTD_FLAG1;
        Status =
      ...
      case PH_FRINFC_NDEFMAP_STATE_GET_ACT_CARDSIZE:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_GET_ACT_CARDSIZE", __func__);

/* START [2020021302] - Patch for Mifare Classic Tag with Size Error (Empty Size Tag) */
        // "0xFF00" means a "0x00 - alpha" value.
        if( (NdefMap->StdMifareContainer.remainingSize > 0x0FFF) &&
            (NdefMap->StdMifareContainer.currentBlock >= NdefMap->StdMifareContainer.NoOfNdefCompBlocks) )
        {
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: Size Error Tag :: RemainingSize = 0x%x", __func__, NdefMap->StdMifareContainer.remainingSize);
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: currentBlock = 0x%x", __func__, NdefMap->StdMifareContainer.currentBlock);
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: NoOfNdefCompBlocks = 0x%x", __func__, NdefMap->StdMifareContainer.NoOfNdefCompBlocks);

            if(NdefMap->StdMifareContainer.NoOfNdefCompBlocks == PH_FRINFC_NDEFMAP_MIFARESTD_1KNDEF_COMPBLOCK)
                NdefMap->StdMifareContainer.remainingSize = 0x03FF;
            else if(NdefMap->StdMifareContainer.NoOfNdefCompBlocks == PH_FRINFC_NDEFMAP_MIFARESTD_2KNDEF_COMPBLOCK)
                NdefMap->StdMifareContainer.remainingSize = 0x07FF;
            else
                NdefMap->StdMifareContainer.remainingSize = 0x0FFF;
        }
/* END [2020021302] - Patch for Mifare Classic Tag with Size Error (Empty Size Tag) */
...
/* START [2020021302] - Patch for Mifare Classic Tag with Size Error (Empty Size Tag) */
        if( (NdefMap->State == PH_FRINFC_NDEFMAP_STATE_GET_ACT_CARDSIZE) &&
            (NdefMap->PrevOperation == PH_FRINFC_NDEFMAP_STATE_CHK_NDEF_COMP) &&
            (NdefMap->StdMifareContainer.remainingSize < 0x10) )
        {
            phFriNfc_MifareStdMap_H_Reset(NdefMap);
            NdefMap->State = PH_FRINFC_NDEFMAP_STATE_DISCONNECT;
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: Empty Size Tag Error :: Status = 0x%x", __func__, Status);
        }
/* END [2020021302] - Patch for Mifare Classic Tag with Size Error (Empty Size Tag) */
      ...
      case PH_FRINFC_NDEFMAP_STATE_RD_BEF_WR:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_RF_BEF_WR", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_WR_TLV:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_WR_TLV", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_RD_TLV:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_RD_TLV", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_TERM_TLV:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_TERM_TLV", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_DISCONNECT:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_DISCONNECT", __func__);
        NdefMap->StdMifareContainer.PollFlag = PH_FRINFC_MIFARESTD_FLAG0;
      ...
      case PH_FRINFC_NDEFMAP_STATE_CONNECT:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_CONNECT", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_RD_SEC_ACS_BIT:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_RD_SEC_ACS_BIT", __func__);
      ...
      case PH_FRINFC_NDEFMAP_STATE_WRITE_SEC:
        DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: STATE_WRITE_SEC", __func__);
    ...
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: NdefMap->State == STATE_AUTH", __func__);
    NdefMap->StdMifareContainer.PollFlag = PH_FRINFC_MIFARESTD_FLAG1;
