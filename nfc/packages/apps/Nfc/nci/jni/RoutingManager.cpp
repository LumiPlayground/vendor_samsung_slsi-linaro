...
extern SyncEvent sNfaListenTechMaskCmdEvent;
...
// [SEC] Listen tech mask
uint8_t mListenTechMask = NFA_TECHNOLOGY_MASK_A | NFA_TECHNOLOGY_MASK_B
...
bool RoutingManager::initialize(nfc_jni_native_data* native) {
  ...
  // [SEC] Set default route again according to NCI version
  if (getDefaultEe() != NFC_DH_ID &&
        NFC_GetNCIVersion() == NCI_VERSION_2_0) {
    setDefaultEe(getDefaultEe() | 0x80);
  }

  if (((mDefaultOffHostRoute != 0) || (mDefaultFelicaRoute != 0)) &&
            getEeStatus(mOffHostRouteUicc[0]) != NFA_EE_STATUS_REMOVED) {
    // Wait for EE info if needed
    SyncEventGuard guard(mEeInfoEvent);
    if (!mReceivedEeInfo) {
      LOG(INFO) << fn << "Waiting for EE info";
      if (!mEeInfoEvent.wait(200)) {
        LOG(ERROR) << fn << "Failed to receive req ntf";
      }
    }
  }

  // [SEC] eSE activation and wait REQ_NTF
  if (getEeStatus(mOffHostRouteEse[0]) == NFA_EE_STATUS_INACTIVE) {
    if (activateSecureElement() != NFA_STATUS_OK) {
      LOG(ERROR) << fn << "Failed to activate eSE";
    } else {
      SyncEventGuard guard(meSeEeInfoEvent);
      if (!mReceivedeSEEeInfo) {
        LOG(INFO) << fn << "Waiting for eSE info";
        if (!meSeEeInfoEvent.wait(400)) {
          LOG(ERROR) << fn << "Failed to receive req ntf";
        }
      }
    }
  } else if (getEeStatus(mOffHostRouteEse[0]) == NFA_EE_STATUS_ACTIVE) {
    SyncEventGuard guard(meSeEeInfoEvent);
    if (!mReceivedeSEEeInfo) {
      LOG(INFO) << fn << "Waiting for eSE info";
      if (!meSeEeInfoEvent.wait(400)) {
...
void RoutingManager::nfaEeCallback(tNFA_EE_EVT event,
  ...
  switch (event) {
    ...
    case NFA_EE_ACTION_EVT: {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: NFA_EE_ACTION_EVT; len=0x%s", fn, eventData->action.buffer_t);
      std::string buffer = eventData->action.buffer_t;
      if (action.trigger == NFC_EE_TRIG_SELECT) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: NFA_EE_ACTION_EVT; h=0x%X; trigger=select (0x%X)", fn,
            action.ee_handle, action.trigger);
        // Notify triggered select AID (ISO7816-4)
        routingManager.notifyEeActionProtocol (action.ee_handle,
                                            action.param.protocol, buffer);
...
int RoutingManager::com_android_nfc_cardemulation_doGetDefaultRouteDestination(
    JNIEnv*) {
  int defaultEe = getInstance().mDefaultEe;
  return (defaultEe & 0x0F);
}

int RoutingManager::
    com_android_nfc_cardemulation_doGetDefaultOffHostRouteDestination(JNIEnv*) {
  int defaultOffHostRoute = getInstance().mDefaultOffHostRoute;
  return (defaultOffHostRoute & 0x0F);
}
...
RoutingManager::com_android_nfc_cardemulation_doGetOffHostUiccDestination(
  ...
  for (unsigned int i = 0; i < uicc.size(); i++){
    uicc[i] &= 0x0F;
  }
  CHECK(e);
...
RoutingManager::com_android_nfc_cardemulation_doGetOffHostEseDestination(
  ...
  for (unsigned int i = 0; i < ese.size(); i++){
    ese[i] &= 0x0F;
  }
  CHECK(e);
...
int RoutingManager::
  ...
  int defaultIsoDepRoute = getInstance().mDefaultIsoDepRoute;
  return (defaultIsoDepRoute & 0x0F);
}
...
bool RoutingManager::setDefaultEe(int defaultEe) {
  ...
  if (defaultEe == mOffHostRouteEse[0]) { // [SEC] Fix Proto and Tech to ISO-DEP for eSE
    mDefaultIsoDepRoute = defaultEe;
...
// [SEC] Get EE status
uint8_t RoutingManager::getEeStatus(uint8_t eeHandle) {
  static const char fn[] = "RoutingManager::getEeStatus";

  uint8_t actualNumEe = MAX_NUM_EE;
  tNFA_EE_INFO eeInfo[MAX_NUM_EE];
  memset(&eeInfo, 0, sizeof(eeInfo));
  tNFA_STATUS nfaStat;
  uint8_t status = NFA_EE_STATUS_REMOVED;

  if ((nfaStat = NFA_EeGetInfo(&actualNumEe, eeInfo)) != NFA_STATUS_OK) {
    LOG(ERROR) << StringPrintf("%s: fail get info; error=0x%X", fn, nfaStat);
    return NFA_EE_STATUS_REMOVED;
  }
  if (actualNumEe != 0) {
    for (uint8_t xx = 0; xx < actualNumEe; xx++) {
      if (eeInfo[xx].ee_interface[0] != NCI_NFCEE_INTERFACE_HCI_ACCESS) {
        if (eeInfo[xx].ee_handle == (eeHandle | NFA_HANDLE_GROUP_EE)) {
          status = eeInfo[xx].ee_status;
          break;
        }
      }
    }
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: eeHandle[%d] status %d", fn, eeHandle, status);
  return status;
}

// [SEC] Check supporting technology
bool RoutingManager::isEeSuportTech(uint8_t eeHandle, uint8_t tech) {
  bool isSupport = false;
  for (uint8_t i = 0; i < mEeInfo.num_ee; i++) {
    if (mEeInfo.ee_disc_info[i].ee_handle == (eeHandle | NFA_HANDLE_GROUP_EE)) {
      switch (tech) {
        case NFA_TECHNOLOGY_MASK_A:
          isSupport = mEeInfo.ee_disc_info[i].la_protocol != 0;
          break;
        case NFA_TECHNOLOGY_MASK_B:
          isSupport = mEeInfo.ee_disc_info[i].lb_protocol != 0;
          break;
        case NFA_TECHNOLOGY_MASK_F:
          isSupport = mEeInfo.ee_disc_info[i].lf_protocol != 0;
          break;
      }
    }
  }
...
// [SEC] Get AID matching mode
int RoutingManager::getAidMatchingMode() {
  return getInstance().mAidMatchingMode;
}

void RoutingManager::notifyEeActionProtocol(uint16_t ee_handle, uint8_t protocol, std::string strBuffer) {
  static const char fn[] = "SecureElement::notifyEeActionProtocol";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: ee_handle=%X, protocol=%X", fn, ee_handle, protocol);
  ...
  ScopedLocalRef<jobject> srcJavaString(e, e->NewStringUTF(strBuffer.c_str()));
  CHECK(srcJavaString.get());

  // Generate ee handle
  uint16_t gen_handle = 0;
  if ((ee_handle & ~NFA_HANDLE_GROUP_EE) == mOffHostRouteEse[0])
      gen_handle = 0x01; //SEID_GEN_ESE
  else if ((ee_handle & ~NFA_HANDLE_GROUP_EE) == mOffHostRouteUicc[0])
      gen_handle = 0x02; //SEID_GEN_UICC

  e->CallVoidMethod(mNativeData->manager,
          android::gCachedNfcManagerNotifyEeActionProtocol,
          gen_handle, protocol, srcJavaString.get());
}

int RoutingManager::getListenTechMask() {
  return mListenTechMask ;
}

void RoutingManager::setListenTechMask(int techMask) {
  static const char fn[] = "RoutingManager::setListenTechMask()";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: techMask=%X", fn, techMask);
  tNFA_STATUS nfaStat;

  SyncEventGuard guard(sNfaListenTechMaskCmdEvent);
  nfaStat = NFA_SetListenTechMask(techMask);
  if (nfaStat == NFA_STATUS_OK) {
    sNfaListenTechMaskCmdEvent.wait();
    mListenTechMask  = techMask;
  } else {
    LOG(ERROR) << fn << "Fail to set listen tech mask";
  }
}

void RoutingManager::abort() {
  SyncEventGuard guard(mEeUpdateEvent);
  mEeUpdateEvent.notifyOne();
}

void RoutingManager::clearRoute() {
  static const char fn[] = "RoutingManager::clearRoute()";
  tNFA_STATUS nfaStat;

  tNFA_PROTOCOL_MASK protoMask = NFA_PROTOCOL_MASK_ISO_DEP | NFA_PROTOCOL_MASK_T3T | NFA_PROTOCOL_MASK_ISO7816;
  tNFA_TECHNOLOGY_MASK techMask = NFA_TECHNOLOGY_MASK_A | NFA_TECHNOLOGY_MASK_B | NFA_TECHNOLOGY_MASK_F;

  nfaStat = NFA_EeClearDefaultProtoRouting(NFC_DH_ID, protoMask);
  if (nfaStat != NFA_STATUS_OK)
    LOG(ERROR) << fn << "Failed to clear protocol routing for DH.";
  nfaStat = NFA_EeClearDefaultTechRouting(NFC_DH_ID, techMask);
  if (nfaStat != NFA_STATUS_OK)
    LOG(ERROR) << fn << "Failed to clear technology routing for DH.";

  for (uint8_t i = 0; i < mEeInfo.num_ee; i++) {
    tNFA_HANDLE eeHandle = mEeInfo.ee_disc_info[i].ee_handle;
    nfaStat = NFA_EeClearDefaultProtoRouting(eeHandle, protoMask);
    if (nfaStat != NFA_STATUS_OK)
      LOG(ERROR) << fn << "Failed to clear EE protocol routing.";
    nfaStat = NFA_EeClearDefaultTechRouting(eeHandle, techMask);
    if (nfaStat != NFA_STATUS_OK)
      LOG(ERROR) << fn << "Failed to clear EE technology routing.";
  }
  if (nfaStat == NFA_STATUS_OK)
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << ": Succeed to clear route";
}
