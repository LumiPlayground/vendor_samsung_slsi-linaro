...
NfcTag::NfcTag()
    : mNumTechList(0),
/* START [18042401J] - KOVIO Tag reactive time interval */
      mKovioReactiveTimeInterval (2000),
/* END [18042401J] - KOVIO Tag reactive time interval */
      ...
      mPresenceCheckAlgorithm(NFA_RW_PRES_CHK_RB_CH0), /* [J16082901] change presence check algorithm value */
      mIsFelicaLite(false),
      mNumDiscNtf(0),
      mNumDiscTechList(0),
      mTechListTail(0),
...
#define SLSI_PATCHFLAG_WAIT_ENABLE_DISCOVERY    0x0002 /* [J14111103] - workaround for rf_info after P2P */
#define SLSI_PATCHFLAG_WAIT_DISABLE_DISCOVERY   0x0004 /* [J14111103] - workaround for rf_info after P2P */
...
void NfcTag::initialize(nfc_jni_native_data* native) {
  ...
  if (NfcConfig::hasKey(NAME_PRESENCE_CHECK_ALGORITHM))
    ...
    mKovioReactiveTimeInterval = 1100;
/* START [17092801J] Add Tech_Type for WCE  */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    // This value shall be updated by the setTransceiveTimeout (TARGET_TYPE_ISO14443_WCE, timeout);
    mTechnologyTimeoutsTable [TARGET_TYPE_ISO14443_WCE] = ISO14443_WCE_TIMEOUT;
#endif
/* END [17092801J] Add Tech_Type for WCE    */
}
...
/* START [18042401J] - KOVIO Tag reactive time interval */
      // same tag
      if (TimeDiff(mLastKovioTime, now) < mKovioReactiveTimeInterval){
/* END [18042401J] - KOVIO Tag reactive time interval */
...
/* START [NFC3128] Patch for NFC3128 (China ID) */
    ...
    mTechList[mNumTechList] = TARGET_TYPE_ISO14443_3B;
/* END [NFC3128] Patch for NFC3128 (China ID) */
  } else {
...
/*******************************************************************************
**
** Function:        isProtocolUnknown
**
** Description:     Whether the currently activated tag is Unknown protocol.
**
** Returns:         True if tag is Unknown protocol.
**
*******************************************************************************/
bool NfcTag::isProtocolUnknown() {
  static const char fn[] = "NfcTag::isProtocolUnknown";
  bool isUnknown = false;

  if (NFC_DISCOVERY_TYPE_POLL_B == mTechParams[0].mode ||
      NFC_DISCOVERY_TYPE_POLL_B_PRIME == mTechParams[0].mode ||
      NFC_DISCOVERY_TYPE_LISTEN_B == mTechParams[0].mode ||
      NFC_DISCOVERY_TYPE_LISTEN_B_PRIME == mTechParams[0].mode) {
    // PUPI should zero
    if (!mTechParams[0].param.pb.sensb_res[0] && !mTechParams[0].param.pb.sensb_res[1] &&
        !mTechParams[0].param.pb.sensb_res[2] && !mTechParams[0].param.pb.sensb_res[3]) {
      // PICC not compliant with 14443-4
      if ((0x0F & mTechParams[0].param.pb.sensb_res[9]) == 0) {
        isUnknown = true;
      }
    }
  }

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: protocol unknown=%d", fn, isUnknown);
  return isUnknown;
}
/* END [NFC3128] Patch for NFC3128 (China ID) */
...
/* START [NSQE-329] passive P2P Issue */
  if(mNumTechList == 0) {
    for (int i = 0; i < mNumDiscTechList; i++) {
      if (mTechLibNfcTypesDiscData[i] == NFA_PROTOCOL_NFC_DEP) {
...
/* END [NSQE-329] passive P2P Issue */
...
/* START [NFC-3098] */
bool NfcTag::isChinaTransitTag() {
  static const char fn[] = "NfcTag::isChinaTransitTag";
  bool retval = false;

  for (int i = 0; i < mNumTechList; i++) {
    if (mTechParams[i].mode == NFC_DISCOVERY_TYPE_POLL_A) {
      /* China Transit Card has one sak byte and 2 ATQA bytes */
      if ((!mIsMultiProtocolTag) &&
          (mTechParams[i].param.pa.sens_res[0] == 0x08) &&
          (mTechParams[i].param.pa.sens_res[1] == 0x00) &&
          (mTechParams[i].param.pa.sel_rsp == 0x20)) {
        retval = true;
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s:  China Transit Card=%u", fn, retval);
      }
      break;
    }
  }
...
/* START [J16111601] - eID card of Germany */
  mTechnologyTimeoutsTable [TARGET_TYPE_ISO14443_4] = 2000; //ISO-DEP
/* END [J16111601] - eID card of Germany */
...
/* START [17092801J] Add Tech_Type for WCE  */
#if (NFC_SEC_NOT_OPEN_INCLUDED == TRUE)
    mTechnologyTimeoutsTable [TARGET_TYPE_SNFC_RFU_01] = 618;
    mTechnologyTimeoutsTable [TARGET_TYPE_SNFC_RFU_02] = 618;
    mTechnologyTimeoutsTable [TARGET_TYPE_SNFC_RFU_03] = 618;
    mTechnologyTimeoutsTable [TARGET_TYPE_SNFC_RFU_04] = 618;

    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("resetAllTransceiveTimeouts()");
#endif
/* END [17092801J] Add Tech_Type for WCE    */
...
void NfcTag::resetTagTechnologies() {
    resetTechnologies();
}
...
bool NfcTag::getMultiProtocolTagSupport(void) {
