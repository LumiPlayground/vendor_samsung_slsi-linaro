...
/* START [2020021401] - Abnormal T4T Read Patch for Beijing Card */
// RETRY_LIMIT_FOR_RESELECT must be odd number (ex. 1, 3, 5, 7)
// RETRY_LIMIT_TIMEOUT_FOR_RESELECT is margin time about RF_ACTIVATED_NTF
#define RETRY_LIMIT_FOR_RESELECT            3
#define RETRY_LIMIT_TIMEOUT_FOR_RESELECT    300
/* END [2020021401] - Abnormal T4T Read Patch for Beijing Card */
...
/* START [J18060801] - Recovery Mifare CheckNdef Timeout */
void nativeNfcTag_doCheckNdefResult (tNFA_STATUS status, uint32_t maxSize, uint32_t currentSize, uint8_t flags);
/* END [J18060801] - Recovery Mifare CheckNdef Timeout */

/* START [2020021401] - Abnormal T4T Read Patch for Beijing Card */
static bool          gIsSkipRfSwitching = false;
/* END [2020021401] - Abnormal T4T Read Patch for Beijing Card */

/* START [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
static bool          gIsTagDeSelected = false;
/* END [2020042101] - Patch for T4T tag  presence checking after tag was deselected (0xC2) */

/* START [NFC2973] - Patch for normal T4T tag and China ID */
static bool          gIsTimeoutedError = false;
/* END [NFC2973] - Patch for normal T4T tag and China ID */
...
/* START [2020042203] - Additional patch for retry logic in reSelect() : old t-money */
  if(!sConnectWaitingForComplete)
  {
    ...
    LOG(ERROR) << StringPrintf(
                        "%s: Skip setting sCurrentTagInfos (sWaiting : 0x%x)",
                        __func__, sConnectWaitingForComplete);
/* END [2020042203] - Additional patch for retry logic in reSelect() : old t-money */
}
...
/* START [17121201J] Conflict prevention between Tag Reading and ApplyRouting */
bool nativeNfcTag_isWaitingForComplete ()
{
    bool status = false;
    ...
    if(status)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                    "%s: status=0x%x; sConnect=%u, sCheckNdef=%u, sMakeRead=%u"
                    , __func__, status, sConnectWaitingForComplete,
                    sCheckNdefWaitingForComplete,
                    sMakeReadonlyWaitingForComplete);
    }
    return status;
}
/* END [17121201J] Conflict prevention between Tag Reading and ApplyRouting */
/*******************************************************************************
**
...
void nativeNfcTag_setActivatedRfProtocol(tNFA_INTF_TYPE rfProtocol) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s:sCurrentActivatedProtocl = 0x%02X",
               __func__, sCurrentActivatedProtocl);
}
...
/* START [NFC2608] - Patch for NFC2608 */
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                "%s: sConnectWaitingForComplete = 0x%x",
                                __func__, sConnectWaitingForComplete);
...
static jint nativeNfcTag_doConnect(JNIEnv*, jobject, jint targetHandle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: sCurrentConnectedTargetType: 0x%02X, sCurrentConnectedTargetProtocol: 0x%02X ",
          __func__,sCurrentConnectedTargetType, sCurrentConnectedTargetProtocol);

/* START [2020021401] - Abnormal T4T Read Patch for Beijing Card */
  if (sCurrentConnectedTargetProtocol != NFC_PROTOCOL_ISO_DEP)
    gIsSkipRfSwitching = false;
/* END [2020021401] - Abnormal T4T Read Patch for Beijing Card */
...
/* START [NFC3128] Patch for NFC3128 (China ID) */
  if (NfcTag::getInstance().isProtocolUnknown()) {
    LOG(ERROR) << StringPrintf("%s: reselect frame for china id card", __func__);
    retCode = switchRfInterface(NFA_INTERFACE_FRAME) ? NFA_STATUS_OK
                                                     : NFA_STATUS_FAILED;
    goto TheEnd;
  }
/* END [NFC3128] Patch for NFC3128 (China ID) */
...
/*******************************************************************************
**  Subfunctions for reSelect()

bool sendPreprocessingT4TPacketForReselect()
bool sendPreprocessingMifarePacketForReselect()
bool sendDeactivateCmdForReselect()
bool reSendDeactivateCmdForErrorProcessing()
int checkTagState()
void retryLogicForSelecingError()

*******************************************************************************/
bool sendPreprocessingT2TnT4TPacketForReselect() {
  tNFA_STATUS status = NFA_STATUS_OK;
    ...
    /* START [20012901J] - Adding a multi-tag processing for considering RATS(e0 80) */
    else if ((sCurrentActivatedProtocl == NFA_PROTOCOL_ISO_DEP) ||
             (sCurrentActivatedProtocl == NFA_PROTOCOL_MIFARE)) {
        status = NFA_SendRawFrame(RW_DESELECT_REQ, sizeof(RW_DESELECT_REQ), 0);
    }
    /* END [20012901J] - Adding a multi-tag processing for considering RATS(e0 80) */

    sReconnectEvent.wait(8);
        if (status != NFA_STATUS_OK) {
...
bool sendPreprocessingMifarePacketForReselect() {
    tNFA_STATUS status = NFA_STATUS_OK;
      uint8_t RW_TAG_RATS_REQ[] = {0xE0, 0x80};

    if (sCurrentActivatedProtocl == NFA_PROTOCOL_ISO_DEP)
    {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: Send a E0 (RATS) for switching (M1 => ISO)", __func__);
        {
          SyncEventGuard g3(sReconnectEvent);
          status = EXTNS_MfcTransceive(RW_TAG_RATS_REQ, sizeof(RW_TAG_RATS_REQ));
          sReconnectEvent.wait(8);
          if (status != NFA_STATUS_OK) {
            LOG(ERROR) << StringPrintf("%s: send error=%d", __func__, status);
            return false;
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: Send a C2 (DESELECT) for switching (M1 => ISO)", __func__);
        {
          SyncEventGuard g3(sReconnectEvent);
          status = EXTNS_MfcTransceive(RW_DESELECT_REQ, sizeof(RW_DESELECT_REQ));
          sReconnectEvent.wait(8);
          if (status != NFA_STATUS_OK)
          {
            LOG(ERROR) << StringPrintf("%s: send error=%d", __func__, status);
...
bool sendDeactivateSleepCmdForReselect() {
    tNFA_STATUS status = NFA_STATUS_OK;

      SyncEventGuard g(sReconnectEvent);
    LOG(ERROR) << StringPrintf("%s: deactivate to sleep", __func__);
...
bool reSendDeactivateCmdForErrorProcessing() {
    tNFA_STATUS status = NFA_STATUS_OK;
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Check sGotDeactivate = 0x%x", __func__, sGotDeactivate);

    /*Retry logic in case of RF_DEACTIVATE_NTF(sleep, 04) error while selecting a tag*/
    if (sGotDeactivate == false) {
        LOG(ERROR) << StringPrintf("%s: waiting for Card to be activated",__func__);
        int retry = 0;
        sConnectWaitingForComplete = JNI_TRUE;
        do {
            SyncEventGuard g(sReconnectEvent);
            LOG(ERROR) << StringPrintf("%s: deactivate to sleep (retry = %d)", __func__, retry);
            // deactivate to sleep state
            ...
            if (sReconnectEvent.wait(RETRY_LIMIT_TIMEOUT_FOR_RESELECT) == false)  // if timeout occurred
                LOG(ERROR) << StringPrintf("%s: timeout waiting for deactivate",__func__);

            retry++;
        } while (sGotDeactivate == false && retry < RETRY_LIMIT_FOR_RESELECT);

        if((retry > (RETRY_LIMIT_FOR_RESELECT -1)) && sGotDeactivate == false)
        {
            LOG(ERROR) << StringPrintf("%s: retry complete (retry = %d)", __func__, retry);
            gIsSkipRfSwitching = true;
            sGotDeactivate = true;
      }
    }
    return true;
...
void retryLogicForSelecingError() {
    ...
    if (sConnectOk == false) {
        ...
        do {
            ...
            if (sReconnectEvent.wait(RETRY_LIMIT_TIMEOUT_FOR_RESELECT) == false) {  // if timeout occured
        ...
        } while (sConnectOk == false && retry < (RETRY_LIMIT_FOR_RESELECT-1));
    }
}

bool sendRfSelectCmd(uint8_t rf_disc_id, tNFA_NFC_PROTOCOL protocol,
                    tNFA_INTF_TYPE rf_interface)
{
...

    {
      ...
      sConnectWaitingForComplete = JNI_TRUE;
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: select interface = %u, gIsSelectingRfInterface %u",
            __func__, rf_interface, gIsSelectingRfInterface);
        if (NFA_STATUS_OK !=
            (status = NFA_Select(rf_disc_id, protocol, rf_interface)) )
        {
    ...
    /*Retry logic in case of core Generic error while selecting a tag*/
    if ( (sConnectOk == false) && (protocol == NFC_PROTOCOL_ISO_DEP) )
    {
        LOG(ERROR) << StringPrintf("%s: waiting for Card to be activated", __func__);
        int retry = 0;
/* START [NFC2973] - Patch for normal T4T tag and China ID */
        gIsTimeoutedError = false;
/* END [NFC2973] - Patch for normal T4T tag and China ID */
        sConnectWaitingForComplete = JNI_TRUE;
        do {
            SyncEventGuard reselectEvent(sReconnectEvent);
...
/* START [NFC2973] - Patch for normal T4T tag and China ID */
        if(retry > (RETRY_LIMIT_FOR_RESELECT-1))
        {
            gIsTimeoutedError = true;
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: This T4T Tag with RF interface error", __func__);
        }
/* END [NFC2973] - Patch for normal T4T tag and China ID */
    }
...
static int reSelect(tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded) {
  ...
  do {
    ...
    if (NfcTag::getInstance().isNdefDetectionTimedOut()) {
      ...
      /* START [20022501] - Patch for no response while checking ndef format (GTO/Kookmin) */
      if (sCurrentConnectedTargetProtocol != NFC_PROTOCOL_ISO_DEP)
      /* END [20022501] - Patch for no response while checking ndef format (GTO/Kookmin) */
        break;
    }

    /* Send Preproessing Data Packet for RF Switching */
    if ((sCurrentRfInterface == NFA_INTERFACE_FRAME) &&
        (NFC_GetNCIVersion() >= NCI_VERSION_2_0)) {
/* START [NFC3128] Patch for NFC3128 (China ID) */
          if(!NfcTag::getInstance().isProtocolUnknown()) {
            if(!sendPreprocessingT2TnT4TPacketForReselect())
                break;
          }
/* END [NFC3128] Patch for NFC3128 (China ID) */
        }
        /* START [20013101J] - Adding a multi-tag processing for switching Mifare to ISO-DEP */
        else if ((sCurrentRfInterface == NFC_INTERFACE_MIFARE) &&
            (NFC_GetNCIVersion() >= NCI_VERSION_2_0)) {
          if(!sendPreprocessingMifarePacketForReselect())
              break;
        }
        /* END [20013101J] - Adding a multi-tag processing for switching Mifare to ISO-DEP */

        gIsTagDeactivating = true;
        sGotDeactivate = false;
        /* START [2020042102] Patch for RF_DEACTIVATE_NTF(04) error tag as like beijing and seven eleven card */
        gIsSelectingRfInterface = true;
        /* END [2020042102] Patch for RF_DEACTIVATE_NTF(04) error tag as like beijing and seven eleven card */
        /* START [17121201J] Conflict prevention between Tag Reading and ApplyRouting */
          sConnectWaitingForComplete = JNI_TRUE;
        /* END [17121201J] Conflict prevention between Tag Reading and ApplyRouting */

        /* Send RF_DEACTIVATE_CMD(sleep) for RF Switching */
        if(!sendDeactivateSleepCmdForReselect())
            break;
        ...
        /* Retry logic in case of RF_DEACTIVATE_NTF(sleep, 04) error while selecting a tag */
        bool isSingleTag = true;
        isSingleTag = natTag.getMultiProtocolTagSupport();
        if(!reSendDeactivateCmdForErrorProcessing())
            break;

/* START [NSQE313] - Add a option to check discovery state of connected tag */
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: isSingleTag = %d, getMultiProtocolTagSupport = %d", __func__,
                isSingleTag, natTag.getMultiProtocolTagSupport());
        if(isSingleTag != natTag.getMultiProtocolTagSupport())
            sGotDeactivate = false;
/* END [NSQE313] - Add a option to check discovery state of connected tag */
        /* END [2020021401] - Abnormal T4T Read Patch for Beijing Card */

        if (!sGotDeactivate) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tag is lost", __func__);
            rVal = STATUS_CODE_TARGET_LOST;
            break;
        }

        /* Check connected tag state */
        ...
        else if (NfcTag::getInstance().getActivationState() != NfcTag::Sleep) {
        ...
        /* Send RF selection command */
        if( !sendRfSelectCmd(natTag.mTechHandles[sCurrentConnectedHandle],
                            natTag.mTechLibNfcTypes[sCurrentConnectedHandle],
                            rfInterface) )
        {
            LOG(ERROR) << StringPrintf("%s: Rf selection is failed", __func__);
            rVal = STATUS_CODE_TARGET_LOST;
            break;
        }

        /* Check rf switched tag state */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
...
/* END [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
  ...
  sRfInterfaceMutex.unlock();
  DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: exit; status=%d, gIsSelectingRfInterface=%u",
        __func__, rVal, gIsSelectingRfInterface);
  return rVal;
}
...
/* START [2020021401] - Abnormal T4T Read Patch for Beijing Card */
  {
    if (natTag.isMifareDESFire ()) // Add to prevent checkndef timeout
      retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
    /* START [NFC-3098] */
    else if (natTag.isChinaTransitTag ())
      retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
    /* END [NFC-3098] */
/* START [NFC3128] Patch for NFC3128 (China ID) */
    else if (sCurrentRfInterface == NFA_INTERFACE_FRAME && natTag.isProtocolUnknown())
    {
      LOG(ERROR) << StringPrintf("%s: China ID card detected", __func__);
    }
/* END [NFC3128] Patch for NFC3128 (China ID) */
    else if(!gIsSkipRfSwitching)
...
/* START [NFC2608] - Patch for NFC2608 */
  {
    ...
    if(retCode == 0x01)     // Tag Absent
    {
        LOG(ERROR) << StringPrintf("%s: tag already deactivated2", __func__);
        retCode = NFCSTATUS_FAILED;
    }
  }
/* END [NFC2608] - Patch for NFC2608 */
...
/* START [NSQE313] - Add a option to check discovery state of connected tag */
  if ( (NfcTag::getInstance().getActivationState() != NfcTag::Active) &&
    (NfcTag::getInstance().getActivationState() != NfcTag::Discovery) ) {
/* END [NSQE313] - Add a option to check discovery state of connected tag */
    LOG(WARNING) << StringPrintf("%s: tag already deactivated", __func__);
/* START [2020052101] - Call resetTechnolgies() to reset tag */
    if(NfcTag::getInstance().getMultiProtocolTagSupport())
        NfcTag::getInstance().resetTagTechnologies();
/* START [2020052101] - Call resetTechnolgies() to reset tag */
...
void nativeNfcTag_doTransceiveStatus(tNFA_STATUS status, uint8_t* buf,
  ...
  if (sCurrentConnectedTargetProtocol == NFC_PROTOCOL_MIFARE && legacy_mfc_reader) {
/* START [J18060801] - Recovery Mifare CheckNdef Timeout */
        if((sCheckNdefWaitingForComplete == JNI_TRUE) && (status == NFA_STATUS_TIMEOUT))
        {
           nativeNfcTag_doCheckNdefResult(NFA_STATUS_TIMEOUT, 0, 0, RW_NDEF_FL_UNKNOWN);
           return;
        }
/* END [J18060801] - Recovery Mifare CheckNdef Timeout */
...
/* START [20021301] - Patch for RATS and DESELECT in the ISO RF Interface mode */
/* START [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
  uint8_t RW_TAG_RATS_REQ[] = {0xE0, 0x80};
  uint8_t RW_TAG_DESELECT_REQ[] = {0xC2};
  gIsTagDeSelected = false;

  if( ((bufLen == 0x02) &&
    (buf[0] == RW_TAG_RATS_REQ[0]) &&
    (buf[1] == RW_TAG_RATS_REQ[1])) ||
    ((bufLen == 0x01) &&
    (buf[0] == RW_TAG_DESELECT_REQ[0])) )
  {
    if(sCurrentRfInterface == NFA_INTERFACE_ISO_DEP)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                "%s: Change Interface from ISO_DEP to Frame because of RATS",
                __func__);

        sCurrentRfInterface = NFA_INTERFACE_FRAME;
        status = reSelect(NFA_INTERFACE_FRAME, false);
        if(status != 0x00)
            LOG(ERROR) << StringPrintf("%s: reSelecting failed", __func__);
    }
    else if( (sCurrentRfInterface == NFA_INTERFACE_FRAME) &&
        (bufLen == 0x01) &&
        (buf[0] == RW_TAG_DESELECT_REQ[0]) )
    {
        gIsTagDeSelected = true;
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                "%s: [dbg] T4T Tag is deselected", __func__);
    }
  }
/* END [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
/* END [20021301] - Patch for RATS and DESELECT in the ISO RF Interface mode */
...
/* START [2020021701] - Abnormal T4T Read Patch for ezlink card */
/* ezlink card occur RF_DEACTIVATE_NTF(sleep, 4 error) after sending RF_ACTIVATE_NTF */
    // if timeout occurred
    if ( (waitOk == false || sTransceiveRfTimeout) &&
        (sCurrentConnectedTargetProtocol == NFC_PROTOCOL_ISO_DEP) &&
        (natTag.getMultiProtocolTagSupport() == false) )
    {
        LOG(ERROR) << StringPrintf(
            "%s: Retry reSelect() for resolving doTransceive timeout",
            __func__);
/* START [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
        gIsTagDeSelected = false;
/* END [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */

        tNFA_STATUS retCode = NFA_STATUS_OK;
        if(sCurrentRfInterface == NFA_INTERFACE_FRAME)
            retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
        else
            retCode = reSelect(NFA_INTERFACE_FRAME, false);
...
/* START [NSQE313] - Add a option to check discovery state of connected tag */
        if(retCode != STATUS_CODE_TARGET_LOST)     // Tag Absent
        {
            sTransceiveRfTimeout = false;
            waitOk = true;
        }
        ...
        {
            sIsTagPresent = false;
            LOG(ERROR) << StringPrintf("%s: Tag Absent", __func__);
        }
/* END [NSQE313] - Add a option to check discovery state of connected tag */
    }
/* END [2020021701] - Abnormal T4T Read Patch for ezlink card */
...
/* START [2020042701] - Patch for NETS FlashPay card when it is occured timeout after sending 2b data */
    else if (sRxDataBuffer.size() == 0x00) {
        LOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: try reconnect..#2", __func__);
        status = nativeNfcTag_doReconnect(NULL, NULL);
        ...
        if(targetLost) {
          if(status != NFCSTATUS_FAILED)
            ...
            *targetLost = 1;  // success, tag is absent
        }

        LOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: reconnect finish..#2", __func__);
    }
/* END [2020042701] - Patch for NETS FlashPay card when it is occured timeout after sending 2b data */
  } while (0);
...
/* START [17083101J] - Smart OTP Test Patch */
    LOG(ERROR) << StringPrintf(
                    "%s: Check Ndef Failed (flags =0x%X) , (status =0x%X)",
                                                    __func__, flags, status);
    if ( status == NFC_STATUS_WRONG_SIZE_BAD_RESP )
    {
      //NDEF content is on the tag, but NDEF size is wrong
      sCheckNdefMaxSize = maxSize;
      sCheckNdefCurrentSize = maxSize;
      sCheckNdefCardReadOnly = flags & RW_NDEF_FL_READ_ONLY;
      sCheckNdefCapable = true;
      sCheckNdefStatus = NFA_STATUS_OK;
      LOG(ERROR) << StringPrintf("%s: Size Error Tag ...!! (status =0x%X)",
                                                            __func__, status);
    }
    else
    {
    // no NDEF content on the tag
      LOG(ERROR) << StringPrintf(
            "%s: no NDEF content on the tag (status =0x%X)", __func__, status);
      sCheckNdefMaxSize = 0;
...
/* END [17083101J] - Smart OTP Test Patch */
  } else {
...
/* START [J18100201] - Prevent conflict doCheckNdef and doPresenceCheck */
  sCheckNdefWaitingForComplete = JNI_TRUE;
/* END [J18100201] - Prevent conflict doCheckNdef and doPresenceCheck */
...
/* ezlink card occur RF_DEACTIVATE_NTF(sleep, 4 error) after sending RF_ACTIVATE_NTF */
    // if timeout occurred
    NfcTag& natTag = NfcTag::getInstance();
    if ( (status == NCI_STATUS_TIMEOUT) &&
        (sCurrentConnectedTargetProtocol == NFC_PROTOCOL_ISO_DEP) &&
        (natTag.getMultiProtocolTagSupport() == false) )
    {
        LOG(ERROR) << StringPrintf(
           "%s: Retry reSelect() for resolving doCheckNdef timeout", __func__);
        tNFA_STATUS retCode = NFA_STATUS_OK;
        if(sCurrentRfInterface == NFA_INTERFACE_FRAME)
            retCode = reSelect(NFA_INTERFACE_ISO_DEP, false);
        else
        ...
        if(retCode != NFA_STATUS_REJECTED)     // Tag Absent
        {
            status = NFA_STATUS_FAILED;
            sCheckNdefStatus = status;
        }
...
static jboolean nativeNfcTag_doPresenceCheck(JNIEnv*, jobject) {
  ...
  if (sCurrentConnectedTargetProtocol == NFC_PROTOCOL_MIFARE && legacy_mfc_reader) {
/* START [17112401J] - Patch for Multi-Protocol Tag Reading (Extra_Part7) */
//  Patch for Presence Checking Error of Mifare Classic Tag
    status = reSelect (NFA_INTERFACE_MIFARE, false);
    if(status == NFCSTATUS_SUCCESS)
      isPresent = JNI_TRUE;
    else {
      isPresent = JNI_FALSE;
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tag absent",
                                                                __func__);
/* START [2020052101] - Call resetTechnolgies() to reset tag */
      NfcTag::getInstance().connectionEventHandler(NFA_DEACTIVATED_EVT, NULL);
/* END [2020052101] - Call resetTechnolgies() to reset tag */
    }
    return isPresent;
/* END [17112401J] - Patch for Multi-Protocol Tag Reading (Extra_Part7) */
  }
/* START [18072401J] - Patch for checking presence confition*/
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                        "%s: Proto=0x%x, Rf_Intf=0x%x, Algo=0x%x", __func__,
                        sCurrentConnectedTargetProtocol, sCurrentRfInterface,
                        NfcTag::getInstance().getPresenceCheckAlgorithm());
/* END [18072401J] - Patch for checking presence confition*/
...
// [11. doPresenceCheck changes]
  if(sCurrentConnectedTargetProtocol == NFA_PROTOCOL_ISO_DEP)
  {
/* START [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: gIsSkipRfSwitching = 0x%x, gIsTagDeSelected = 0x%x",
            __func__, gIsSkipRfSwitching, gIsTagDeSelected);
/* START [NFC2973] - Patch for normal T4T tag and China ID */
    if( (gIsTagDeSelected) || (gIsTimeoutedError) )
/* END [NFC2973] - Patch for normal T4T tag and China ID */
/* END [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
    {
/* START [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
/* START [NFC2973] - Patch for normal T4T tag and China ID */
      status = reSelect(sCurrentRfInterface, false);
/* END [NFC2973] - Patch for normal T4T tag and China ID */
/* END [2020042101] - Patch for T4T tag presence checking after tag was deselected (0xC2) */
      if(status == NFCSTATUS_SUCCESS)
        isPresent = JNI_TRUE;
