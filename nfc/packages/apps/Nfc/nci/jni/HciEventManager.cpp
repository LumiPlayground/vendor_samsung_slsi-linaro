...
// [SEC] Handle dynamic pipe id
tNFA_HANDLE mNfaHciHandle = 0;
const uint8_t HCI_HOST_TYPE_UICC = 0x02;
const uint8_t HCI_HOST_TYPE_ESE = 0x03;
static SyncEvent sNfaHciGetGateAndPipeEvent;
...
void HciEventManager::initialize(nfc_jni_native_data* native) {
  mNativeData = native;
  SyncEventGuard guard(sNfaHciGetGateAndPipeEvent);
  ...
  // [SEC] Update pipe id dynamically
  if (nfaStat == NFA_STATUS_OK) {
    sNfaHciGetGateAndPipeEvent.wait();
    if (mNfaHciHandle != 0) {
      nfaStat = NFA_HciGetGateAndPipeList(mNfaHciHandle);
      if (nfaStat != NFA_STATUS_OK) {
        LOG(ERROR) << "HCI get gate and pipe list failed; status=" << nfaStat;
      }
    }
  }
}
...
void HciEventManager::nfaHciCallback(tNFA_HCI_EVT event,
  ...
  switch (event) {
    case NFA_HCI_REGISTER_EVT:
    {
      ...
      SyncEventGuard guard(sNfaHciGetGateAndPipeEvent);
      sNfaHciGetGateAndPipeEvent.notifyOne();
    } break;
    case NFA_HCI_GET_GATE_PIPE_LIST_EVT:
    {
      if (eventData->gates_pipes.status == NFA_STATUS_OK) {
        for (uint8_t xx = 0; xx < eventData->gates_pipes.num_pipes; xx++) {
          if (eventData->gates_pipes.pipe[xx].dest_gate == NFA_HCI_CONNECTIVITY_GATE) {
            if (eventData->gates_pipes.pipe[xx].dest_host == HCI_HOST_TYPE_ESE) {
              sEsePipe = eventData->gates_pipes.pipe[xx].pipe_id;
              DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                         "update ese pipe id 0x%x", sEsePipe);
            } else if (eventData->gates_pipes.pipe[xx].dest_host == HCI_HOST_TYPE_UICC) {
              sSimPipe = eventData->gates_pipes.pipe[xx].pipe_id;
              DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                         "update sim pipe id 0x%x", sSimPipe);
            } else {
              LOG(ERROR) << "Undefined destination host";
            }
          }
        }
      }
    ...
    case NFA_HCI_CREATE_PIPE_EVT:
    {
      if (eventData->created.status == NFA_STATUS_OK) {
        if (eventData->created.dest_gate == NFA_HCI_CONNECTIVITY_GATE) {
          if (eventData->created.dest_host == HCI_HOST_TYPE_ESE) {
            sEsePipe = eventData->created.pipe;
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                       "update ese pipe id 0x%x", sEsePipe);
          } else if(eventData->created.dest_host == HCI_HOST_TYPE_UICC) {
            sSimPipe = eventData->created.pipe;
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
                                        "update sim pipe id 0x%x", sSimPipe);
          }
        }
