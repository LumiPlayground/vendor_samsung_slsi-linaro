...
class RoutingManager {
 public:
  ...
  int getDefaultEe(); // [SEC] Get default EE
  bool setDefaultEe(int defaultEe); // [SEC] Set default EE
  bool isUiccDetected(); // [SEC] Check UICC exists
  bool activateSecureElement(); // [SEC] eSE activation
  jintArray getSecureElementList(JNIEnv* e); // [SEC] get SE list
  int getAidMatchingMode(); // [SEC] Get AID matching mode
  int getListenTechMask(); // [SEC] Get listen tech mask
  void setListenTechMask(int techMask); // [SEC] Set listen tech mask
  void abort(); // [SEC] abort waits
  void clearRoute();
 ...
 private:
  ...
  int getActiveSe(int defaultEe); // [SEC] Get active SE
  bool isEeSuportTech(uint8_t eeHandle, uint8_t tech);
  void notifyEeActionProtocol (uint16_t ee_handle, uint8_t protocol, std::string strBuffer);
  uint8_t getEeStatus(uint8_t eeHandle); // [SEC] Check EE activation
  ...
  bool mReceivedeSEEeInfo; // [SEC] for eSE
  ...
  SyncEvent meSeEeInfoEvent; // [SEC] for eSE
