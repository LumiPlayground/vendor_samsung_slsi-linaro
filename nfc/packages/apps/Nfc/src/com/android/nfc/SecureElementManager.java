...
public class SecureElementManager {
    ...
    public void init() {
        ...
        Log.d(TAG, "SecureElementManager :: " + SEID_SIM);
    }
    ...
    private class DebounceTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            mNfcService.setWaitForRestarting(false);
        }
    }
    ...
    private final ContentObserver mUserValueObserver = new ContentObserver(null) {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            boolean preferredSimChange = false;
            ...
            Log.d(TAG, "onChange; defaultSe: " + defaultSe + ", currentSe" + currentSe);
            if (currentSe > SEID_ESE && defaultSe > SEID_ESE) {
                preferredSimChange = true;
            }

            if (currentSe == SEID_SIM) {
            ...
            if (preferredSimChange) {
                mNfcService.setDefaultRoute(SEID_SIM, true);
                mIsSlotSwitched = true; // Do not switch slot because it's intended behavior
                setPreferredSimSlot(defaultSe - SEID_SIM);
                mNfcService.restartNfcService();
            } else {
