...
import com.samsung.nfc.ISecNfcAdapter;
...
public class NfcService implements DeviceHostListener {
    ...
    // [SEC] Preferences
    static final String SEC_PREF_DEFAULT_ROUTE = "default_route";
    static final String SEC_PREF_PREFERRED_SIM = "preferred_sim";
    static final String SEC_PREF_POWEROFF_CARDEMULATION = "poweroff_ce";
    static final String SEC_PREF_LISTEN_TECH_MASK = "listen_mask";
    static final String SEC_PREF_NFC_FACTORY_MODE = "factory_mode";
    static final int SEC_PREF_UNDEFINED = -1;
    static final int SEC_PREF_SIM_SLOT1 = 0;
    static final int SEC_PREF_SIM_SLOT2 = 1;
    // [SEC] SecureElement ID
    static final int SEID_HOST = 0;
    static final int SEID_ESE = 2;
    static final int SEID_SIM = 3;
    // [SEC] Listening technology masks
    static final int NFC_LISTEN_DISABLE = 0x00;
    static final int NFC_LISTEN_A = 0x01;
    static final int NFC_LISTEN_B = 0x02;
    static final int NFC_LISTEN_F = 0x04;
    // [SEC] Factory mode
    static final byte FACTORY_MODE_DISABLE = 0x00;
    static final byte FACTORY_MODE_ENABLE = 0x01;
    static final byte FACTORY_MODE_USER_CAL = 0x02;
    static final byte FACTORY_MODE_CAL_DONE = 0x03;
    static final byte FACTORY_MODE_GET_CAL = 0x04;
    ...
    static final int TASK_RESTART = 6;
    ...
    private SecureElementManager mSecureElementManager; // [SEC]
    private boolean mIsWaitForRestart = false; // [SEC]
    private boolean mRestartingNfc = false;

    SecNfcAdapterService mSecNfcAdapter;
    ...
    public void onSeListenActivated() {
        Log.e(TAG, "se listen activated");
        //TODO: customer modify
    }

    @Override
    public void onSeListenDeactivated() {
        Log.e(TAG, "se listen deactivated");
        //TODO: customer modify
    }

    @Override
    public void onEeActionProtocolEvent(int handle, int protocol, String evtBuffer) {
        int[] eeActInfo = {handle, protocol};
        Log.d(TAG, "ee action ntf.."+evtBuffer);
        //TODO: customer modify
    }
    ...
    public NfcService(Application nfcApplication) {
        ...
        mSecNfcAdapter = new SecNfcAdapterService();
        ...
        mSecureElementManager = new SecureElementManager(mContext); // [SEC]
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        protected Void doInBackground(Integer... params) {
            ...
            switch (params[0].intValue()) {
                case TASK_ENABLE:
                    enableInternal();
                    // [SEC] Check SIM switch
                    if (isRestartToSwitchSim()) {
                        Log.d(TAG, "NFC restart");
                ...
                case TASK_DISABLE_ALWAYS_ON:
                    ...
                    break;
                case TASK_RESTART: // [SEC] NFC restart task
                    Log.d(TAG, "NFC restart task start");
                    mRestartingNfc = true;
                    disableInternal();
                    enableInternal();
                    mRestartingNfc = false;
                    Log.d(TAG, "NFC restart task end");
                    break;
            }
        ...
        boolean enableInternal() {
            ...
            try {
                ...
                try {
                    // [SEC] Get and set preferred SIM slot
                    int preferredSlotId = getPreferredSimSlot();
                    Log.d(TAG, "Preferred SIM slot " + preferredSlotId);
                    mDeviceHost.setPreferredSimSlot(preferredSlotId);

                    // [SEC] Get default route from preference. Get from config if not set
                    int defaultRoute = mPrefs.getInt(SEC_PREF_DEFAULT_ROUTE, SEC_PREF_UNDEFINED);
                    if (defaultRoute > SEC_PREF_UNDEFINED) {
                        mDeviceHost.setDefaultRoute(defaultRoute, false);
                    }
...
//        @Override
        public IBinder getNfcAdapterVendorInterface(String vendor) {
            if(vendor.equalsIgnoreCase("sec")) {
                return (IBinder) mSecNfcAdapter;
            }
            return null;
        }
    }

    final class SecNfcAdapterService extends ISecNfcAdapter.Stub {
        @Override
        public boolean setRfOptionUpdate(String filePath) {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.setRfOptionFilePath(filePath);
        }

        @Override
        public boolean setDefaultRoute(int route) {
            NfcPermissions.enforceAdminPermissions(mContext);
            boolean isRestartRfDiscovery = true;
            return NfcService.this.setDefaultRoute(route, isRestartRfDiscovery);
        }

        @Override
        public int getDefaultRoute() {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.getDefaultRoute();
        }

        @Override
        public boolean setListenTechMask(int techMask) {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.setListenTechMask(techMask);
        }

        @Override
        public int getListenTechMask() {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.getListenTechMask();
        }

        @Override
        public boolean setFWDebugLevel(int type, boolean enable) {
            NfcPermissions.enforceAdminPermissions(mContext);
            NfcService.this.setFirmwareDebugLevel(type, enable);
            return true;
        }

        @Override
        public void doFactoryRfCalibration() {
            NfcPermissions.enforceAdminPermissions(mContext);
            NfcService.this.doFactoryRf();
        }

        @Override
        public String getFactoryRfCalValue() {
            NfcPermissions.enforceAdminPermissions(mContext);
            return new String(NfcService.this.getFactoryRf());
        }

        @Override
        public void setPowerOffCardEmulation(boolean enable) {
            NfcPermissions.enforceAdminPermissions(mContext);
            NfcService.this.setPowerOffCardEmulation(enable);
        }

        @Override
        public int[] getSecureElementList() {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.getSecureElementList();
        }

        @Override
        public void setPreferredSimSlot(int slotId) {
            NfcPermissions.enforceAdminPermissions(mContext);
            NfcService.this.setPreferredSimSlot(slotId);
        }

        @Override
        public int getPreferredSimSlot() {
            NfcPermissions.enforceAdminPermissions(mContext);
            return NfcService.this.getPreferredSimSlot();
        }
    }
...
//            return false;     // [sec_tos] temporary disable for TagWriter App
            return true;
        }
    }
    ...
    // [SEC] Get default route destination
    public int getDefaultRoute() {
        int defaultRoute;
        defaultRoute = mPrefs.getInt(SEC_PREF_DEFAULT_ROUTE, SEC_PREF_UNDEFINED);
        if (defaultRoute == SEC_PREF_UNDEFINED) {
            defaultRoute = mDeviceHost.getDefaultRoute();
        }
        return defaultRoute;
    }

    // [SEC] Set default route destination
    public boolean setDefaultRoute(int defaultRoute, boolean restart) {
        Log.d(TAG, "setDefaultRoute route: " + defaultRoute);
        if (defaultRoute < SEID_HOST || defaultRoute > SEID_SIM) {
            Log.e(TAG, "setDefaultRoute invalid route: " + defaultRoute);
            return false;
        }
        if (isNfcEnabled()) {
            synchronized (NfcService.this) {
                mDeviceHost.setDefaultRoute(defaultRoute, restart);
            }
        } else {
            Log.e(TAG, "setDefaultRoute() called while NFC is not enabled.");
            return true;
        }
        if (restart) {
            mPrefsEditor.putInt(SEC_PREF_DEFAULT_ROUTE, defaultRoute);
            mPrefsEditor.apply();
        }
        if (!isNfcEnabled()) {
            Log.e(TAG, "NFC needs to be enabled to do setDefaultRoute");
            return false;
        }
        synchronized (NfcService.this) {
            mDeviceHost.setDefaultRoute(defaultRoute, restart);
    ...
    // [SEC] Remove all pending messages if AID overflow is happened
    public void removeAidMessages() {
        if(DBG) Log.d(TAG, "remove pending messages");
        mHandler.removeMessages(MSG_ROUTE_AID);
        mHandler.removeMessages(MSG_UNROUTE_AID);
        mHandler.removeMessages(MSG_COMMIT_ROUTING);
    }

    // [SEC] Get preferred SIM slot
    public int getPreferredSimSlot() {
        return mPrefs.getInt(SEC_PREF_PREFERRED_SIM, SEC_PREF_SIM_SLOT1);
    }

    // [SEC] Set preferred SIM slot
    public void setPreferredSimSlot(int slotId) {
        int curSlot = mPrefs.getInt(SEC_PREF_PREFERRED_SIM, SEC_PREF_SIM_SLOT1);
        if (curSlot == slotId) {
            return;
        }

        if(DBG) Log.d(TAG, "setPreferredSimSlot: " + slotId);
        mPrefsEditor.putInt(SEC_PREF_PREFERRED_SIM, slotId);
        mPrefsEditor.apply();
    }

    // [SEC] Re-iniailize to detect SIM
    public void restartNfcService() {
        if (DBG) Log.d(TAG, "restartNfcService");

        if (mState == NfcAdapter.STATE_ON || mState == NfcAdapter.STATE_TURNING_ON) {
            if (DBG) Log.d(TAG, "Restart NFC due to SIM state change");
            new EnableDisableTask().execute(TASK_RESTART);
        }
    }

    // [SEC] Set to wait EE discovery
    public void waitNfcEeDiscovery(boolean isWait) {
        if (DBG) Log.d(TAG, "Set to wait NFC EE discovery");
        mDeviceHost.doSetWaitNfcEeDiscovery(isWait);
    }

    // [SEC] Get SE list
    public int[] getSecureElementList() {
        return mDeviceHost.doGetSecureElementList();
    }

    // [SEC] Check SIM auto switch
    boolean isRestartToSwitchSim() {
        if (DBG) Log.d(TAG, "isRestartToSwitchSim");
        if (getDefaultRoute() < SEID_SIM) {
            if (DBG) Log.d(TAG, "Default route is not a UICC, do nothing");
            return false;
        }
        if (mDeviceHost.getSimDetection() == false &&
                    mSecureElementManager.isSupportAutoSwitch() &&
                    mSecureElementManager.isSupportDualSim()) {
            if (mSecureElementManager.simAutoSelectionCheck(getPreferredSimSlot())) {
                if (DBG) Log.d(TAG, "need to restart service");
                return true;
            } else {
                if (DBG) Log.d(TAG, "don't need to restart service");
                return false;
            }
        }
        return false;
    ...
    // [SEC] Set enable, disable lowpower CE
    void setPowerOffCardEmulation(boolean enable) {
        boolean current = mPrefs.getBoolean(SEC_PREF_POWEROFF_CARDEMULATION, true);
        if (current == enable) {
            if (DBG) Log.d(TAG, "No need to update");
            return;
        }
        if (DBG) Log.d(TAG, "setPowerOffCardEmulation, enable: " + enable);
        mPrefsEditor.putBoolean(SEC_PREF_POWEROFF_CARDEMULATION, enable);
        mPrefsEditor.apply();
    }

    // [SEC] Get set wait for restarting
    public boolean isWaitForRestarting() {
        return mIsWaitForRestart;
    }

    public void setWaitForRestarting(boolean isWait) {
        if (DBG) Log.d(TAG, "setWaitForRestarting: " + isWait);
        mIsWaitForRestart = isWait;
    }

    // [SEC] Set Firmware Debug Notification Level
    public void setFirmwareDebugLevel(int type, boolean enable) {
      if (DBG) Log.d(TAG, "setFirmwareDebugLevel");
      if (isNfcEnabled()) {
        synchronized (NfcService.this) {
          mDeviceHost.doSetFirmwareDebugLevel(type, enable);
        }
      }
    }

    public boolean setRfOptionFilePath(String filePath) {
        if (DBG) Log.d(TAG, "setRfOptionFilePath, filepath: " + filePath);
        boolean rfResult = false;
        // TODO: customer modify
        if (filePath == null) {
            Log.d(TAG,"checkFileExists,file does not exist");
            return false;
        }
        if (isNfcEnabled()) {
            synchronized (NfcService.this) {
                rfResult = mDeviceHost.doRFoptionUpdate(filePath.getBytes());
            }
        }
        return rfResult;
    }
    ...
    int getListenTechMask() {
        return  mDeviceHost.doGetListenTechMask();
    }

    boolean setListenTechMask(int techMask) {
        if (techMask < 0 || techMask > (NFC_LISTEN_A | NFC_LISTEN_B | NFC_LISTEN_F)) {
            if (DBG) Log.d(TAG, "invalid tech mask, value: " + techMask);
            return false;
        }

        if (DBG) Log.d(TAG, "setListenTechMask, value: " + techMask);
        if (!isNfcEnabled()) {
            Log.e(TAG, "NFC needs to be enabled to do ListenTech");
            return false;
        }
        synchronized (NfcService.this) {
            mDeviceHost.doSetListenTechMask(techMask);
        }
        return true;
    }

    byte[] getFactoryRf() {
        Log.d(TAG, "getFactoryRf...");
        int factoryMode = mPrefs.getInt(SEC_PREF_NFC_FACTORY_MODE, FACTORY_MODE_ENABLE);
        int wait_cnt = 0;
        while((!isNfcEnabled() && FACTORY_MODE_ENABLE != factoryMode &&
                        factoryMode != FACTORY_MODE_CAL_DONE) && wait_cnt++ < 20){
            try {
                Thread.sleep(200);
            } catch (InterruptedException e) {
                Log.e(TAG, "InterruptedException", e);
            }
            factoryMode = mPrefs.getInt(SEC_PREF_NFC_FACTORY_MODE, FACTORY_MODE_ENABLE);
        }
        if(!isNfcEnabled()) {
            Log.e(TAG, "NFC needs to be enabled to get calibration result!");
            return null;
        }
        if(factoryMode != FACTORY_MODE_CAL_DONE) {
            Log.e(TAG, "Factory calibration not performed yet.factoryMode="+factoryMode);
            return null;
        }
        try {
            Thread.sleep(100);
        ...
        byte[] result = mDeviceHost.doSetFactoryTestMode((byte)FACTORY_MODE_GET_CAL);
        Log.d(TAG, "getFactoryRf=" + ( result != null ? result.length : -1));
        return result;
    }

    void doFactoryRf() {
        Log.d(TAG, "doFactoryRfCalibration do user calibration");
        mPrefsEditor.putInt(SEC_PREF_NFC_FACTORY_MODE, FACTORY_MODE_USER_CAL);
        mPrefsEditor.apply();
        restartNfcService();
    }
    ...
    private final BroadcastReceiver mOwnerReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_PACKAGE_REMOVED) ||
                ...
                if (isNfcEnabled()) {
                    // [SEC] Lowpower CE disable
                    boolean isDIsableCe = !mPrefs.getBoolean(SEC_PREF_POWEROFF_CARDEMULATION, true);
                    Log.d(TAG, "isDIsableCe: "+isDIsableCe);
                    mDeviceHost.shutdown(isDIsableCe);
                }
            }
        }
    };
