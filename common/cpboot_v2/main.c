...
static void check_debug_level(struct boot_args *args)
{
	...
	if (fd < 0) {
		cbd_log("%s file open fail (%s)\n", FILE_DEBUG_LEVEL, ERR2STR);
		goto exit;
	}
	err = read(fd, rdbuf, 4);
	if (err < 0) {
		cbd_log("debug level read fail (%s)\n", ERR2STR);
		goto exit;
	}
...
exit:
	...
	if (debug_level == DBG_LOW) {
		cmd = "androidboot.force_upload=";
		memset(cmdline, 0x00, CMDLINE_BUF_SIZE);
		str = get_cmdline_str(cmdline, CMDLINE_BUF_SIZE, cmd);
		if (str) {
			cbd_log("force_upload = %s\n", str + strlen(cmd));

			if (strcmp(str + strlen(cmd), "0x5") == 0) {
				debug_level = DBG_MID;
				cbd_log("force_upload on!\n");
			}
		}
	}
...
retry:
	...
	if (g) {
		...
		cbd_log("log path (%s) gid(%d) changed(%d)\n", path, g->gr_gid, err);
	}
...
static void check_factory_log_path(void)
{
	...
	if (fd < 0) {
		cbd_log("fail to open %s (%s)\n", SWITCH_PATH, ERR2STR);
		return;
	}

	n = read(fd, buf, 3);
	if (n < 0) {
		cbd_log("fail to read %s (%s)\n", SWITCH_PATH, ERR2STR);
		goto exit;
	}
...
static int status_loop(struct boot_args *args)
{
	...
	if (fd < 0) {
		cbd_log("%s open fail, err %s\n", args->cpn->node_status, ERR2STR);
	...
	if (debug_level != DBG_LOW && debug_cp_opt == DBG_CP_NORMAL) {
		...
		if (cpboot_log[0] == '1' && cpboot_log_needed()) {
...
void traffic_monitor()
{
	...
	while (1) {
		...
		if (!fp) {
			cbd_log("Fail to open /proc/net/dev node with(%s)\n", ERR2STR);
			break;
		}
...
extern void set_sim_configuration(void);

void cbd_do_test(char *cmd)
{
	if (!cmd)
		return;

	switch (cmd[0]) {
	case '1': /* test remove_logs */
		remove_logs(LOG_DMESG, FACTORY_CPDUMP_PATH"/test", "nr");
		break;

	case '2': /* test remove_directory */
		remove_directory(FACTORY_CPDUMP_PATH"/test");
		break;

	case '3': /* test writing of sim configuration */
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		opt = getopt(argc, argv, "hdt:s:b:m:n:o:p:P:B:D:T:");
		if (opt == -1)
			break;
		...
		case 's':
			cmd = optarg;
			cbd_log("SRINFO type: %c\n", cmd[0]);
			switch(cmd[0]) {
			case '0':
				cbd_log("No srinfo");
				cbd_args.reset_info_type = NO_SRINFO;
				break;
			case '1':
				cbd_args.reset_info_type = SRINFO_TYPE_1;
				break;
			case '2':
				cbd_args.reset_info_type = SRINFO_TYPE_2;
				break;
			case '3':
				cbd_args.reset_info_type = SRINFO_TYPE_3;
				break;
			default:
