...
void switch_user(void)
{
	...
	count = getgroups(ARRAY_LEN(gids), gids);
	gids[count] = AID_WAKELOCK;
	setgroups(count + 1, gids);
...
static void *cld_reader_loop(void *param)
{
	...
	char cp_rst_buf[PROPERTY_VALUE_MAX];
	int rst_old = 0, rst_cur = 0;
	...
	property_get("vendor.cbd.cp_reset", cp_rst_buf, "0");
	rst_old = atoi(cp_rst_buf);

	while (lb_arg->forever || lb_arg->duration > 0) {
		unsigned recved_size = 0;
retry:
		property_get("vendor.cbd.cp_reset", cp_rst_buf, "0");
		rst_cur = atoi(cp_rst_buf);

		if (rst_cur != rst_old) {
			cld_log("Detect cp reset\n");
			ret = -1;
			goto exit;
		}
		rst_old = rst_cur;
		...
		ret = select(lb_arg->fd + 1, &rfds, NULL, NULL, &tv);
	...
	if (lb_arg->mode & AP_INIT_LB) {
		...
		while (lb_arg->forever || lb_arg->duration > 0) {
			...
			if (seq_diff(lb_arg->tx_seq, lb_arg->rx_seq) > 2) {
				...
				ret = ioctl(lb_arg->fd, IOCTL_MODEM_FORCE_CRASH_EXIT, CRASH_REASON_CLD);
				if (ret < 0) {
