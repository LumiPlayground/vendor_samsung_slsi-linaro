...
static void boot_wake_lock(int lock)
{
	...
	if (fd < 0) {
		cbd_log("user wake_%s open fail(%s)\n", lock ? "lock" : "unlock", ERR2STR);
		return;
	}
	ret = write(fd, name, strlen(name));
	if (ret < 0) {
		cbd_log("write fail - %s (%s)\n", name, ERR2STR);
		goto exit;
	...
	*/
	...
	if (toc[IMG_TOC].toc_count == 1) {
		...
		if (toc[IMG_TOC].m_offset == 0) {
			...
			cbd_log("ERR! invalid TOC : No info for TOC verification\n");
			goto exit;
		}
...
static int load_cp_image_by_stage(struct std_boot_args *args, u32 stage, enum cp_boot_mode mode)
{
	...
	if (ret < 0) {
		cbd_log("ERR! lseek fail at stage %u (%s)\n", stage, ERR2STR);
		goto exit;
	}
	...
	while(1) {
		...
		if (ret < 0) {
			cbd_log("ERR! read fail at stage %u (%s)\n", stage, ERR2STR);
			goto exit;
		}
		...
		if (ret) {
			cbd_log("ERR! IOCTL_LOAD_CP_IMAGE fail (%u,%d)\n", stage, ret);
			goto exit;
		}
...
void check_board_revision(void)
{
	...
	if (fd < 0) {
		cbd_log("%s open fail (%s)\n", DT_REVISION_PATH, ERR2STR);
		return;
	}
	ret = read(fd, rev, 256);
	if (ret < 0) {
		cbd_log("%s read fail (%s)\n", DT_REVISION_PATH, ERR2STR);
		close(fd);
		return;
	}
...
{
	int sim_count = get_factory_prop();
	...
	if (sim_count > 0 && sim_count <= 4) {
		...
		if (fd < 0) {
			cbd_log("SIM conf: open failed(%s)\n", ERR2STR);
			return;
		}
		...
		if (ret < 0) {
			cbd_log("SIM conf: write failed(%s)\n", ERR2STR);
			close(fd);
			return;
		}
		fsync(fd);
...
int start_shannon310_boot(struct boot_args *cbd_args)
{
	...
	*/
	...
	store_srinfo(cbd_args->cpn->rat, dl_args->std_args->dev_fd, cbd_args->reset_info_type);
	...
	if (ret < 0) {
		cbd_log("ERR! security check fail for BOOT/MAIN\n");
		goto exit;
	}
	...
	restore_srinfo(cbd_args->cpn->rat, dl_args->std_args->dev_fd, cbd_args->reset_info_type);
...
int start_shannon310_dump(struct boot_args *cbd_args)
{
	...
	** Save kernel log
	*/
	snprintf(reason, MAX_PREFIX_LEN, "cpcrash_%s_klog_before", cbd_args->cpn->rat);
	...
	snprintf(reason, MAX_PREFIX_LEN, "cpcrash_%s_klog_after", cbd_args->cpn->rat);
	save_logs(LOGB_DMESG, reason);
...
exit:
	if (ret < 0) {
		snprintf(reason, MAX_PREFIX_LEN, "cpcrash_%s_dump_fail", cbd_args->cpn->rat);
		save_logs(LOGB_DMESG, reason);
	}
...
int upload_shannon310_modem(struct boot_args *cbd_args)
{
	...
	if (ret < 0) {
		cbd_log("ERR! IOCTL_TRIGGER_KERNEL_PANIC fail (%d,%s)\n", ret, ERR2STR);
		goto exit;
	}
