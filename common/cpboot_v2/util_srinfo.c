...
static int create_srinfo_directory(char *path)
{
	...
	if (!ret) { /* path exist */
		...
		ret = mkdir(path, 0775);
		if (ret) {
			cbd_log("log path create fail : %s (%s)\n", path, ERR2STR);
			goto exit;
		}
...
static struct shmem_srinfo *alloc_srinfo(unsigned *len, int boot_fd)
{
	...
	if (ret < 0) {
		cbd_log("ioctl fail - Get srinfo(%d,%s)\n", ret, ERR2STR);
		goto exit;
	}
	*len = args->size;
...
static struct shmem_srinfo *alloc_srinfo_type2(unsigned *len, int boot_fd)
{
	struct shmem_srinfo *args;
	unsigned *magic;
	int ret;
...
static int open_srinfo_file(char *surfix)
{
	...
	ret = fstat(fd, &sb);
	if (ret) {
		cbd_log("stat fail - srinfo log (%s)\n", ERR2STR);
		close(fd);
		goto exit;
	}
	ret = fd;
	...
	if (sb.st_size > SRINFO_MAX_SIZE) {
		...
		ret = unlink(infobak);
		if (ret < 0 && ret != -EPERM) {
			cbd_log("deleate (%s) file fail(%s)\n", infobak, ERR2STR);
			goto exit;
		}
		ret = rename(infofile, infobak);
		if (ret < 0) {
			cbd_log("rename (%s -> %s) file fail(%s)\n",
					infofile, infobak, ERR2STR);
			goto exit;
		}
		ret = fd = open(infofile,
				O_WRONLY | O_APPEND | O_CREAT,
...
static unsigned write_srinfo_file(int fd, char *buf, unsigned size)
{
	...
	dprintf(fd, "\n");
	if (fsync(fd))
		cbd_log("ERR! fsync(srinfo_hex) fail (%s)\n", ERR2STR);
...
static int write_srinfo_file_type2(int fd, char *buf, unsigned size)
{
	int i;
	int ret = -1;
	char *rdbuf = buf + 0x10;
...
static int open_srinfo_last(char *surfix)
{
	...
	fd = open(file, O_RDWR);
	if (fd < 0) {
		cbd_log("last_sr_info open fail(%s)\n", ERR2STR);
		goto exit;
	}

	ret = lseek(fd, 0, SEEK_SET);
	if (ret < 0) {
		cbd_log("fd_last lseek fail(%s)\n", ERR2STR);
		close(fd);
		goto exit;
	}
	return fd;
...
static int create_srinfo_last(char *surfix)
{
	...
	fd = open(file,
		...
		S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
	if (fd < 0) {
		cbd_log("last_sr_info create fail (%s)\n", ERR2STR);
		goto exit;
	}
...
static int write_srinfo_last(int fd, char *buf, unsigned size)
{
	...
	cbd_log("store to ascii text (%d bytes)\n", i);
	ret = write(fd, rdbuf, i+1);

	if (fsync(fd))
		cbd_log("ERR! fsync(srinfo) fail (%s)\n", ERR2STR);
...
static void store_srinfo_type1(char *surfix, int dev_fd)
{
	int ret, fd;
	unsigned size;
...
static void store_srinfo_type2(char *surfix, int dev_fd)
{
	int ret, fd;
	unsigned size;
	struct shmem_srinfo *rdbuf;

	rdbuf = alloc_srinfo_type2(&size, dev_fd);
	if (!rdbuf) {
		cbd_log("alloc srinfo fail\n");
		return;
	}
	...
	ret = write_srinfo_file_type2(fd, rdbuf->buf, size);
	if (ret < 0) {
		cbd_log("srinfo write fail(%d)\n", ret);
		close(fd);
		goto exit;
...
static struct shmem_srinfo *store_srinfo_type3(char *surfix, int dev_fd)
{
	unsigned size;
	struct shmem_srinfo *rdbuf;
...
void store_srinfo(char *surfix, int dev_fd, enum srinfo_type reset_info_type)
{
	struct shmem_srinfo *rdbuf;

	switch(reset_info_type) {
	case NO_SRINFO:
		cbd_log("no srinfo");
		break;
	case SRINFO_TYPE_1:
		store_srinfo_type1(surfix, dev_fd);
		break;
	case SRINFO_TYPE_2:
		store_srinfo_type2(surfix, dev_fd);
		break;
	case SRINFO_TYPE_3:
		rdbuf = store_srinfo_type3(surfix, dev_fd);
		if (rdbuf) {
			/* TODO : send srinfo data directly to server without writing to a file */
			free(rdbuf);
		}
	}
...
void restore_srinfo(char *surfix, int dev_fd, enum srinfo_type reset_info_type)
{
	int ret, fd, size;
	char *rdbuf, filepath[64], filebak[64];
	struct shmem_srinfo *sr_args;

	if (reset_info_type != SRINFO_TYPE_1)
		return;
	...
	if (size <= 0) {
		cbd_log("last_sr_info file read fail(%s)\n", ERR2STR);
		goto exit_free;
	}
	sr_args->size = size;
	...
	if (ret < 0) {
		cbd_log("ioctl fail - Set srinfo(%d,%s)\n", ret, ERR2STR);
		goto exit_free;
	}
