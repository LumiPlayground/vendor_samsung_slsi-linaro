...
static int std_ul_recv_info(struct std_dump_args *args)
{
	...
	char string_crash_reason[][5] = { "RIL", "USER", "CPIF", "CP", "CLD" };
	...
	if (fsync(info_fd)) {
		cbd_log("ERR! fsync(info_fd) fail (%d,%s)\n", ret, ERR2STR);
		dprintf(log_fd, "%s: ERR! fsync(info_fd) fail (%d,%s)\n",
			__func__, ret, ERR2STR);
		goto exit;
	}
...
int std_boot_load_cp_bootloader(struct std_boot_args *args, u32 stage)
{
	...
	if (ret) {
		cbd_log("ERR! IOCTL_LOAD_CP_IMAGE fail (%d)\n", ret);
		goto exit;
	}
...
int std_boot_power_on(struct std_boot_args *args)
{
	...
	if (ret < 0) {
		cbd_log("ERR! IOCTL_POWER_ON fail (%d,%s)\n", ret, ERR2STR);
		goto exit;
	}
...
int std_boot_power_off(struct std_boot_args *args)
{
	...
	if (ret < 0) {
		cbd_log("ERR! IOCTL_POWER_OFF (%d,%s)\n", ret, ERR2STR);
		goto exit;
	}
...
int std_boot_power_reset(struct std_boot_args *args, enum cp_boot_mode mode_idx)
{
	...
	if (ret < 0) {
		cbd_log("ERR! IOCTL_POWER_RESET (%d,%s)\n", ret, ERR2STR);
		goto exit;
	}
...
int std_boot_complete_normal_bootup(struct std_boot_args *args)
{
	...
	if (ret < 0) {
		cbd_log("ERR! IOCTL_COMPLETE_NORMAL_BOOTUP fail (%d,%s)\n", ret, ERR2STR);
...
static int std_recv_raw_data_sit(int dev_fd, void *buffer, u32 size)
{
	...
	if (ret < 0) {
		cbd_err("ERR! poll fail (%s)\n", ERR2STR);
		goto exit;
	}

	readLen = ret = read(dev_fd, buffer, size);
	if (ret < 0) {
		cbd_err("ERR! read fail (%s)\n", ERR2STR);
		goto exit;
	}
	ret = readLen;
...
int std_dump_receive_cp_dump_sit(struct std_dump_args *args)
{
	...
	if (ret < 0) {
		cbd_log("ERR! Send DUMP START request (req:0x%X) (%s)\n", req, ERR2STR);
		goto exit;
	}
	...
	if (ret < (int)sizeof(struct sit_header)) {
		cbd_err("ERR! Try reading dump information (%s)\n", ERR2STR);
		ret = -1;
		goto exit;
	}
	...
	if (ret < recvHeader.length) {
		cbd_err("ERR! Receiving Dump info (%s)\n", ERR2STR);
		goto exit;
	}
	...
	if (ret < 0)
		...
		case CRASH_REASON_CP_ACT_CRASH:
			sprintf(string, ": Crash by %s - ",
					string_crash_reason[3]); /* CP */
			strcat(string, (char *) dump_info.reason);
			break;
	...
	ret = write(info_fd, args->cbd_args->reason, STD_CRASH_REASON_SIZE);
	if (ret < 0) {
		cbd_err("ERR! writing dump info to info fd (%s)\n", ERR2STR);
	}
	if (fsync(info_fd) < 0) {
	...
	while (1) {
		...
		if (ret < 0) {
			cbd_err("ERR write dump data fail (%s)\n", ERR2STR);
			goto exit;
		}
...
int std_security_req(struct std_boot_args *args, u32 mode, u32 p2, u32 p3)
{
	...
	if (ret != 0) {
		cbd_log("ERR! IOCTL_CHECK_SECURITY fail (%d,%s)\n", ret, ERR2STR);
