...
static int save_mif_dump(char *name)
{
	...
	if (fd == -1) {
		cbd_log("[MIF] Device node open failed!! (%s)\n", ERR2STR);
		return -EINVAL;
	}
	...
	if (!fp) {
		cbd_log("[MIF] %s File open failed!(%s)\n", file_path, ERR2STR);
		close(fd);
		return -EFAULT;
	}
	...
	if (err < 0) {
		cbd_log("[MIF] MIF_DUMP ioctl failed! (%d,%s)\n", err, ERR2STR);
		close(fd);
		fclose(fp);
		return -EFAULT;
	} else {
...
int create_empty_nv(char *path, size_t size)
{
	...
	if (nv_fd < 0) {
		cbd_log("ERR! NV(%s) open fail with %s\n", path, ERR2STR);
		ret = -EFAULT;
		goto exit;
	}
...
void remove_logs(int type, char *log_dir, char *reason)
{
	...
	cbd_log("remove logs under %s\n", log_dir);

	pdir = opendir(log_dir);
	if (!pdir) {
		cbd_log("DIR doesn't exist %s\n", log_dir);
		return;
	}

	log_name[0] = '\0';
	snprintf(log_name, MAX_PATH_LEN, "%s_%s_", cmd_name[type], reason);

	while ((de = readdir(pdir))) {
		char d_file[MAX_PATH_LEN];

		if (!strstr(de->d_name, log_name))
			continue;

		d_file[0] = '\0';
		snprintf(d_file, MAX_PATH_LEN, "%s/%s", log_dir, de->d_name);
		unlink(d_file);
	}
...
{
	int err = strncmp(path, FACTORY_CPDUMP_PATH,
			sizeof(FACTORY_CPDUMP_PATH) - 1);
	if (err) {
...
int set_file_value(char* file, char* strval)
{
	...
	fd = open(file, O_RDWR);
        if (fd < 0) {
			cbd_log("Error opening %s (%s)\n", file, ERR2STR);
		return -ENODEV;
        }

	ret = write(fd, strval, strlen(strval));
	if (ret < 0) {
		cbd_log("failed to write the value to %s(%s)\n", file, ERR2STR);
		close(fd);
		return ret;
	}
...
int get_file_value(char* file, char* ch, int len)
{
	...
	if (read(fd, ch, len) < 0) {
		cbd_log("failed to read the value from %s (%s)\n",
			file, ERR2STR);
		close(fd);
		return -1;
	}
