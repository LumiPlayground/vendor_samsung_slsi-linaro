...
static pthread_mutex_t fmradio_mutex_rds;
...
FmRadioController_slsi::FmRadioController_slsi()
{
    ...
    fmradio_mutex_rds = PTHREAD_MUTEX_INITIALIZER;
#if !defined(FM_RADIO_TEST_MODE)
    memset(&AFList, 0, sizeof(AF_List));
#endif
}
...
FmRadioController_slsi::~FmRadioController_slsi()
{
    ...
    pthread_mutex_destroy(&fmradio_mutex_rds);
...
int FmRadioController_slsi::Initialise()
{
    ...
    pthread_mutex_init(&fmradio_mutex_rds, NULL);
...
long FmRadioController_slsi::GetSoftStereoBlendCoeff(void)
{
    int ret;
    long value;

    ALOGI("FmRadioController::GetSoftStereoBlendCoeff\n");

    ret = fm_radio_check_state(&radio_state);
    if (ret < 0)
        return FM_FAILURE;

    ret = fm_radio_get_control(radio_fd, V4L2_CID_S610_SOFT_STEREO_BLEND_COEFF, &value);
    if (ret < 0)
        return FM_FAILURE;

    ALOGI("FmRadioController::GetSoftStereoBlendCoeff: %ld\n", value);

    return value;
}

long FmRadioController_slsi::GetSoftMuteCoeff(void)
{
    int ret;
    long value;

    ALOGI("FmRadioController::GetSoftMuteCoeff\n");

    ret = fm_radio_check_state(&radio_state);
    if (ret < 0)
        return FM_FAILURE;

    ret = fm_radio_get_control(radio_fd, V4L2_CID_S610_SOFT_MUTE_COEFF, &value);
    if (ret < 0)
        return FM_FAILURE;

    ALOGI("FmRadioController::GetSoftMuteCoeff: %ld\n", value);
...
void FmRadioController_slsi::SeekCancel()
{
    ...
    if (!is_seeking) {
        ALOGI("FmRadioController: is not seeking\n");
	return;
    }
...
void FmRadioController_slsi::SetVolume(long volume)
{
    ...
    if (volume < 0 || volume > 15) {
        ALOGE("FmRadioController::SetVolume: out of bound[0-15]\n");
	return;
    }
    ...
    ret = fm_radio_set_control(radio_fd, V4L2_CID_AUDIO_VOLUME, volume);
    if (ret < 0)
        return;

    current_volume = volume;

    ALOGI("FmRadioController::SetVolume: set volume %ld\n", volume);
}
...
{
    ALOGI("FmRadioController::GetMaxVolume: %d\n", FM_RADIO_MAX_VOLUME);
    return FM_RADIO_MAX_VOLUME;
}
...
/*
 * FM Radio Controller: GetSoftMute
 * Input : none
 * Output: (bool)
 *         - Current SoftMuteMode
 */
bool FmRadioController_slsi::GetSoftMute()
{
    long value;
    int ret;

    ALOGI("FmRadioController::GetSoftMute\n");

    ret = fm_radio_check_state(&radio_state);
    if (ret < 0)
        return FM_FAILURE;

    ret = fm_radio_get_control(radio_fd, V4L2_CID_S610_SOFT_STEREO_BLEND, &value);
    if (ret < 0)
        return FM_FAILURE;

    ALOGI("FmRadioController::GetSoftMute: %ld\n", value);
...
int FmRadioController_slsi::af_signal_check(AF_List *af_list)
{
    ...
    for (i = 0; i < af_list->AFCount; i++) {
        ...
        ALOGD("FmRadioController_slsi: af_signal_check: channel:%ldKHz, rssi:%d, af_threshold:%d(%d)\n",
...
void FmRadioController_slsi::radio_af_switching()
{
    ...
    if (c_rssi < af_threshold) {
        ...
        ALOGD("FmRadioController: AF Switching: pi:%d, channel:%ld\n", current_pi, curr_channel);
...
void FmRadioController_slsi::radio_data_proc(unsigned char *buf, int count)
{
    ...
    int i;
    unsigned short rdsblka;

    memset(&r_data, 0, sizeof(radio_data_t));

    for (i = 0; i < count; i += FM_RADIO_RDS_SET_NUM) {
	/* rds block seq error */
	rdsblka = ((buf[i+ 2] & FM_RADIO_RDS_BLCNT_MASK));
	if ((rdsblka != 0)) continue;
...
void FmRadioController_slsi::EnableRDS()
{
    ...
    pthread_mutex_lock(&fmradio_mutex_rds);
...
fail:
    pthread_mutex_unlock(&fmradio_mutex_rds);
}
...
void FmRadioController_slsi::SetIFCount1(int threshold)
{
    int ret;

    ALOGI("FmRadioController::SetIFCount1: threshold:%d\n", threshold);
    ...
    ALOGI("FmRadioController::SetIFCount1: set[%d] success\n", threshold);
}

int FmRadioController_slsi::GetIFCount1()
{
    long value;
    int ret;

    ALOGI("FmRadioController::GetIFCount1\n");
    ...
    ALOGI("FmRadioController::GetIFCount1: %d\n", ret);

    return ret;
}

void FmRadioController_slsi::SetIFCount2(int threshold)
{
    int ret;

    ALOGI("FmRadioController::SetIFCount2: threshold:%d\n", threshold);
    ...
    ALOGI("FmRadioController::SetIFCount2: set[%d] success\n", threshold);
}

int FmRadioController_slsi::GetIFCount2()
{
    long value;
    int ret;

    ALOGI("FmRadioController::GetIFCount2\n");
    ...
    ALOGI("FmRadioController::GetIFCount2: %d\n", ret);
...
{
    ALOGI("FmRadioController::SetSNR_th: threshold(%d) is not supported\n", threshold);
}

int FmRadioController_slsi::GetSNR_th()
{
    ALOGI("FmRadioController::GetSNR_th is not supported\n");
...
{
    ALOGI("FmRadioController::SetCnt_th: threshold(%d) is not supported \n", threshold);
}

int FmRadioController_slsi::GetCnt_th()
{
    ALOGI("FmRadioController::GetCnt_th is not supported \n");
...
#if defined(FM_RADIO_TEST_MODE) && !defined(FM_RADIO_MPTOOL)
