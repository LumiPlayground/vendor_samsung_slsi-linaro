            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (intent.getAction().equals(BROADCAST_MSG)){
                    ...
                    if (testItem.equals("LOAD")) {
                        for (Map.Entry<UUID, String> entry : mModelNames.entrySet()) {
                            mService.setCaptureAudio(entry.getKey(), true);
                            mService.loadModel(entry.getKey());
                        }
                    } else if (testItem.equals("START")) {
                        for (Map.Entry<UUID, String> entry : mModelNames.entrySet()) {
                            mService.startRecognition(entry.getKey());
                        }
                    } else if (testItem.equals("STOP")) {
                        for (Map.Entry<UUID, String> entry : mModelNames.entrySet()) {
                            mService.stopRecognition(entry.getKey());
                        }
                    } else if (testItem.equals("UNLOAD")) {
                        for (Map.Entry<UUID, String> entry : mModelNames.entrySet()) {
                            mService.unloadModel(entry.getKey());
                        }
                    } else if (testItem.contains("VTS_REC")) {
                        mService.setTestItem("VTS_REC");
                        int second;
                        try {
                            String[] arr = testItem.split("/");
                            second = Integer.parseInt(arr[1]);
                        } catch (Exception e) {
                            e.printStackTrace();
                            second = 5;
                        }
                        mService.startRecord(second, "VTS");
                    } else if (testItem.contains("NORMAL_REC")) {
                        mService.setTestItem("NORMAL_REC");
                        int second;
                        try {
                            String[] arr = testItem.split("/");
                            second = Integer.parseInt(arr[1]);
                        ...
                        mService.startRecord(second, "NORMAL");
