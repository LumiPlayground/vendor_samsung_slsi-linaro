...
{
    stdev->odm_voicemodel_mode = ODM_TRIGGER_ALWAYS_MODE;
}
...
int odm_open_pcm_for_streaming(void *stdev_ptr) {
    ...
    stdev->streaming_pcm = pcm_open(VTS_SOUND_CARD, VTS_TRICAP_DEVICE_NODE, PCM_IN,
...
int stdev_odm_set_power(struct sound_trigger_device *stdev, struct sound_model_info *sm_info,
        bool enable) {
    ...
    if (enable) {
        ...
        else if (stdev->odm_voicemodel_mode == ODM_LPSD_MODE)
            ctrl_values = svoice_lpsdrecognize_start_ctlvalue;
        else if (stdev->odm_voicemodel_mode == ODM_TRIGGER_ALWAYS_MODE)
        ...
        ALOGV("%s: Svoice Model [%s] recognization start", __func__,
            ((stdev->odm_voicemodel_mode == ODM_LPSD_MODE) ?
            "BabyCry" : ((stdev->odm_voicemodel_mode == ODM_TRIGGER_MODE) ?
            "Bixby" : "Bixby Always")));
        ...
        if ((stdev->odm_voicemodel_mode == ODM_TRIGGER_MODE
            ...
            ALOGD("%s: Svoice Model Backlog size [%d] configured", __func__, sm_info->backlog_size);
        }
        ...
        const char *key_value_pairs)
{
    ...
    if (ret >= 0) {
        ALOGV("%s: wakeup_mode = (%d)", __func__, value);
        stdev->odm_voicemodel_mode = value;
