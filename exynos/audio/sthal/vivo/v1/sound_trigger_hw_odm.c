...
void stdev_odm_init(struct sound_trigger_device *stdev)
{
    ...
    stdev->sound_card_abnormal_state = false;
#endif

#ifdef ODM_SOUND_TRIGGER
    stdev->voice_cmd_fp = NULL;
    stdev->stream_amp_buf = NULL;
    stdev->streaming_thread = 0;
    stdev->stream_gain = 0;
    stdev->kw_length = VTS_KW_LENGTH;
    stdev->kw_count = 0;
    stdev->kw_type = 0;
    pthread_mutex_init(&stdev->streaming_open_lock, (const pthread_mutexattr_t *) NULL);
    pthread_cond_init(&stdev->streaming_open_cond, (const pthread_condattr_t *) NULL);
    pthread_mutex_init(&stdev->streaming_pcm_lock, (const pthread_mutexattr_t *) NULL);
    pthread_mutex_init(&stdev->streaming_thread_loop_lock, (const pthread_mutexattr_t *) NULL);
#endif
}
...
int sound_trigger_update_sound_card_status(bool state)
{
    ...
    if (stdev->sound_card_abnormal_state == state) {
        ALOGE("%s: same as previous state, stdev->sound_card_abnormal_state:%d", __func__,stdev->sound_card_abnormal_state);
        ret = 0;
        goto exit;
    }

    stdev->sound_card_abnormal_state = state;

    if (state && stdev->is_bargein_mode_configed){
#ifdef ODM_SOUND_TRIGGER
       streaming_stop(stdev);
#endif
       close_bargein_device();
    } else if (stdev->recognize_started && !stdev->voicecall_state &&
            !state && stdev->requires_bargein_mode) {
...
static int read_data_from_ringbuf(struct sound_trigger_device *stdev, unsigned char *buff, int size) {
	...
	stdev->stop_read_data = 0;
    do {
        int avail = availSize(stdev->rbuf);
        if (avail > 0) {
            if (stdev->kw_length - read_size >= avail) {
                readRbuf(stdev->rbuf, buff + read_size, avail);
                read_size += avail;
            } else {
                int needReadSize = stdev->kw_length - read_size;
                readRbuf(stdev->rbuf, buff + read_size, needReadSize);
    ...
    } while((get_cur_time_ms() - start_time < 2000) && (stdev->kw_length != read_size) && !stdev->stop_read_data);

    ALOGD("%s, read_data time = %lldms", __func__, get_cur_time_ms() - start_time);
    return stdev->kw_length == read_size ? 0 : -1;
}

int odm_open_streaming(struct sound_trigger_device *stdev)
{
	if (!stdev->streaming_thread_start) {
...
#ifdef SUPPORT_BARGEIN_MODE	
	if (stdev->is_bargein_mode_configed) {
		stdev->stream_gain = (int32_t)(pow(10, (VTS_BARGEIN_ONESHOT_ADD_DB / 20.0f)) * (1 << 13));
		stdev->stream_gain = (int32_t)(pow(10, (VTS_ONESHOT_ADD_DB / 20.0f)) * (1 << 13));
	}
#endif
	if (checkDumpVoiceCmd()) {
		...
		stdev->voice_cmd_fp = fopen(filePath, "wb+");
	}

	return 0;
}

size_t odm_read_samples(struct sound_trigger_device *stdev,
	void *buffer,
    size_t  buffer_len)
{
	signed short *amp_out = (signed short *)buffer;

	if (stdev->stream_amp_buf == NULL) {
		stdev->stream_amp_buf = (signed short *)malloc(buffer_len * 2); // stereo
		if (!stdev->stream_amp_buf) {
			ALOGE("%s, malloc stream_amp_buf fail", __func__);
			return -1;
		}
	}
	
	readRbuf(stdev->rbuf, stdev->stream_amp_buf, buffer_len * 2);
	for (int i = 0; i < buffer_len / 2; i ++) { // 2ch -> 1ch
		*(amp_out + i) = apply_gain(stdev->stream_gain, *(stdev->stream_amp_buf + 2 * i));
	}
	
	if (stdev->voice_cmd_fp != NULL) {
		ALOGD("vts dump vocie cmd buffer_len %d", buffer_len);
		fwrite(buffer, 1, buffer_len, stdev->voice_cmd_fp);
	}
...
{
	if (stdev->voice_cmd_fp != NULL) {
        fclose(stdev->voice_cmd_fp);
        stdev->voice_cmd_fp = NULL;
    }

    if (stdev->stream_amp_buf != NULL) {
        ALOGD("%s free stream_amp_buf", __func__);
        free(stdev->stream_amp_buf);
        stdev->stream_amp_buf = NULL;
    }
...
int streaming_read(struct sound_trigger_device *stdev,
	void *buffer,
	size_t  buffer_len)
{
    size_t ret = 0;
...
#ifdef SUPPORT_BARGEIN_MODE
    if ((stdev->sound_card_abnormal_state) && stdev->requires_bargein_mode){
        ret = 0;
...
{
    int ret = 0;
    DEINIT_RBUF(stdev->rbuf);

    stdev->rbuf = (ringbuf*)calloc(1, sizeof(ringbuf));
    if (stdev->rbuf) {
        if (initRbuf(stdev->rbuf)) {
    ...
    pthread_mutex_lock(&stdev->streaming_open_lock);

    ret = pthread_create(&stdev->streaming_thread, (const pthread_attr_t *) NULL,
    ...
    if (!ret) {
        ...
        pthread_cond_wait(&stdev->streaming_open_cond, &stdev->streaming_open_lock);
    }

    pthread_mutex_unlock( &stdev->streaming_open_lock);
...
void streaming_stop(struct sound_trigger_device *stdev)
{
    ...
    if (stdev->streaming_thread_start) {
        ...
        stdev->streaming_thread_start = false;
        pthread_mutex_lock(&stdev->streaming_thread_loop_lock);
        if (stdev->streaming_thread) {
            ALOGD("%s pthread_join", __func__);
            pthread_join(stdev->streaming_thread, (void **)NULL);
            ALOGD("%s pthread_join exit", __func__);
            stdev->streaming_thread = 0;
        }
        pthread_mutex_unlock(&stdev->streaming_thread_loop_lock);
        ...
        struct sound_trigger_device *stdev, int index)
{
	...
	struct sound_trigger_phrase_recognition_event *event =
		...
		1, sizeof(struct sound_trigger_phrase_recognition_event) + stdev->kw_length);
    ...
    event->common.data_size = stdev->kw_length;
    event->common.trigger_in_data = 1;

    memset(buf_kw, 0, VTS_KW_LENGTH);
    err = read_data_from_ringbuf(stdev, buf_kw, stdev->kw_length);
    if (err) {
        free(event);
        event = NULL;
    ...
    memcpy((char *)event + data_offset + sizeof(int), &stdev->kw_length, sizeof(int));
    memcpy((char *)event + data_offset + (2 * sizeof(int)), buf_kw, stdev->kw_length - (2 * sizeof(int)));

    ALOGD("%s data_offset %d, data_size %d", __func__, data_offset, stdev->kw_length);
        ...
        struct sound_trigger_device *stdev, int index)
{
	...
	struct sound_trigger_generic_recognition_event *event =
		(struct sound_trigger_generic_recognition_event *)calloc(
					1, sizeof(struct sound_trigger_generic_recognition_event) + stdev->kw_length);
	if (!event) {
		return NULL;
	}
...
int odm_vts_load_sound_model(struct sound_trigger_device *stdev,
