...
static void set_sound_trigger_config(struct sound_trigger_device *stdev, const XML_Char **attr)
{
    ...
    if (set_info == COMMON_INFO) {
        ...
        else if (strcmp(name, "support_silent_reset") == 0)
            stdev->support_silent_reset = str_to_bool(value);
        ...
        else if (strcmp(name, "set_param") == 0) {
            int index = sm_info->param_cnt;
            ptr = strtok(value, ",");
            sm_info->param[index].msg = strtol(ptr, NULL, 16);
            ptr = strtok(NULL, ",");
            sm_info->param[index].values[0] = strtol(ptr, NULL, 16);
            ptr = strtok(NULL, ",");
            sm_info->param[index].values[1] = strtol(ptr, NULL, 16);
            ptr = strtok(NULL, ",");
            sm_info->param[index].values[2] = strtol(ptr, NULL, 16);
            sm_info->param_cnt++;
        }
    }
...
static void set_sound_model_id(struct sound_trigger_device *stdev, const XML_Char **attr)
{
    ...
    stdev->sm_info[sm_index].param_cnt = 0;
...
static void print_sound_trigger_info(struct sound_trigger_device *stdev)
{
    ...
    ALOGD("%s: support_silent_reset: %d", __func__, stdev->support_silent_reset);
    ...
    for (int id = 0; id < stdev->num_sound_model; ++id) {
        ...
        ALOGD("%s: sm_max_size: 0x%llx", __func__,  sm_info->sm_config.max_sz);
        ALOGD("%s: mem_offset: 0x%lx", __func__,  sm_info->sm_config.offset);
...
void st_xml_parser(void *dev)
{
    ...
    strlcpy(info_file_name, ST_INFO_XML_PATH, 50);
