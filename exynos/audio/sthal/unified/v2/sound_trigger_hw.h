...
typedef enum {
    ...
    EVENT_RESTART        = 0x80,
    EVENT_BARGEIN        = 0x200,  /* in case of that sthal gets barge-in event from driver */
} POLL_EVENT_TYPE;
...
struct sound_trigger_device {
    ...
    /* For ODM specific */
    struct odm_sound_trigger_device odm_stdev;
};
...
extern void stdev_vts_reset(struct sound_trigger_device* stdev);
extern void handle_stop_recognition_l(struct sound_trigger_device *stdev);
extern void stdev_join_callback_thread(struct sound_trigger_device *stdev, bool keep_vts_powered);
extern int stdev_active_callback_bitmask(struct sound_trigger_device* stdev);
extern int stdev_start_callback_thread(struct sound_trigger_device *stdev);
