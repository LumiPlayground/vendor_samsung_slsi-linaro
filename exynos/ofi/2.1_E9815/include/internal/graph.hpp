...
class Graph final {
  ...
  public:
    ...
    bool DumpTempBuffer(const char *);
  ...
  private:
    ...
    bool GetNodeDistance(const Node *node, std::size_t &distance);
    ...
    bool UpdateBufferInfo(node_info_list_t *nodeinfos, unsigned int &node_gid, int &port,
            bool &isCtiListCtrlrNode, buffer_info_t &bufinfo, const BufferList& buffers,
            bool isInputBuffer);
    ...
    bool Compile(const char *cgoPath, char **cgoBuffer, uint32_t *size, bool generate);
