...
class Pyramid : public Buffer {
  public:
    ...
    bool SetMetaData(int level, float scale, uint32_t width, uint32_t height, Image::Format format, bool malloc=false);
  ...
  private:
    ...
    void Initialize(int level, float scale, uint32_t width, uint32_t height, Image::Format format, bool malloc=false);
