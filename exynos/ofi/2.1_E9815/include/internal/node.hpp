...
class Node final : public std::enable_shared_from_this<Node> {
  ...
  public:
    ...
    int GetTempOutputOffset(int port, size_t& offset, size_t& size);
  ...
  private:
    ...
    bool AddInputBuffer(const std::size_t &port, const BufferPtr &input) {
        // system::Log::V(TAG, "%s(): port(%zu)", __FUNCTION__, port);

        if ((mInfo.IsMandatoryInputPort(port) == true) && input == nullptr) {
            // system::Log::E(TAG, "%s(): Port(%zu) should be filled", __FUNCTION__, port);
            return false;
        }

        if (port >= mInfo.GetNumOfInPorts()) {
            // system::Log::E(TAG, "%s(): Port(%zu) is too large for node(%s)", __FUNCTION__, port, GetInfo().GetName());
            return false;
        }

        if (mInputBuffers.find(port) != mInputBuffers.end()) {
            // system::Log::E(
            //     TAG, "%s(): Port(%zu) on node(%s) already has an input buffer", __FUNCTION__, port, GetInfo().GetName());
            return false;
        }
    ...
    bool AddOutputBuffer(const std::size_t &port, const BufferPtr &output) {
        ...
        if (mOutputBuffers.find(port) != mOutputBuffers.end()) {
            // system::Log::E(TAG,
            //               "%s(): Port(%zu) on node(%s) already has an output buffer",
            //               __FUNCTION__,
            //               port,
            //               GetInfo().GetName());
            return false;
    ...
    bool AddUserParam(const std::size_t &port, const UserParamPtr &param) {
        ...
        if (port > inPorts + outPorts) {
            ...
            //                __FUNCTION__,
            //                mInfo.GetName());
            // system::Log::E(TAG, "port: %zu inPorts: %zu outPorts: %zu\n", port, inPorts, outPorts);
            return false;
        }

        if (mInfo.GetSizeOfUserParam() != param->GetSize()) {
            // system::Log::E(TAG,
            //                "%s(): node(%s) invalid param size expected(%zu) and your param size(%zu).",
            //                __FUNCTION__,
            //                mInfo.GetName(),
