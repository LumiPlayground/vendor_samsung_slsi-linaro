...
/**
 * @brief ofi_load_model_at_service: load nn pre compiled model from file at service
 * @param compiled_graph_path pre-compiled graph from compiler
 * @return if err, returns 0
 */
 ...
 * @return int 0 if succeed
 */
extern int ofi_model_set_core_buf(ofi_model_id graph_id, ofi_memory mobj, int buf_idx);
 ...
 * @return ofi_memory memory object
 */
extern ofi_memory ofi_create_memory_from_fd_with_offset(int size,
                                            int fd, int offset,
                                            void *predefined_va,
                                            int32_t memtype);
 ...
 * @return ofi_usr_image target image oejct
 */
extern ofi_usr_image
ofi_convert_image_format_with_tuning(ofi_usr_image src, ofi_usr_image_type type,
                                     void *, void *);
 ...
 * @return index(int) if succeed
 */
extern int ofi_get_buffer_index_by_name(ofi_model_id model_id, const char *layer_name,
                                        ofi_buf_dir_e dir);
 ...
 */
extern int ofi_get_num_buffer_info_by_prefix(ofi_model_id model_id, const char *prefix);
 ...
 * @return int 0 if succeed
 */
extern int ofi_get_inbuf_shape_info(ofi_model_id model_id, const char *layer_name,
...
extern int ofi_get_buffer_infos_by_prefix(ofi_model_id model_id,
            const char *prefix, ofi_buffer_info *info);
...
extern int ofi_get_outbuf_shape_info_with_idx(
    ofi_model_id model_id, const char *layer_name, uint32_t *width, uint32_t *height,
