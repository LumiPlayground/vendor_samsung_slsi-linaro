...
public final class UrspRule implements Parcelable {
    ...
    UrspRule(vendor.samsung_slsi.telephony.hardware.radio.V1_2.UrspRule ur) {
    ...
    public UrspRule(int precedence,
            List<vendor.samsung_slsi.telephony.hardware.radio.V1_2.TrafficDescriptor> trafficDescriptors,
            List<vendor.samsung_slsi.telephony.hardware.radio.V1_2.RouteSelectionDescriptor> routeSelectionDescriptor) {
        mPrecedence = precedence;
        mTrafficDescriptors = new ArrayList<TrafficDescriptor>();
        for (vendor.samsung_slsi.telephony.hardware.radio.V1_2.TrafficDescriptor td : trafficDescriptors) {
            mTrafficDescriptors.add(convertToTrafficDescriptor(td));
        }
        mRouteSelectionDescriptor = new ArrayList<RouteSelectionDescriptor>();
        for (vendor.samsung_slsi.telephony.hardware.radio.V1_2.RouteSelectionDescriptor rsd : routeSelectionDescriptor) {
    ...
    private TrafficDescriptor convertToTrafficDescriptor(
        ...
        String osAppId = td.osAppId.getDiscriminator() == OptionalOsAppId.hidl_discriminator.noinit
                ? null : new String(RILUtils.arrayListToPrimitiveArray(td.osAppId.value().osAppId));
        // Rlog.d("UrspRule", "length=" + osAppId.length() + ", OSID=" + new String(RIL.extractOsId(osAppId)) + ", OsAppId=" + RIL.extractOsAppId(osAppId));
        ...
        */
        return new TrafficDescriptor(false, osAppId, null, null, dnn, null);
