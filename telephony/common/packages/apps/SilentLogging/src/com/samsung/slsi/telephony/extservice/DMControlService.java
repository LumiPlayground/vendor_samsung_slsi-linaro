...
public class DMControlService extends Service {
    ...
    private static final int EVENT_SET_AP_SYSTEM_TIME_DONE = 13;
    private static final int EVENT_SET_TCP_DUMP_MODE = 14;
    ...
    private static final int RILC_REQ_SET_AP_SYSTEM_TIME = 42;
    ...
    private static final int RILC_REQ_SET_SELFLOG_TCP_DUMP_MODE  = 619;
    ...
    // A status returned by RILC_REQ_GET_SELFLOG_STATUS/RILC_REQ_SET_SELFLOG
    public static final int SELFLOG_RESULT_SUCCESS = 0;
    public static final int SELFLOG_RESULT_FAILED = 1;
    public static final int SELFLOG_STATUS_NOT_STARTED_AVAILABLE = 0;
    public static final int SELFLOG_STATUS_NOT_STARTED_NOT_AVAILABLE = 1;
    public static final int SELFLOG_STATUS_STARTED = 2;
    public static final int SELFLOG_STATUS_NOT_SUPPORTED = 3;

    /* EVENT_NOTIFY_AUTOLOG_STATUS */
    public static final int AUTO_LOGGING_BUFFER_FULL = 2;
    ...
    private Boolean mRestartAutoLogging = false;
        ...
        public void handleMessage(Message msg) {
                ...
                */
                if (ar.exception == null) {
                    ...
                    if (data != null && data.length > 0) {
                        ...
                        mOnAutoLogging = (mAutoLogState == SELFLOG_STATUS_STARTED);
                        Log.d(TAG, "mAutoLogState=" + mAutoLogState +
                                " mOnAutoLogging=" + mOnAutoLogging);
                    }
                }
            ...
            case EVENT_AUTO_LOGGING_START:
                ...
                if (!mAutoLogRequested) {
                    ...
                    if (mAutoLogState == SELFLOG_STATUS_NOT_STARTED_AVAILABLE) {
                        mOemRil.invokeRequestRaw(RILC_REQ_SET_AP_SYSTEM_TIME, null, mHandler.obtainMessage(EVENT_SET_AP_SYSTEM_TIME_DONE));
                        int size = SystemProperties.getInt(SilentLoggingConstant.PROPERTY_AUTOLOG_SIZE, SilentLoggingConstant.AUTOLOG_SIZE_MIN);
                        Log.d(TAG, "AutoLogSize=" + size);
                        DataWriter out = new DataWriter();
                        try {
                            out.writeInt(0);
                            out.writeInt(size);
                            mOemRil.invokeRequestRaw(RILC_REQ_SET_SELFLOG, out.toByteArray(),
                                    mHandler.obtainMessage(EVENT_AUTO_LOGGING_START_DONE));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else if (mAutoLogState == SELFLOG_STATUS_NOT_STARTED_NOT_AVAILABLE) {
                        if (isSilentMode()) {
                            mAutoLogRequested = true;
                            mHandler.sendEmptyMessage(EVENT_SILENT_LOGGING_STOP);
                        } else {
                            Log.d(TAG, "AutoLogging Start Fail");
                            sendResponse(AUTO_LOGGING_START, SELFLOG_RESULT_FAILED);
                        }
                    } else if (mAutoLogState == SELFLOG_STATUS_STARTED) {
                        Log.d(TAG, "AutoLogging already started");
                        // consider SELFLOG_STATUS_STARTED as success
                        sendResponse(AUTO_LOGGING_START, SELFLOG_RESULT_SUCCESS);
                    } else {
                        Log.d(TAG, "AutoLogging not supported or unknown state");
                        sendResponse(AUTO_LOGGING_START, SELFLOG_STATUS_NOT_SUPPORTED);
                    }
                }
                break;
            case EVENT_AUTO_LOGGING_START_DONE:
                ...
                Log.d(TAG, "AutoLogging restult: " + error);
                if (error == SELFLOG_RESULT_SUCCESS || error == SELFLOG_STATUS_STARTED) {
                    mOnAutoLogging = true;
                    error = SELFLOG_RESULT_SUCCESS;  // consider SELFLOG_STATUS_STARTED as success
                }
            ...
            case EVENT_NOTIFY_AUTOLOG_STATUS:
                ...
                /*
                0x01 : Self-logging is stopped
                0x02 : Self-logging buffer is full
                */
                if (param == AUTO_LOGGING_BUFFER_FULL) {
                    Log.d(TAG, "AUTO_LOGGING_BUFFER_FULL");
                    mRestartAutoLogging = true;
            ...
            case EVENT_SAVE_AUTOLOG_RESP:
                ...
                if (mRestartAutoLogging) {
                    mRestartAutoLogging = false;
                    mHandler.sendEmptyMessage(EVENT_AUTO_LOGGING_START);
                }
                break;
            ...
            case EVENT_AUTO_LOGGING_START_WITH_PROFILE_DONE:
                ...
                mHandler.sendEmptyMessage(EVENT_AUTO_LOGGING_START);
                break;
            case EVENT_SET_AP_SYSTEM_TIME_DONE:
                Log.d(TAG, "EVENT_SET_AP_SYSTEM_TIME_DONE");
                break;
            case EVENT_SET_TCP_DUMP_MODE:
                Log.d(TAG, "EVENT_SET_TCP_DUMP_MODE");
                int tcpDumpEnable = msg.getData().getInt("tcpDumpEnable");
                int headerDumpEnable = msg.getData().getInt("headerDumpEnable");

                DataWriter dw = new DataWriter();
                try {
                    dw.writeInt(tcpDumpEnable);
                    dw.writeInt(headerDumpEnable);
                    mOemRil.invokeRequestRaw(RILC_REQ_SET_SELFLOG_TCP_DUMP_MODE, dw.toByteArray(), null);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
    ...
    private void connectToOemRilService() {
        ...
        filters.add(OemRilConstants.RILC_UNSOL_SELFLOG_STATUS);

        mOemRil = OemRil.init(getApplicationContext(), SIM1, "DMControlService", filters);
    ...
    private void setAutoLogSizeOnProperty(int size) {
        if (size < SilentLoggingConstant.AUTOLOG_SIZE_MIN) {
            size = SilentLoggingConstant.AUTOLOG_SIZE_MIN;
        } else if (size > SilentLoggingConstant.AUTOLOG_SIZE_MAX) {
            size = SilentLoggingConstant.AUTOLOG_SIZE_MAX;
        }
    ...
    class DMControlServiceBinder extends IDMControlService.Stub {
        ...
        @Override
        public void setTcpDumpModeForAutoLogging(int tcpDumpEnable, int headerDumpEnable) throws RemoteException {
            Log.d(TAG, "setTcpDumpModeForAutoLogging tcpDumpEnable:" + tcpDumpEnable + " headerDumpEnable:" + headerDumpEnable);
            Bundle data = new Bundle();
            data.putInt("tcpDumpEnable", tcpDumpEnable);
            data.putInt("headerDumpEnable", headerDumpEnable);
            Message msg = mHandler.obtainMessage(EVENT_SET_TCP_DUMP_MODE);
            msg.setData(data);
            mHandler.sendMessage(msg);
        }
    }
