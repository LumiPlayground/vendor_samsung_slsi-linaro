...
public class OemService {
    ...
    private static final int EVENT_DMD_PROXY_DEAD = 101;
    private static final int EVENT_SCED_PROXY_DEAD = 102;
    ...
    final AtomicLong mOemServiceDmdProxyCookie = new AtomicLong(0);
    final AtomicLong mOemServiceScedProxyCookie = new AtomicLong(0);
    final OemServiceDmdProxyDeathRecipient mOemServiceDmdProxyDeathRecipient;
    final OemServiceScedProxyDeathRecipient mOemServiceScedProxyDeathRecipient;
    private Handler mHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
            case EVENT_DMD_PROXY_DEAD:
                Log.e(TAG, "handleMessage: EVENT_DMD_PROXY_DEAD cookie = " + msg.obj + " mOemServiceDmdProxyCookie = " + mOemServiceDmdProxyCookie.get());
                if ((long) msg.obj == mOemServiceDmdProxyCookie.get()) {
                    resetDmdProxy();
                    getOemServiceDmdProxy();
                }
                break;
            case EVENT_SCED_PROXY_DEAD:
                Log.e(TAG, "handleMessage: EVENT_SCED_PROXY_DEAD cookie = " + msg.obj + " mOemServiceScedProxyCookie = " + mOemServiceScedProxyCookie.get());
                if ((long) msg.obj == mOemServiceScedProxyCookie.get()) {
                    resetScedProxy();
    ...
    public OemService(Context context) {
        ...
        mOemServiceDmdProxyDeathRecipient = new OemServiceDmdProxyDeathRecipient();
        mOemServiceScedProxyDeathRecipient = new OemServiceScedProxyDeathRecipient();
        getOemServiceDmdProxy();
        getOemServiceScedProxy();
    }

    private void resetDmdProxy() {
        mOemServiceDmdProxy = null;
        // increment the cookie so that death notification can be ignored
        mOemServiceDmdProxyCookie.incrementAndGet();
    }

    private void resetScedProxy() {
        mOemServiceScedProxy = null;
        // increment the cookie so that death notification can be ignored
        mOemServiceScedProxyCookie.incrementAndGet();
    }
    ...
    private IOemService getOemServiceDmdProxy() {
        ...
        try {
            ...
            if (mOemServiceDmdProxy != null) {
                ...
                mOemServiceDmdProxy.linkToDeath(mOemServiceDmdProxyDeathRecipient, mOemServiceDmdProxyCookie.incrementAndGet());
            } else {
        ...
        if (mOemServiceDmdProxy == null) {
            mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_DMD_PROXY_DEAD, mOemServiceDmdProxyCookie.incrementAndGet()), IRADIO_GET_SERVICE_DELAY_MILLIS);
        }
        return mOemServiceDmdProxy;
    }
    ...
    private IOemService getOemServiceScedProxy() {
        ...
        try {
            ...
            if (mOemServiceScedProxy != null) {
                ...
                mOemServiceScedProxy.linkToDeath(mOemServiceScedProxyDeathRecipient, mOemServiceScedProxyCookie.incrementAndGet());
            } else {
        ...
        if (mOemServiceScedProxy == null) {
            mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_SCED_PROXY_DEAD, mOemServiceScedProxyCookie.incrementAndGet()), IRADIO_GET_SERVICE_DELAY_MILLIS);
        }
        return mOemServiceScedProxy;
    }
    ...
    final class OemServiceDmdProxyDeathRecipient implements HwBinder.DeathRecipient {
        @Override
        public void serviceDied(long cookie) {
            // Deal with service going away
            Log.e(TAG, "serviceDied");
            notifyForOemServiceDmdDisconnected();
            mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_DMD_PROXY_DEAD, cookie), IRADIO_GET_SERVICE_DELAY_MILLIS);
        }
    }

    final class OemServiceScedProxyDeathRecipient implements HwBinder.DeathRecipient {
        @Override
        public void serviceDied(long cookie) {
            // Deal with service going away
            Log.e(TAG, "serviceDied");
            notifyForOemServiceScedDisconnected();
            mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_SCED_PROXY_DEAD, cookie), IRADIO_GET_SERVICE_DELAY_MILLIS);
        }
    }
}
