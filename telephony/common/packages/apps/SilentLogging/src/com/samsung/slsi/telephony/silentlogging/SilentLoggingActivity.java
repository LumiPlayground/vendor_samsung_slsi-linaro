...
public class SilentLoggingActivity extends Activity {
    ...
    private void enableUI() {
        ...
        boolean enforcing = SilentLoggingControlInterface.getInstance().isSelinuxEnforcing();
        boolean allowTcpdump = !Build.IS_USER;
        Log.d(TAG, "IS_ENG=" + Build.IS_ENG + " IS_USERDEBUG=" + Build.IS_USERDEBUG +
                   " IS_USER=" + Build.IS_USER + " enforcing=" + enforcing + " allowTcpdump=" + allowTcpdump);

        if (!allowTcpdump) {
            tcpCheckbox.setChecked(false);
            tcpCheckbox.setText("TCP (Not allowed by SELinux)");
        } else {
            tcpCheckbox.setText("TCP");
        }

        tcpCheckbox.setEnabled(allowTcpdump);
