...
public class SelectSimPreferenceFragment extends PreferenceFragment {
    ...
    public void onResume() {
        mPrefSim1 = findPreference(ApnModeConstants.KEY_SIM1);
        mPrefSim2 = findPreference(ApnModeConstants.KEY_SIM2);
        mSimOperator1 = TelephonyManager.from(getContext())
                .getSimOperatorNumericForPhone(SIM1);
        mSimOperator2 = TelephonyManager.from(getContext())
                .getSimOperatorNumericForPhone(SIM2);
        String operatorName1 = TelephonyManager.from(getContext())
                .getSimOperatorNameForPhone(SIM1);
        String operatorName2 = TelephonyManager.from(getContext())
                .getSimOperatorNameForPhone(SIM2);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        ApnModeActivity activity = (ApnModeActivity) getActivity();
        if (preference.getKey().equals(ApnModeConstants.KEY_SIM1)) {
            activity.onNextFragment(mSimOperator1,
                    SubscriptionManager.from(getContext()).getSubId(SIM1)[0]);
        } else if (preference.getKey().equals(ApnModeConstants.KEY_SIM2)) {
            activity.onNextFragment(mSimOperator2,
                    SubscriptionManager.from(getContext()).getSubId(SIM2)[0]);
        }
