...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    void registerListeners() {
        ...
        filter.addAction("android.intent.action.ACTION_SET_UICC_SUBSCRIPTION_CHANGED");
        filter.addAction("com.android.internal.telphony.TOTAL_OOS");
    ...
    public boolean getMobileDataConnected(int subid) {
        if (mMobileSignalControllers.indexOfKey(subid) >= 0) {
            return mMobileSignalControllers.get(subid).getState().dataConnected;
        }
        return false;
    }

    public boolean getMobileDefaultConnected(int subid) {
        if (mMobileSignalControllers.indexOfKey(subid) >= 0) {
            return mMobileSignalControllers.get(subid).getState().isDefault;
        }
        return false;
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action) {
            ...
            case CarrierConfigManager.ACTION_CARRIER_CONFIG_CHANGED:
                ...
                mReceiverHandler.post(this::handleConfigurationChanged);
                break;
            case "com.android.internal.telphony.TOTAL_OOS":
                for (int i = 0; i < mMobileSignalControllers.size(); i++) {
                    MobileSignalController controller = mMobileSignalControllers.valueAt(i);
                    controller.handleBroadcast(intent);
                }
    ...
    protected void updateNoSims() {
        ...
        Log.d(TAG, "updateNoSims hasNoSubs("+hasNoSubs+"), simDetected("+simDetected
                +"), mHasMobileDataFeature("+mHasMobileDataFeature+"), size("+mMobileSignalControllers.size()+")");
        ...
        if (mobile != null) {
            ...
            if (datatype != null) {
                controller.getState().iconGroup =
                        ...
                        datatype.equals("3g") ? ((mConfig.show4gFor3g) ? TelephonyIcons.FOUR_G : TelephonyIcons.THREE_G) :
                        datatype.equals("4g") ? ((mConfig.show4gFor3g) ? TelephonyIcons.FOUR_G_LTE : TelephonyIcons.FOUR_G) :
                        ...
                        datatype.equals("lte") ? ((mConfig.show4gFor3g) ? TelephonyIcons.FOUR_G_LTE : TelephonyIcons.LTE) :
