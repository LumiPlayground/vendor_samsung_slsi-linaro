...
public class OemRilBinderManager {
    ...
    private static final String PACKAGE_NAME = "com.samsung.slsi.telephony.oemril";
    private static final String SERVICE_NAME = "com.samsung.slsi.telephony.oemril.service.OemRilService";
    ...
    void onError(int error, Object ret, Message onComplete) {
        CommandException ex;
        ex = CommandException.fromRilErrno(error);
        if (onComplete != null) {
            AsyncResult.forMessage(onComplete, ret, ex);
            onComplete.sendToTarget();
        }
    }

    public void invokeOemRilRequestRaw(int request, byte[] data, Message onComplete, int phoneId) {
        if (mOemRilService != null) {
            try {
            ...
            onError(RILConstants.RADIO_NOT_AVAILABLE, null, onComplete);
        }
    }
}
