...
public:
    IsimAuth();
    IsimAuth(const IsimAuth &) = delete;
    IsimAuth& operator=(IsimAuth const&) = delete;
    virtual ~IsimAuth();
    ...
    BOOL ParseWithAuthType(const char *pString);
    void Erase(void);
...
public:
    ...
    SimAuthentication(const SimAuthentication &) = delete;
    SimAuthentication& operator=(SimAuthentication const&) = delete;
...
public:
    SimGbaAuth();
    SimGbaAuth(const SimGbaAuth &) = delete;
    SimGbaAuth& operator=(SimGbaAuth const&) = delete;
    virtual ~SimGbaAuth();
    BYTE *GetAuth() { return m_pAuth; }
    int GetLength() { return m_nLength; }
...
public:
    ...
    SimAPDU(const SimAPDU &) = delete;
    SimAPDU& operator=(SimAPDU const&) = delete;
    virtual ~SimAPDU();
...
public:
    ...
    SimOpenChannel(const SimOpenChannel &) = delete;
    SimOpenChannel& operator=(SimOpenChannel const&) = delete;
...
public:
    ...
    CarrierRestrictionsData(const CarrierRestrictionsData &) = delete;
    CarrierRestrictionsData& operator=(CarrierRestrictionsData const&) = delete;
    virtual ~CarrierRestrictionsData();

    int GetAllowCarrierLength() { return m_nAllowedCarrierLength; }
    int GetExcludeCarrierLength() { return m_nExcludedCarrierLength; }
    CarrierInfo *GetAllowCarrier() { return m_pAllowedCarriers; }
    CarrierInfo *GetExcludeCarrier() { return m_pExcludedCarriers; }
    bool GetAllowedCarriersPrioritized() { return m_bAllowedCarriersPrioritized; }
    INT32 GetMultiSimLockPolicy() { return m_nMultiSimPolicy; }
...
class SlotPortMapping : public RequestData
{
public:
    SlotPortMapping(const int nReq, const Token tok, const ReqType type = REQ_FW);
    virtual ~SlotPortMapping();

    int GetNumOfSlots() { return m_nNum_Slots; }
    int GetNumOfPorts() { return m_nNum_Ports; }
    int GetPhysicalSlotId(int index) { return m_nPhysicalSlotId[index]; }
    int GetPortId(int index) { return m_nPortId[index]; };

    virtual INT32 encode(char *data, unsigned int length);
private:
    int m_nNum_Slots;
    int m_nNum_Ports;
    int m_nPhysicalSlotId[MAX_PORT_NUM];
    int m_nPortId[MAX_PORT_NUM];
};
