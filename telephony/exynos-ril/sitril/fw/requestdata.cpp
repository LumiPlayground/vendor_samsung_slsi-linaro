...
StringRequestData *StringRequestData::Clone() const
{
    ...
    if (p != NULL && m_szData != NULL) {
        size_t size = strlen(m_szData) + 1;
        p->m_szData = new char[size] {};
        strlcpy(p->m_szData, m_szData, size);
    }
    return p;
}
 ...
 */
OemStringsRequestData::OemStringsRequestData(const int nReq, const Token tok, const ReqType type)
    : RequestData(nReq, tok, type), m_nStringCount(-1), m_ptrStrings(NULL) {
}

OemStringsRequestData::~OemStringsRequestData() {
    if (m_nStringCount > 0 && m_ptrStrings) {
        for (int i = 0; i < m_nStringCount; i++) {
            delete [](m_ptrStrings[i]);
            m_ptrStrings[i] = NULL;
...
int OemStringsRequestData::encode(char *data, unsigned int datalen) {
    if ((0 == datalen) || (NULL == data))
        return 0;

    char *pVal = NULL;
    char *pCur = data;
    m_nStringCount = (int)*pCur;
    pCur += 4;
    ...
    for (int i = 0; i < m_nStringCount; i++) {
        int strNum = (int)*pCur;
        pCur += 4;
        pVal = new char[strNum + 1];
        if (pVal != NULL) {
            memset(pVal, 0, strNum + 1);
            strncpy(pVal, pCur, strNum);
            pCur += strNum;
        }
        m_ptrStrings[i] = pVal;
    }  // end for i ~
...
OemStringsRequestData *OemStringsRequestData::Clone() const {
    OemStringsRequestData *p = new OemStringsRequestData(m_nReq, m_tok, m_reqType);
    if (p != NULL) {
        if (m_ptrStrings && m_nStringCount > 0) {
            p->m_nStringCount = m_nStringCount;
            p->m_ptrStrings = (char **)new char *[m_nStringCount] {};
            if (p->m_ptrStrings) {
                for (int i = 0; i < m_nStringCount; i++) {
                    if (m_ptrStrings[i]) {
                        size_t len = strlen(m_ptrStrings[i]);
                        p->m_ptrStrings[i] = new char[len + 1] {};
                        if (p->m_ptrStrings[i]) {
                            strlcpy(p->m_ptrStrings[i], m_ptrStrings[i], len + 1);
                        }
                    }
                }
            }
...
const char *OemStringsRequestData::GetString(int index) const {
    if (m_ptrStrings != NULL && 0 <= index && index < m_nStringCount) {
        return m_ptrStrings[index];
    }
    return NULL;
