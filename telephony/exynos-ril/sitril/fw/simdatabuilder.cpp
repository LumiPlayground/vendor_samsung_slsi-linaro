...
const RilData *SimDataBuilder::BuildSimDepersonalizationResponse(int nPersoType, int nRemainCount)
{
    RilDataInts *pRilData = new RilDataInts(2);
    if(pRilData != NULL) {
        pRilData->SetInt(0, nPersoType);
        pRilData->SetInt(1, nRemainCount);
    }
    ...
    if (pRilData != NULL) {
        ...
        if (pResponse != NULL && 0 < nLength) {
            ...
            if (pResponse[0] == 0x62 && (nFcpLength + 2) == nLength) { // 2 means 1st(Type) and 2nd(Length) byte
                ...
                for (int nLimit = 0; (nOffset - 2) < nFcpLength && nLimit < 200; nLimit++) {
                    ...
                    case 0x82:
                        ...
                        if (0x02 < pResponse[nOffset+1]) {     // Length
                            if (acSimRsp[13] != EF_TYPE_TRANSPARENT) {
                                // It's too big for 2G specification
                                if(0 < pResponse[nOffset+4]) acSimRsp[14] = 0xFF;        // Record Size
        ...
        if (0 < nLength && pSimResponse != NULL) {
        ...
        if (pszSimIoString != NULL) {
            ...
            RilLogV("SimDataBuilder::%s() Response:[(%d)%s]", __FUNCTION__,
                    strlen(pszSimIoString), pszSimIoString);
...
const RilData *SimDataBuilder::BuildSimGetSimAuthResponse(int nAuthType
        , const int nParameterLength, const int nAuthLen, const BYTE *pAuthResp)
{
    ENTER_FUNC();
    if (nAuthLen == 0 || pAuthResp == NULL) {
    ...
    SimIoResponse *pRilData = new SimIoResponse();
    if(pRilData != NULL) {
        if((nAuthLen + 2 + 2) <= nParameterLength) {  // AuthType(1) + AuthLength(1) + SW1(1) + SW2(1)
            // Encode BASE64
            char *pResponse = NULL;
            ...
            RilLogE("SimDataBuilder::%s() Both SW1 and SW2 are not included, nParameterLength(%d), nAuthLen(%d)"
                    , __FUNCTION__, nParameterLength, nAuthLen);
            delete pRilData;
            LEAVE_FUNC();
            return NULL;
        }
...
const RilData *SimDataBuilder::BuildSimTransmitApduBasicResponse(int nLength, BYTE *pResponse)
{
    ...
    if (pRilData != NULL) {
        if (pResponse != NULL && 2 < nLength) {
            ...
            if (pszSimIoString != NULL) nDataLength += (strlen(pszSimIoString) + 1);
            ...
            RilLogV("SimDataBuilder::%s() SW1:%d=0x%02X, SW2:%d=0x%02X", __FUNCTION__
                    , pRilRsp->sw1, pRilRsp->sw1, pRilRsp->sw2, pRilRsp->sw2);
            if (pszSimIoString != NULL) {
                ...
                RilLogV("SimDataBuilder::%s() Response:[(%d)%s]", __FUNCTION__
                        , strlen(pRilRsp->simResponse), pRilRsp->simResponse);
                delete [] pszSimIoString;
            } else {
...
const RilData *SimDataBuilder::BuildSimTransmitApduChannelResponse
        (BYTE sw1, BYTE sw2, int nLength, BYTE *pResponse)
{
    ENTER_FUNC();
    ...
    if(pRilData != NULL) {
        if(pResponse!=NULL && 0 < nLength) {
            ...
            pszSimIoString = new char[(nLength*2)+1];
            if(Value2HexString(pszSimIoString, pSimResponse, nLength) == -1) {
                RilLogE("SimDataBuilder::%s() Value2HexString() failed, Response Length:%d"
...
const RilData *SimDataBuilder::BuildPbCapaResponse(int* pbCapa)
{
    RilDataInts *rildata = new RilDataInts(10);

    for(int i = 0; i < 10; i++)
        rildata->SetInt(i, pbCapa[i]);
...
const RilData *SimDataBuilder::BuildGetCarrierRestrictionsResponse(
        int nAllowedLength, RIL_Carrier *pAllowed, int nExcludedLength, RIL_Carrier *pExcluded
        , bool m_bAllowedPrioritized, int nMultiSimPolicy)
{
    ...
    if(pRilData != NULL) {
        RIL_CarrierRestrictions_V1_4 carrierRestrictions;
        carrierRestrictions.carriers.len_allowed_carriers = nAllowedLength;
        carrierRestrictions.carriers.len_excluded_carriers= nExcludedLength;
        carrierRestrictions.carriers.allowed_carriers = pAllowed;
        carrierRestrictions.carriers.excluded_carriers = pExcluded;
        carrierRestrictions.carriers.allowedCarriersPrioritized = m_bAllowedPrioritized;
        carrierRestrictions.multiSimPolicy = nMultiSimPolicy;

        pRilData->SetData(&carrierRestrictions, sizeof(RIL_CarrierRestrictions_V1_4));
    }
...
public:
    ...
    void SetData(int lockType, int lockCodeCount, const char *lockCode) {
        int lockCodeSize = 0;
        if (0 < lockCodeCount && lockCode != NULL) {
