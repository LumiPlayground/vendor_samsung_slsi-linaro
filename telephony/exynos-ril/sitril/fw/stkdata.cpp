...
BOOL StkData::Parse(StringRequestData *pString)
{
    ...
    Erase();
    int nLength = strlen(pString->GetString())/2;
    m_pData = new BYTE[nLength];
    m_nLength = HexString2Value(m_pData, nLength, pString->GetString());
...
{
    if(pString==NULL || pString->GetStringCount()==0
        || pString->GetString(0)==NULL || strlen(pString->GetString(0))==0) return FALSE;

    Erase();
    int nLength = strlen(pString->GetString(0))/2;
    m_pData = new BYTE[nLength];
    m_nLength = HexString2Value(m_pData, nLength, pString->GetString(0));
