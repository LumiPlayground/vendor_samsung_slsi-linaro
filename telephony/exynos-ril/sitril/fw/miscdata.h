...
class ApnSettingsData : public RequestData
{
    protected:
        int m_nApnReqType;
        std::map<std::string, std::string> m_ApnTable;
    ...
    public:
        ...
        std::string GetApnValues(char *key);
};
...
public:
    ...
    CarrierInfoForImsiEncryptionData(const CarrierInfoForImsiEncryptionData &) = delete;
    CarrierInfoForImsiEncryptionData& operator=(CarrierInfoForImsiEncryptionData const&) = delete;
    ...
    int GetKeyType() { return m_nkeyType; }
...
public:
    ...
    SignalStrengthReportingCriteria(const SignalStrengthReportingCriteria&) = delete;
    SignalStrengthReportingCriteria& operator=(SignalStrengthReportingCriteria const&) = delete;
...
class SignalStrengthReportingCriteria_V2_0 : public RequestData {
    using SignalThresholdInfo = android::hardware::radio::impl::network::V2_0::RIL_SignalThresholdInfo;
public:
    SignalStrengthReportingCriteria_V2_0(const int nReq, const Token tok, const ReqType type = REQ_FW);
    ~SignalStrengthReportingCriteria_V2_0()=default;

    INT32 encode(char *data, unsigned int length);
    bool isValidArguements() const;
    int GetSignalThresholdInfoNum();
    const SignalThresholdInfo& at(int pos) const;

private:
    std::vector<android::hardware::radio::impl::network::V2_0::RIL_SignalThresholdInfo> m_signalThresholdInfos;
};
...
public:
    ...
    LinkCapacityReportingCriteria(const LinkCapacityReportingCriteria&) = delete;
    LinkCapacityReportingCriteria& operator=(LinkCapacityReportingCriteria const&) = delete;
