    ...
    case CommandDetail::OPEN_CHANNEL:
        ...
        for(int i=0; i<pComprehension->GetTlvCount(); i++)
        {
            ...
            case TAG_BEARER_DESCRIPTION:
                ...
                m_tOpenChannelInfo.BearerType = (BEARER) pBearerDesc->GetType();
        ...
        if(m_pStkTcpSocket == NULL)
        {
           ...
           do {
               if((nError = m_pStkTcpSocket->Connect())!=0)
               {
                   ...
                   if(nRemain>0)
                   {
                       WaitForTimeUs(__FUNCTION__, nInterval * 1000U);
...
{
    ENTER_FUNC();
    m_tOpenChannelInfo.BearerType = (BEARER) 0;
...
int CStkModule::OpenChannel(Comprehension *pComprehension)
{
    ...
    if ((m_tOpenChannelInfo.BearerType == BEARER_DEFAULT)
        || (m_tOpenChannelInfo.strNetAccessName == ""))
    {
...
int CStkModule::SendOnTcp(int nMode, string strData, int nDevId)
{
    ...
    int buffer_len = HexString2Value((unsigned char*)buffer, sizeof(buffer), (char *)m_strSendDataBuffer.c_str());
...
int CStkModule::Transfer(Message *pMsg)
{
    ...
    if(m_pStkService) {
        LOGE("OnStkModule is not implemented");
        //m_pStkService->OnStkModule(pMsg);
    }
    else {
