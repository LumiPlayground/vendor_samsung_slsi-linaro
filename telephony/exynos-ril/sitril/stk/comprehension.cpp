...
/*
Comprehension::Comprehension(std::vector<CTLV *> vectorTlvs)
{
    Comprehension();
    Initialize();
...
void Comprehension::Set(std::vector<CTLV *> vectorTlvs)
{
    ENTER_FUNC();
    Initialize();
...
CTLV *Comprehension::NewTlv(BYTE cTag)
{
    ...
    case TAG_ITEM_IDENTIFIER:
        pTlv = new ItemIdentifier();
        break;
    case TAG_ITEM_ID_IDENTIFIER:
        pTlv = new ItemIDIdentifier();
        break;
    case TAG_URL:
        pTlv = new Url();
        break;
    case TAG_BROWSER_TERMINATION_CAUSE:
        pTlv = new BrowserTerminationCause();
        break;
    case TAG_LANGUAGE:
        pTlv = new Language();
        break;
    case TAG_RESPONSE_LENGTH:
        pTlv = new ResponseLength();
        break;
    case TAG_TONE:
        pTlv = new Tone();
        break;
    case TAG_DEFAULT_TEXT:
        pTlv = new DefaultText();
        break;
    case TAG_IMMEDIATE_RESPONSE:
        pTlv = new ImmediateResponse();
        break;
    case TAG_HELP_REQUEST:
        pTlv = new HelpRequest();
        break;
    case TAG_TIMER_IDENTIFIER:
        pTlv = new TimerIdentifier();
        break;
...
{
    if(nIndex >= (int) m_vectorTlvs.size()) {
        LOGE("%s() index %d is invalid... vector size %d", __FUNCTION__, nIndex, (int) m_vectorTlvs.size());
        return NULL;
    }
