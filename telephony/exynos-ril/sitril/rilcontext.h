...
class RilContext {
  ...
  public:
    ...
    virtual void OnRequest(int request, void *data, unsigned int datalen, RIL_Token t, long delayedMillis) = 0;
    ...
    virtual RIL_CardState GetUiccCardState() = 0;
    ...
    virtual int Send(ModemData *pModemData, UINT nDstServiceId, UINT nResult, RIL_Token token = NULL) = 0;
    virtual int Send(void *data, unsigned int datalen, RIL_Token token = NULL) = 0;
    ...
    virtual const ServiceState &GetRadioServiceState() const = 0;
    virtual void ResetModem(const char *reason) = 0;
    virtual int BroadcastSystemMessage(int messageId, RilData *data = NULL) = 0;
};
