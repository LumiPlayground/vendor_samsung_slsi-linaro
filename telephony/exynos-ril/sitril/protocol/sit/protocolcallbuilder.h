 ...
 * ProtocolCallBuilder
 */
class ProtocolCallBuilder : public ProtocolCallBuilderInterface
{
private:
    ProtocolCallBuilderInterface *protocolCallBuilderInterface = NULL;

public:
    ProtocolCallBuilder();
    ProtocolCallBuilder(const ProtocolCallBuilder&) = delete;
    ProtocolCallBuilder& operator=(ProtocolCallBuilder const&) = delete;
...
public:
    ...
    ModemData*BuildGetCallForwardingStatus(SsStatusType status /*not used*/, SsCfReason reason, SsClassX service_class, int toa, char* number, int timeseconds);
    ModemData *BuildSetCallWaiting(SsModeType status, SsClassX service_class);
