...
/*
 * protocolsoundadapterlegacy.cpp
 *
 *  Created on: 2014. 6. 28.
 *      Author: sungwoo48.choi
 */

#include "protocolsoundadapterlegacy.h"
#include "rildef.h"
#include "sitdef.h"
#include "modemdata.h"

/**
 * ProtocolSoundGetMuteRespAdapterLegacy
 */
int ProtocolSoundGetMuteRespAdapterLegacy::GetMuteState() const
{
...
/**
 * ProtocolSoundRingbackToneIndAdapterLegacy
 */
...
int ProtocolSoundRingbackToneIndAdapterLegacy::GetRingbackToneState() const
{
    if (m_pModemData != NULL) {
...
int ProtocolSoundRingbackToneIndAdapterLegacy::GetFlag() const
{
    if (m_pModemData != NULL) {
...
/**
 * ProtocolSoundGetVolumeRespAdapterLegacy
 */
int ProtocolSoundGetVolumeRespAdapterLegacy::GetVolume() const
{
    if (m_pModemData != NULL) {
...
/**
 * ProtocolSoundGetAudiopathRespAdapterLegacy
 */
int ProtocolSoundGetAudiopathRespAdapterLegacy::GetAudiopath() const
{
    if (m_pModemData != NULL) {
        sit_snd_get_audiopath_rsp *data = (sit_snd_get_audiopath_rsp *)m_pModemData->GetRawData();
        if (data != NULL && data->hdr.id == SIT_GET_AUDIO_PATH) {
...
/**
 * ProtocolSoundGetMultiMICRespAdapterLegacy
 */
int ProtocolSoundGetMultiMICRespAdapterLegacy::GetMultimicmode() const
{
    if (m_pModemData != NULL) {
        sit_snd_get_multimic_rsp *data = (sit_snd_get_multimic_rsp *)m_pModemData->GetRawData();
        if (data != NULL && data->hdr.id == SIT_GET_MULTI_MIC) {
...
/**
 * ProtocolSoundWBAMRReportAdapterLegacy
 */
int ProtocolSoundWBAMRReportAdapterLegacy::GetStatus() const
{
    if (m_pModemData != NULL) {
...
int ProtocolSoundWBAMRReportAdapterLegacy::GetCallType() const
{
...
/**
 * ProtocolSoundGetWBAMRCapabilityAdapterLegacy
 */
int ProtocolSoundGetWBAMRCapabilityAdapterLegacy::GetWbAmr() const
{
    if (m_pModemData != NULL) {
