...
/*
 * protocolsmsbuilderlegacy.cpp
 *
 *  Created on: 2014. 7. 3.
 *      Author: sungwoo48.choi
 */
#include "protocolsmsbuilderlegacy.h"
#include "rillog.h"
#include "modemdata.h"
...
ModemData *ProtocolSmsBuilderLegacy::BuildSendSms(const char *smsc, int smscLen, const char *pdu, int pduSize, bool bExpectMore)
{
    if (pdu == NULL || *pdu == 0) {
        RilLogE("Wrong pdu");
        return NULL;
    ...
    InitRequestHeader(&req.hdr, bExpectMore ? SIT_SEND_SMS_EXPECT_MORE : SIT_SEND_SMS, length);
...
ModemData *ProtocolSmsBuilderLegacy::BuildSmsAck(int result, int tpid, int error)
{
    if (result < 0) {
        return NULL;
    }
...
ModemData *ProtocolSmsBuilderLegacy::BuildSmsAck(int result, int tpid, const char *pdu, int pduSize)
{
    if (result < 0) {
        return NULL;
    }
...
ModemData *ProtocolSmsBuilderLegacy::BuildSmscAddress()
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildSmscAddress(int sca_len, const char *sca)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildSmsMemoryStatus(int status)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildWriteSmsToSim(int status, int index, int pduSize, const char *pdu)
{
    if (pduSize <= 0 || pduSize > MAX_GSM_SMS_TPDU_SIZE) {
        return NULL;
    }
...
ModemData *ProtocolSmsBuilderLegacy::BuildDeleteSmsOnSim(int index)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildGetBroadcastSmsConfig()
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildSetBroadcastSmsConfig(
   const RIL_GSM_BroadcastSmsConfigInfo *rgbsci, int num)
{
    ...
    for (int i = 0; i < num; i++) {
        req.cfgitem[i].from_svc_id = (int16_t)rgbsci[i].fromServiceId;
        req.cfgitem[i].to_svc_id = (int16_t)rgbsci[i].toServiceId;
        req.cfgitem[i].from_code_scheme = (uint8_t)rgbsci[i].fromCodeScheme;
        req.cfgitem[i].to_code_scheme = (uint8_t)rgbsci[i].toCodeScheme;
        req.cfgitem[i].selected = (uint8_t)rgbsci[i].selected;
    }

    return new ModemData((char *)&req, length);
}

ModemData *ProtocolSmsBuilderLegacy::BuildSmsBroadcastActivation(int bcst_act)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildGetStoredSmsCount(int sim_id)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildSendCdmaSms(const char *msg, int msgLen, bool bExpectMore)
{
    ...
    InitRequestHeader(&req.hdr, bExpectMore ? SIT_CDMA_SEND_SMS_EXPECT_MORE : SIT_CDMA_SEND_SMS,
            length);
...
ModemData *ProtocolSmsBuilderLegacy::BuildSendCdmaSmsAck(int tpid, int errClass, int errCode)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildWriteCdmaSmsToRuim(int status, const char *msg, int msgLen)
{
    if (msg == NULL || msgLen <= 0) {
        RilLogE("Wrong message data!!!");
        return NULL;
...
ModemData *ProtocolSmsBuilderLegacy::BuildDeleteCdmaSmsOnRuim(int index)
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildGetCdmaBroadcastSmsConfig()
{
...
ModemData *ProtocolSmsBuilderLegacy::BuildSetCdmaBroadcastSmsConfig(
...
ModemData *ProtocolSmsBuilderLegacy::BuildCdmaSmsBroadcastActivation(int act)
{
