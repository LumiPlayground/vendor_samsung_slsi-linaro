...
INT32 RadioControlMessageMgr::DumpData(char *buffer, UINT32 bufLen, BOOL rx) {
    return DumpDataStatic(buffer, bufLen, rx, m_pIo->GetIoChannelName(), m_nPrintSimIo);
}

INT32 RadioControlMessageMgr::DumpDataStatic(char *buffer, UINT32 bufLen, BOOL rx, const char *ioname_p, int printSimIo) {
    UINT16 rcm_len, rcm_id, rcm_category, rcm_num;
    char headerInfo[128];
    char dump[CRilLog::m_uiMaxLogBufferSize] = { };
    int i, j=0;
    const char *ioname = ioname_p ? ioname_p : "Unknown I/O";

    if (buffer == nullptr || bufLen < sizeof(RCM_GEN_HEADER)) {
        RilLogE("Invalid parameters.");
        return -1;
    }

    RCM_GEN_HEADER *rcm_hdr = (RCM_GEN_HEADER *)buffer;
    uint32_t rcm_type = rcm_hdr->type;
    ...
    if (printSimIo == LOGLV_SIM_IO_NONE && rcm_category == ((SIT_BASE_ID_SIM>>8)&0x00FF) && rcm_num == (SIT_SIM_IO&0x00FF)) return 0;

    switch (rcm_type) {
    case RCM_TYPE_REQUEST:
    case RCM_TYPE_RESPONSE: {
        if (bufLen < (uint32_t)sizeof(RCM_HEADER)) {
            RilLogE("Invalid data size for solicited message.");
            return -1;
        }
        RCM_HEADER *rcm_sol_hdr = (RCM_HEADER *)rcm_hdr;
        uint32_t rcm_token = rcm_sol_hdr->ext.req.token;
        if (rcm_type == RCM_TYPE_REQUEST) {
            snprintf(headerInfo, sizeof(headerInfo) - 1,
                    "[%s][%s] %s, REQ, %d bytes, Token(0x%04x)",
                    ioname, rx==TRUE?"RX":"TX",
                    ProtocolUtils::rcmMsgToString(rcm_id), rcm_len, rcm_token);
        } else {
            int rcm_rsp_err = rcm_sol_hdr->ext.rsp.error;
            if (rcm_rsp_err >= RCM_E_MAX) { //last RCM Error
                RilLogE("RCM error index(%d) is not described", rcm_rsp_err);
            }
            snprintf(headerInfo, sizeof(headerInfo) - 1,
                    "[%s][%s] %s, RSP, %d bytes, Token(0x%04x), %s(%s)",
                    ioname, rx==TRUE?"RX":"TX",
                    ProtocolUtils::rcmMsgToString(rcm_id), rcm_len, rcm_token,
                    (rcm_rsp_err) ? "Error" : "OK",
                    ProtocolUtils::rcmErrorToString(rcm_rsp_err));
        }
        break;
    }
    case RCM_TYPE_INDICATION: {
        if (bufLen < (uint32_t)sizeof(RCM_IND_HEADER)) {
            RilLogE("Invalid data size for indication messages.");
            return -1;
        }
        snprintf(headerInfo, sizeof(headerInfo) - 1,
                    "[%s][%s] %s, IND, %d bytes",
                    ioname, rx==TRUE?"RX":"TX",
                    ProtocolUtils::rcmMsgToString(rcm_id), rcm_len);
        break;
    }
    default :
    ...
    if (printSimIo != LOGLV_SIM_IO_CMD_DUMP && rcm_category == ((SIT_BASE_ID_SIM>>8)&0x00FF) && rcm_num == (SIT_SIM_IO&0x00FF))
    {
    ...
    if (rcm_len > bufLen) {
        RilLogW("rcm_len(%u) of %04X is over data size(%u).",
                rcm_len, rcm_id, bufLen);
        rcm_len = bufLen;
    }

    if (rcm_len > CRilLog::m_uiMaxLogBufferSize) {
        RilLogW("rcm_len(%u) of %04X is over buffer size(%u). Print minimum data",
                rcm_len, rcm_id, CRilLog::m_uiMaxLogBufferSize);
        rcm_len = 32;
    }
