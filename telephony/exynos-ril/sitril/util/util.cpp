...
char *ConvertToHexString(const char *src, const UINT32 srcLen) {
    ...
    if (ConvertToHexString((const BYTE *)src, srcLen, hex, hexLen) == 0) {
        delete[] hex;
        return nullptr;
    }
    return hex;
...
UINT32 ConvertToHexString(const BYTE *src, const UINT32 srcLen, char *dst, const UINT32 dstSize) {
    if (src == nullptr || dst == nullptr || srcLen == 0 || dstSize == 0) {
        return 0;
    }

    if ((srcLen << 1) > dstSize - 1) {
        return 0;
    }

    UINT32 dstIdx = 0;
    for (UINT32 i = 0; i < srcLen; i++) {
        dst[dstIdx++] = HEX_CHARS[src[i] >> 4];
        dst[dstIdx++] = HEX_CHARS[src[i] & 0x0F];
    }
    dst[dstIdx] = 0;
...
int HexString2Value(BYTE *pHexDecOut, int outLen, const char *pszHexStrIn)
{
    ...
    if(nLength > outLen*2 ) nLength = outLen*2;
...
{
    unsigned int id = 0;
    bool isOutOfBoundForDstArr = false;
    auto isValidIndexForDst = [] (int s, int len, int dstSize) {
        return s + len < dstSize;
    };
    ...
    for (unsigned int i = 0; i < srcLen; i++)
    {
        ...
        case '^': {
            if (isValidIndexForDst(id, 2, dstLen)) {
                gsm7[id++] = 0x1B;
                gsm7[id++] = 0x14;
            } else {
                isOutOfBoundForDstArr = true;
            }
        }
        break;
        case '{': {
        ...
        if (true == isOutOfBoundForDstArr)
        {
            RilLogE("%s() index:[%d] can exceeds dstLen:[%d]", __FUNCTION__, id, dstLen);
            return id;
        }
        
        if (false == isValidIndexForDst(id, 1, dstLen)) {
            RilLogE("%s() index exceeds dstLen:[%d]", __FUNCTION__, dstLen);
            return dstLen;
        }
    }
...
static int ConvertUcs2ToUtf8(const unsigned short *src, const unsigned int len, unsigned char *dst, size_t dst_size)
{
    ...
    for ( unsigned int i = 0; i < len; i++ )
    {
        ...
        {
            dst[utf_idx++] = (0xE0 | ((src[i] >> 12) & 0xf));            /* 1110yyyy */
...
int DecodingUssd(const unsigned char dcs, const unsigned char *src, const unsigned int len, unsigned char *dst, size_t dst_size)
{
    ...
    case RIL_DCS_EUC_KR:
        ///TODO:check received data is euc-kr or not.  bEucKr is removed in order to clear prevent (dead code)
        {
            ...
            //if ( bEucKr == true )
            //{
            dec_len = ConvertToHexString(src, len, (char *)dst, dst_size);
            //}
            //else
            //{
                //treat as 8 bit data
                            ...
                            unsigned char dstLen)
{
    ...
    int decLen = 0;
    if (type == 0) {
        // SMS character set: Gsm7bit to string
        ...
        int Ucs2Len = BeByteStream2LeUcs2(src, srcLen, Ucs2, sizeof(Ucs2));
...
int DecodingPbNameFromSIM(const int type,
                            const unsigned char *src,
                            int srcLen,
                            unsigned char *dst,
                            unsigned char dstLen)
{
    ...
    if (type == GSM7BIT) {
        ...
        if ( srcLen % 2 != 0 ) {
            // ucs2 length should be multiple of 2.
            RilLogW("[%s] invalud UCS2 length : %d", __FUNCTION__, srcLen);
            return 0;
        }

        unsigned short Ucs2[MAX_FULL_NAME_LEN] = { 0, };
        memcpy(Ucs2, src, srcLen);
        int ucs2Len = srcLen / 2;
