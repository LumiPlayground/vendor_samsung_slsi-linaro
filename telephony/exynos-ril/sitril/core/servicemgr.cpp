...
IMPLEMENT_MODULE_TAG(ServiceMgr, ServiceMgr)
...
int ServiceMgr::Init(ServiceFactory *pServiceFactory/* = NULL*/)
{
    ...
    // register all service handler
    OnRegisterServiceHandler();
    return 0;
}
...
int ServiceMgr::SendMessageDelayed(Message *msg, long delayMillis)
{
    if (msg == NULL) {
    ...
    if(pService == NULL) {
        RilLogE("SendMessage fail: cannot find service %s, Did you forget AddService in model Init?",
                ServiceInterface::GetServiceName(msg->GetSvcId()));
        return -1;
    }

    if (debug)
        RilLogV("[%s] msg(%p) msgId=%d svc=%s(%d) direction=%s", TAG,
                        msg, msg->GetMsgId(), pService->GetServiceName(), msg->GetSvcId(),
                        msg->MsgDirectionToString(msg->GetDirection()));

    pService->AsyncMsgReqPreProcessing(msg);
    pService->SendMessageDelayed(msg, delayMillis);
    return 0;
}
...
void ServiceMgr::OnRegisterServiceHandler(RouteMap &mmap)
{
    RouteMap::iterator iter = mmap.begin();
    for (; iter != mmap.end(); iter++) {
        std::pair<UINT, UINT> val = iter->second;
        int serviceId = val.first;
        int messageId = val.second;

        if (serviceId > 0 && messageId > 0) {
            Service *service = FindService(serviceId);
            if (service != NULL) {
                ServiceHandler *handler = ServiceHandlerManager::GetHandler(messageId, service);
                if (handler != NULL) {
                    service->RegisterServiceHandler(messageId, handler);
                }
            }
        }
    } // end for iter ~
...
void ServiceMgr::OnRegisterServiceHandler()
{
    OnRegisterServiceHandler(s_RilRequestServiceMap);
    OnRegisterServiceHandler(s_RilOemRequestServiceMap);
    OnRegisterServiceHandler(s_ProtocolServiceMap);
}
