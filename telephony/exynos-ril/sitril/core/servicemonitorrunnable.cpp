...
void ServiceMonitorRunnable::Run()
{
        ...
        if (n > 0) {
            if (FD_ISSET(cmdFd, &rfdSet)) {
                ...
                case ASYNC_REQUEST:
                    ...
                    break;
                case WAKEUP:
                    // wake up signal to re-calculate timeout
                    break;
                default:
    ...
    RilLogW("Service(%s) had been stopped.", m_pSerV->GetServiceName());
}

void ServiceMonitorRunnable::Stop() {
    mRunnable = false;
    if (m_pSerV) {
        m_pSerV->NotifyNewMessage(MsgDirection::WAKEUP);
    }
}
