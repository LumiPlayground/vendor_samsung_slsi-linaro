...
: Service(pRilContext, RIL_SERVICE_GPS)
{
    ...
    mDataRadioTech = RADIO_TECH_UNKNOWN;
    LEAVE_FUNC();
}
...
void GpsService::OnServiceStateChanged(const ServiceState& state) {
    int oldDataRadioTech = mDataRadioTech;
    mDataRadioTech = state.getDataRadioTechnology();
        RilLogV("[%s]%s(%d -> %d)", GetServiceName(), __FUNCTION__,
                oldDataRadioTech, mDataRadioTech);
    if (NetworkUtils::isLte(oldDataRadioTech) != NetworkUtils::isLte(mDataRadioTech)) {
        OnNotifyDataNetworkState(NetworkUtils::isLte(mDataRadioTech),
                NetworkUtils::isInService(state.getDataRegState()));
    }
}

void GpsService::OnRadioNotAvailable() {
    mDataRadioTech = RADIO_TECH_UNKNOWN;
    OnNotifyDataNetworkState(false, false);
}

void GpsService::OnNotifyDataNetworkState(bool isLte, bool inService) {
    int32_t resp[] = { isLte, inService };
    OnUnsolicitedResponse(RIL_UNSOL_OEM_GPS_CAMPING_ON_LTE, resp, sizeof(resp));
}
...
BOOL GpsService::OnHandleRequest(Message* pMsg)
{
    ...
    switch (pMsg->GetMsgId())
    {
        ...
        case MSG_GPS_SET_GNSS_BLANKING:
        // CDMA & HEDGE GANSS
        ...
        case MSG_GPS_SET_GNSS_BLANKING_TDL:
        // CDMA & HEDGE GANSS
        case MSG_GPS_IND_GANSS_AP_POS_CAP_RSP:
            nRet = DoAgpsDefaultRequestIndHandler(pMsg);
            break;

        case MSG_GPS_GET_SIM_STATE:
            nRet = DoGpsGetSimState(pMsg);
            break;
        case MSG_GPS_GET_NETWORK_TYPE:
            nRet = DoGpsGetNetworkType(pMsg);
            break;
        case MSG_GPS_IS_CAMPING_ON_LTE:
            nRet = DoIsCampingOnLte(pMsg);
            break;
...
BOOL GpsService::OnHandleSolicitedResponse(Message* pMsg)
{
    ...
    switch(pMsg->GetMsgId())
    {
        ...
        case MSG_GPS_SET_GNSS_BLANKING_DONE:
        // CDMA & HEDGE GANSS
...
int GpsService::OnAgpsDefaultUnsolRespHandler(Message *pMsg)
{
    ...
    if (resultId != -1) {
        ...
        RilLogW("[%s] Invalid or undefined protocolId=%d", GetServiceName(), adapter.GetId());
    }

    return 0;
}

int GpsService::DoGpsGetSimState(Message *pMsg)
{
    ...
    string simState = SystemProperty::Get(PROP_GSM_SIM_STATE, "");
    OnRequestComplete(RIL_E_SUCCESS, (void *)simState.c_str(), simState.length());
    return 0;
}

int GpsService::DoGpsGetNetworkType(Message *pMsg)
{
    RilLogI("[%s] %s()", m_szSvcName, __FUNCTION__);
    if (pMsg == NULL) {
        RilLogE("pMsg is NULL");
    ...
    string netwokrType = SystemProperty::Get(PROP_GSM_NETWORK_TYPE, "");

    OnRequestComplete(RIL_E_SUCCESS, (void *)netwokrType.c_str(), netwokrType.length());

    return 0;
}

int GpsService::DoIsCampingOnLte(Message * /* pMsg */) {
    RilLogV("[%s] %s()", m_szSvcName, __FUNCTION__);
    if (GetRilContext()) {
        const ServiceState &ss = GetRilContext()->GetRadioServiceState();
        int resp[] = {
            NetworkUtils::isLte(ss.getDataRadioTechnology()),
            NetworkUtils::isInService(ss.getDataRegState())
        };
        RilLogV("[%d] isCampingOnLte(%d, %d)", GetRilSocketId(), resp[0], resp[1]);
...
bool GpsService::IsPossibleToPassInRadioOffState(int /* request_id */) {
    // allow all requests handled by GPS service.
    return true;
}
