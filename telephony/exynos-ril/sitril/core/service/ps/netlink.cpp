...
private:
    ...
    int len = 0;
    //int exitcode=0;
    int nlmsg_count = 0;  // For debugging purpose
    ...
    // TestConfiguration
    const bool emulateRAForTest = false;
    const bool emulatePrefixReconfigurationForTest = false;
public:
    ...
    void processLink(struct nlmsghdr *nh) {
    ...
    void processRoute(struct nlmsghdr *nh) {
    ...
    void processNeigh(struct nlmsghdr *nh) {
        ...
        RilLogV("%s(): RTM_NEWNEIGH index=%d, nh=%p, nlmsg_len=%d, %d", __FUNCTION__, ndinfo->ndm_ifindex, nh, nh->nlmsg_len, nlmsg_count++);
    ...
    void processNewPrefix(struct nlmsghdr *nh) {
        if (m_pManager != NULL) {
            if (emulateRAForTest) {
            ...
            } else if (emulatePrefixReconfigurationForTest) {
                RilLogV("%s(): Call FakeNewPrefix nh=%p, nlmsg_len=%d, %d", __FUNCTION__, nh, nh->nlmsg_len, nlmsg_count++);
    ...
    void processNlMsg(struct nlmsghdr *nh) {
        switch (nh->nlmsg_type) {
          ...
          case RTM_NEWNEIGH:
              ...
              break;
          case RTM_DELNEIGH:
              // RilLogV("%s(): RTM_DELNEIGH nh=%p, nlmsg_len=%d, %d", __FUNCTION__, nh, nh->nlmsg_len, nlmsg_count++);
              break;
          case RTM_GETNEIGH:
              // RilLogV("%s(): RTM_GETNEIGH nh=%p, nlmsg_len=%d, %d", __FUNCTION__, nh, nh->nlmsg_len, nlmsg_count++);
              break;
          case RTM_NEWPREFIX:
              processNewPrefix(nh);
              break;
    ...
    void Run() {
        ...
        while (1) {
            ...
            if (len < 0) {
                ...
                if (recoverNetlinkSocket(4) < 0) return;
                else continue;
            }

            if (len == 0) {
                RilLogV("%s() EOF on netlink", __FUNCTION__);
                if (recoverNetlinkSocket(5) < 0) return;
                else continue;
            }
            ...
            for (nh = (struct nlmsghdr *)buffer; NLMSG_OK(nh, (__u32)len); nh = NLMSG_NEXT(nh, len)) {
                ...
                if (nh->nlmsg_type == NLMSG_DONE) {
                    ...
                    if (recoverNetlinkSocket(6) < 0) return;
                    else continue;
                }

                if (nh->nlmsg_type == NLMSG_ERROR) {
                    RilLogV("%s(): Error in the message received", __FUNCTION__);
                    if (recoverNetlinkSocket(7) < 0) return;
                    else continue;
                }
...
int NetLinkMonitor::FakeNewPrefix(struct nlmsghdr *nh, int period)
{
    ...
    else
    {
        while(m_iTestCount <= FAKE_COUNT_MAX)
        {
            ...
            WaitForTimeUs(__FUNCTION__, period * 1'000'000U);
        }
        return HandleNewPrefix(nh);
    }
    return 0;
