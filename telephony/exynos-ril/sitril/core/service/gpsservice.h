...
protected:
    ...
    void OnServiceStateChanged(const ServiceState& state);
    ...
    void OnNotifyDataNetworkState(bool isLte, bool inService);
...
protected:
    ...
    int DoGpsGetSimState(Message *pMsg);
    int DoGpsGetNetworkType(Message *pMsg);
    int DoIsCampingOnLte(Message *pMsg);

    bool IsPossibleToPassInRadioOffState(int request_id);
};
