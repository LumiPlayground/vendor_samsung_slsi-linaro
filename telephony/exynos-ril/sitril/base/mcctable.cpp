...
void MccTable::InitNotUsePnnOplOperator()
{
    ...
    m_NotUsePnnOplOperator.push_back("00101");
...
MccTable *MccTable::GetTestInstance() {
    return new MccTable();
}
...
{
    if (mcc.length() >= 3) {
        int value = strtol(mcc.substr(0, 3).c_str(), NULL, 10);
...
bool MccTable::isValidPlmn(const char *plmn)
{
    if (TextUtils::IsDigitsOnly(plmn) == false) {
        return false;
    }
...
bool MccTable::IsEsmFlagZeroOperator(const char *carrier)
{
    ...
    int esmZero = SystemProperty::GetInt(PROPERTY_TEST_ESM_ZERO, INT_MAX);
    if (esmZero == 1) {
