...
void RilExternalResponseListener::OnUnsolicitedResponse(int unsolResponse, const void *data, unsigned int datalen, RIL_SOCKET_ID socket_id)
{
    ...
    if (FilterOut(unsolResponse, data, datalen)) {
        RilLogV("RADIO_EXT::[%s] %s Filter out Unsolicited response=%d, channel=%d", TAG, __FUNCTION__, unsolResponse, (int)socket_id);
        return;
    }
...
void RilExternalResponseListener::OnUpdateReqModemData(RIL_Token t, void *data, unsigned int len) {
    // In case of RIL request
    const struct RIL_External_EnvUnitTest *pRil_Ext_EnvUnitTest = (RIL_External_EnvUnitTest *)m_pRilExternalEnv;
    if (pRil_Ext_EnvUnitTest != NULL && pRil_Ext_EnvUnitTest->OnUpdateReqModemData != NULL) {
        pRil_Ext_EnvUnitTest->OnUpdateReqModemData(t, data, len);
    } else {
        RilLogW("[%s] %s cannot invoke pRil_Ext_EnvUnitTest->OnUpdateReqModemData()", TAG, __FUNCTION__);
    }
}
