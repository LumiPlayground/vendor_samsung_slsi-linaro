...
{
    if (Build::IS_USER) {
        std::string mode = SystemProperty::Get(CRASH_MODE_SYS_PROP, "");
...
bool ModemStateMonitor::IsCpNoBoot() {
    const int CP_NOBOOT = 3;
    const char *propertyName = "persist.vendor.cbd.debug_cp_opt";
    return (SystemProperty::GetInt(propertyName, -1) == CP_NOBOOT);
}
        ...
        case STATE_BOOTING: {
            ...
            m_pRilApp->OnModemStateChanged(mState);
                ...
                while (spin--) {
                    ...
                    WaitForTimeUs(__FUNCTION__, 100'000U);
                } // end while ~
...
void ModemStateMonitor::Run()
{
    ...
    if (IsCpNoBoot()) {
        RilLogW("Stop monitoring due to CP_NOBOOT");
        return ;
    }

    mFd = open(MODEM_BOOT, O_RDWR | O_CLOEXEC);
    if (mFd < 0)
    {
