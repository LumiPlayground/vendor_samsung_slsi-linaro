...
class MccTable {
    DECLARE_MODULE_TAG()
private:
  std::map<int, MccEntry *> m_mcctable;
  std::list<std::string> m_EsmZeroOperator;
  std::list<std::string> m_NotUsePnnOplOperator;
...
public:
    ...
    static MccTable *GetTestInstance();
    ...
    static int GetSmallestDigitsMccForMcc(const std::string &mcc);
    static int GetSmallestDigitsMccForImsi(const char *imsi);
    static int GetSmallestDigitsMccForImsi(const std::string &mcc);
