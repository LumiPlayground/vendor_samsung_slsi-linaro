...
bool ApnSetting::CanHandleType(const int apnTypeBitmask) {
    return mSupportedTypesBitmask & apnTypeBitmask;
}
...
bool ApnSetting::isPcscfRequiredForDefaultPDN()
{
    // T-mobile requires PCSCF for Internet APN to use for RCS Service Registration.
    // From InitialAttach and SetupDataCall time, PCSCF REQ will be sent.
    // GID-MTRREQ-95801, GID_MTRREQ-96001
    if (TextUtils::Equals(m_carrier, "310260") && CanHandleType(APN_TYPE_BIT_DEFAULT) ) {
        return true;
    } else {
        return false;
    }
