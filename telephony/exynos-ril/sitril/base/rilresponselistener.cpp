...
void RilResponseListener::OnUnsolicitedResponse(int unsolResponse, const void *data, unsigned int datalen, RIL_SOCKET_ID socket_id)
{
    ...
    if (FilterOut(unsolResponse, data, datalen)) {
        RilLogV("RADIO::[%s] %s Filter out Unsolicited response=%d", TAG, __FUNCTION__, unsolResponse);
        return;
    }
...
void RilResponseListener::OnUpdateReqModemData(RIL_Token t, void *data, unsigned int len) {
    // In case of RIL request
    const struct RIL_EnvUnitTest *pRilEnvUnitTest = (RIL_EnvUnitTest *)m_pRilEnv;
    if (pRilEnvUnitTest != NULL && pRilEnvUnitTest->OnUpdateReqModemData != NULL) {
        pRilEnvUnitTest->OnUpdateReqModemData(t, data, len);
    } else {
        RilLogW("[%s] %s cannot invoke pRilEnvUnitTest->OnUpdateReqModemData()", TAG, __FUNCTION__);
    }
}
