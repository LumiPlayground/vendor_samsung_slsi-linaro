...
public:
    ...
    RilData(const RilData &) = delete;
    RilData& operator=(RilData const&) = delete;
    virtual ~RilData() {
...
public:
    RilDataString();
    RilDataString(const char *str);
    RilDataString(const RilDataString &) = delete;
    RilDataString& operator=(RilDataString const&) = delete;
...
public:
    ...
    RilDataStrings(const RilDataStrings &) = delete;
    RilDataStrings& operator=(RilDataStrings const&) = delete;
...
public:
    RilDataInts(int c);
    RilDataInts(const RilDataInts &) = delete;
    RilDataInts& operator=(RilDataInts const&) = delete;
    virtual ~RilDataInts();
...
public:
    ...
    RilDataRaw(const RilDataRaw &) = delete;
    RilDataRaw& operator=(RilDataRaw const&) = delete;
    virtual ~RilDataRaw();
...
template<typename T>
class RilDataTemplate : public RilData {
    std::function<T()> mFn;
public:
    RilDataTemplate(std::function<T()> fn) : mFn(fn) {}
    T Get() const { return mFn(); }
    RilDataTemplate *Clone() {
        return new RilDataTemplate(mFn);
    }
};

template<typename T>
struct RilDataPlaceHolder : public RilData {
    T type;
    void *data = nullptr;
    uint32_t size = 0;

    RilDataPlaceHolder(const T& type) {
        this->type = type;
        data = &this->type;
        size = sizeof(T);
    }
    void *GetData() const { return data; }
    unsigned int GetDataLength() const { return size; }
};

template<typename T>
class RilDataListHolder : public RilData {
protected:
    std::vector<T> mList;
public:
    RilDataListHolder()=default;
    RilDataListHolder(std::initializer_list<T> il) : mList(il) { }
    void *GetData() const { return (void *)mList.data(); }
    uint32_t GetDataLength() const { return mList.size() * sizeof(T); }
};

using RilDataUniqPtr = std::unique_ptr<const RilData>;
