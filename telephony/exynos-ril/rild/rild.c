...
/* RIL vendor external */
extern  void RIL_External_register (const RIL_RadioExternalFunctions *callbacks);
extern  void RIL_External_onRequestComplete(RIL_External_Token t, RIL_Errno e, void *response, size_t responselen);
extern  void RIL_External_onUnsolicitedResponse(int unsolResponse, const void *data, size_t datalen, int slotId);

static struct RIL_External_Env s_rilExternalEnv = {
    RIL_External_onRequestComplete,
    RIL_External_onUnsolicitedResponse,
};
...
int main(int argc, char **argv) {
    ...
    /* RIL vendor external */
    const RIL_RadioExternalFunctions *(*rilExternalInit)(const struct RIL_External_Env *);
    const RIL_RadioExternalFunctions *external_funcs;
    ...
    dlerror();
    rilExternalInit =
        (const RIL_RadioExternalFunctions *(*)(const struct RIL_External_Env *))
        dlsym(dlHandle, "RIL_External_Init");
    if (rilExternalInit == NULL) {
        RLOGE("RIL_External_Init not defined or exported in %s\n", rilLibPath);
    }
    ...
    /* RIL vendor external */
    if (rilExternalInit) {
        external_funcs = rilExternalInit(&s_rilExternalEnv);
        if (external_funcs == NULL) {
            RLOGD("RIL_External service is not registered");
        } else {
            RIL_External_register(external_funcs);
            RLOGD("RIL_External_Init completed");
        }
    }
