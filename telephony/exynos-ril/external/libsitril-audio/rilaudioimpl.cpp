 ...
 *    - change the file name to rilaudioapiimpl.cpp
 */
...
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
#define CLIENT_MODULE_NAME "RilAudioApi"

static RilAudioImpl g_RilAudioApi;
IMPLEMENT_MODULE_TAG(RilAudioImpl, RilAudioImpl)

RilAudioImpl *RilAudioImpl::GetInstance()
{
    return &g_RilAudioApi;
}

void RilAudioImpl::OnSolicitedResponse(unsigned msgId, int status, void* data, size_t length, unsigned int channel)
{
    RilAudioImpl *instance = RilAudioImpl::GetInstance();
    if (instance != NULL) {
        instance->HandleSolicitedResponse(msgId, status, data, (unsigned int)length, channel);
    }
}

void RilAudioImpl::OnUnsolicitedResponse(unsigned msgId, void* data, size_t length, unsigned int channel)
{
    RilAudioImpl *instance = RilAudioImpl::GetInstance();
    if (instance != NULL) {
...
void RilAudioImpl::HandleSolicitedResponse(unsigned int msgId, int status, void* data, unsigned int length, unsigned int channel)
{
...
void RilAudioImpl::HandleUnsolicitedResponse(unsigned msgId, void *data, unsigned int length, unsigned int channel)
{
...
bool RilAudioImpl::IsTransactionExisted() const
{
...
void RilAudioImpl::WaitAllTransactionCompleted()
{
...
int RilAudioImpl::Open()
{
    ...
    const char *name = CLIENT_MODULE_NAME;
    m_client = m_pRilClientHelper->Open(name);
    if (m_client == NULL) {
    ...
    int filters[] = {
        RILC_UNSOL_IMS_SRVCC_HO,
        RILC_UNSOL_AUDIO_RINGBACK,
        RILC_UNSOL_AUDIO_RINGBACK_BY_NETWORK,
        RILC_UNSOL_WB_AMR_REPORT,
    };
    RLOGE("%s::%s filter=%p size=%zu", TAG, __FUNCTION__, filters, sizeof(filters) / sizeof(filters[0]));
    m_pRilClientHelper->SetIndicationFilter(m_client, filters, sizeof(filters) / sizeof(filters[0]));

    if (m_pRilClientHelper->RegisterUnsolicitedResponseHandler(m_client, &RilAudioImpl::OnUnsolicitedResponse) != RILC_STATUS_SUCCESS) {
...
int RilAudioImpl::Close()
{
    ...
    /* for unit test */
    mUnitTestSolicitedResponse = nullptr;
    m_pRilClientHelper->SetUnitTest(m_client, NULL);

    (void)m_pRilClientHelper->RegisterUnsolicitedResponseHandler(m_client, NULL);
...
int RilAudioImpl::RegisterEventCallback(HANDLE handle, RILAUDIO_EventCallback callback)
{
...
int RilAudioImpl::SetAudioVolume(int volume)
{
...
int RilAudioImpl::SetAudioPath(int audioPath)
{
...
int RilAudioImpl::SetMultiMic(int mode)
{
...
int RilAudioImpl::SetMute(int mode)
{
    RLOGI("%s::%s mode=%d", TAG, __FUNCTION__, mode);
    if (m_pRilClientHelper == NULL || m_client == NULL) {
        RLOGW("%s::%s error: not initialized", TAG, __FUNCTION__);
...
int RilAudioImpl::SetAudioClock(int mode)
{
    RLOGI("%s::%s mode=%d", TAG, __FUNCTION__, mode);
    if (m_pRilClientHelper == NULL || m_client == NULL) {
        RLOGW("%s::%s error: not initialized", TAG, __FUNCTION__);
...
int RilAudioImpl::SetAudioLoopback(int onoff, int path)
{
...
int RilAudioImpl::SetTtyMode(int ttyMode)
{
...
/* for unit test */
const void* RilAudioImpl::SetUnitTest(int openStatus, const struct RILC_EnvUnitTest *env) {
    static RILC_UnitTestFunctions test;

    if (openStatus == RILAUDIO_ERROR_NONE && m_client != NULL) {
        mUnitTestSolicitedResponse = env->OnRequestComplete;

        m_pRilClientHelper->SetUnitTest(m_client, env->OnRequestSend);
    }

    test.openStatus = openStatus;
    test.onIndSend = OnUnsolicitedResponse;
    test.onRspSend = OnSolicitedResponse;
