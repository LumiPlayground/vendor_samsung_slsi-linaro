...
int main()
{
    ...
    pfnSetMute = (int (*)(int))dlsym(h, "SetMute");
    if (pfnSetMute == NULL) {
        DLOG("fail to load SetMute");
        return 0;
    }
    ...
    DLOG("pfnSetMute=%p", pfnSetMute);
    DLOG("pfnSetTtyMode=%p", pfnSetTtyMode);

    DLOG("RilAudioOpen");
    pfnRilAudioOpen();

    pfnSetMute(RILAUDIO_MUTE_DISABLED);
    pfnSetMute(RILAUDIO_MUTE_ENABLED);
