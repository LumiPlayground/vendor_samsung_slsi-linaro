 ...
 * @1.1 compatible
 */
/* Open client with a name*/
void* RILC_OpenV2(const char *name);

/* Set indication filters which are available in the current client */
int RILC_SetIndicationFilter(void* client, int *filters, size_t size);

/* for unit test */
typedef void (*RILC_ReqSend)(unsigned int msgId, void *data, size_t length, unsigned int channel);

typedef struct {
    int openStatus;
    Rilc_OnResponse onRspSend;
    Rilc_OnUnsolicitedResponse onIndSend;
} RILC_UnitTestFunctions;

struct RILC_EnvUnitTest {
    Rilc_OnResponse OnRequestComplete;
    Rilc_OnUnsolicitedResponse OnUnsolicitedResponse;
    RILC_ReqSend OnRequestSend;
};

/* Regeister Unit test callback for send reqeust*/
int RILC_SetUnitTest(void* client, RILC_ReqSend handler);
