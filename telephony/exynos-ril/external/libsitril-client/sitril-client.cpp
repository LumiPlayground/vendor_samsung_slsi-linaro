...
#define MAX_RADIO_DATA_SIZE (1 << 12)    // set 2's multiplier
...
/* Client Module name */
#define AUDIO_CLIENT_NAME "RilAudioApi"
...
struct rilc_client {
    ...
    RILC_ReqSend unitTestReqSendHandler;    // for unit test
};
...
private:
    ...
    bool isAudioClient(struct rilc_client *p);
public:
    SitRilClient();
...
public:
    static bool checkReturnStatus(Return<void>& ret) {
        if (!ret.isOk()) {
    ...
    static bool checkReturnStatus(Return<int32_t>& ret) {
        if (!ret.isOk()) {
            // Remote process hosting the callbacks must be dead.
            // HAL proxy and callback objects will be reset by
            // processRadioExternalProxyDeath wihch is callback by hidl_death_recipient
...
bool SitRilClient::isAudioClient(struct rilc_client *client)
{
    if (client == NULL ) return false;
    if (strncmp(client->name, AUDIO_CLIENT_NAME, strlen(AUDIO_CLIENT_NAME)) == 0) return true;
    return false;
}
...
sp<IOemSlsiRadioExternal> SitRilClient::getRadioExternalProxy(struct rilc_client *p)
{
    ...
    if (isAudioClient(p)) mRadioExternalProxy = IOemSlsiRadioExternal::tryGetService("rilExternal");    // no retry in the API
    else mRadioExternalProxy = IOemSlsiRadioExternal::getService("rilExternal");   // retries in the API
    ...
    if (mRadioExternalProxy == NULL) {
        ...
        sp<V1_1::IOemSlsiRadioExternal> radioExternalProxy11 =
                V1_1::IOemSlsiRadioExternal::castFrom(mRadioExternalProxy).withDefault(nullptr);
        Return<int32_t> ret(0);
        if (radioExternalProxy11 != NULL) {
            hidl_string name = p->name;
            ret = radioExternalProxy11->setResponseFunctions_1_1(mRadioExternalRes, mRadioExternalInd, hidl_string(p->name));
        }
        else {
            ret = mRadioExternalProxy->setResponseFunctions(mRadioExternalRes, mRadioExternalInd);
        }
        if (!SitRilClient::checkReturnStatus(ret)) {
            newClientId = -1;
        }
        else {
            newClientId = ret;
...
/* for unit test */
/*
    Regeister Unit test callback for send reqeust

    input : pointer to client structure, pointer to handler or NULL
    output: 0 if setting was successful, error status otherwise
*/
extern "C" int RILC_SetUnitTest(void* client, RILC_ReqSend handler)
{
    LogD("[OemClient] %s", __FUNCTION__);
    ...
    pthread_mutex_lock(&p->lock);
    p->unitTestReqSendHandler = handler;
    pthread_mutex_unlock(&p->lock);
...
//    rilc_request_t *req = NULL;
    ...
    switch (msgId) {
        ...
        case RILC_REQ_GPS_SET_GNSS_BLANKING:
        case RILC_REQ_GPS_GET_SIM_STATE:
        case RILC_REQ_GPS_GET_NETWORK_TYPE:
        case RILC_REQ_GPS_IS_CAMPING_ON_LTE:
        ...
        case RILC_REQ_GPS_SET_GNSS_BLANKING_TDL:
        /* CDMA & HEDGE GANSS */
        ...
        case RILC_REQ_UPDATE_EMERGENCY_NUMBER_LIST:
        ...
        case RILC_REQ_AIMS_SEND_UL_SIP_MSG_IND:
        case RILC_REQ_AIMS_DL_SIP_MSG_RESULT_IND:
        case RILC_REQ_AIMS_REGISTER_IND:
        //WFC
    ...
    /* send */
    /* for unit test */
    RILC_ReqSend unitTesthandler = p->unitTestReqSendHandler;
    if (unitTesthandler != NULL) {
        LogD("[OemClient]UNIT TEST send request");
        unitTesthandler(msgId, data, length, channel);
        return RILC_STATUS_SUCCESS;
    }
...
static void ConvertIntsToHidlVec(hidl_vec<int32_t> &list, int *filter, size_t size) {
    list.resize(0);
...
extern "C" void* RILC_OpenV2(const char *name)
{
    /* create client */
    ...
    snprintf(p->name, MAX_CLIENT_NAME - 1, "%s(%u)", (name ? name : "PID"), getpid());
...
extern "C" int RILC_SetIndicationFilter(void* client, int *filter, size_t size)
{
    LogD("[OemClient] %s", __FUNCTION__);
    ...
    sp<IOemSlsiRadioExternal> radioExternalProxy = pInstance->getRadioExternalProxy(p);
    sp<V1_1::IOemSlsiRadioExternal> radioExternalProxy11 =
            V1_1::IOemSlsiRadioExternal::castFrom(radioExternalProxy).withDefault(nullptr);
    if (radioExternalProxy == NULL || radioExternalProxy11 == NULL) {
        return -RILC_STATUS_NOT_CONNECTED;
    }

    LogD("[OemClient] 222 filter=%p size=%zu", filter, size);
    hidl_vec<int32_t> data;
    ConvertIntsToHidlVec(data, filter, size);
    LogD("[OemClient] 222 data.size()=%zu", data.size());
    Return<void> retStatus = radioExternalProxy11->setIndicationFilter(clientId, data);
    if (!SitRilClient::checkReturnStatus(retStatus)) {
        return -RILC_STATUS_NOT_CONNECTED;
    }
