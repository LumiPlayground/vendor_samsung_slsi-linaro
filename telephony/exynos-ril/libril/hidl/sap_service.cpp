...
struct SapImpl : public android::hardware::radio::V1_2::ISap {
    ...
    void sendFailedResponse(MsgId msgId, int32_t token, size_t numPointers, Args&&... args);
...
template<typename... Args>
void SapImpl::sendFailedResponse(MsgId msgId, int32_t token, size_t numPointers, Args&&... args) {
    std::vector<void *> vec = {args...};
    if (numPointers >= vec.size()) numPointers = vec.size();
