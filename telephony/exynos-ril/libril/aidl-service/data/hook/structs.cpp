...
V1_5::DataProfileInfo toHidl(const aidl::DataProfileInfo &info) {
    return {
        ...
        .supportedApnTypesBitmap =
            toHidlBitfield<V1_5::ApnTypes>(info.supportedApnTypesBitmap),
        .bearerBitmap =
            toHidlBitfield<V1_4::RadioAccessFamily>(info.bearerBitmap),
        .mtuV4 = info.mtuV4,
        .mtuV6 = info.mtuV6,
...
V1_6::SliceInfo toHidl(const aidl::SliceInfo &info) {
    return {
        ...
        .sliceDifferentiator = info.sliceDifferentiator,
        .mappedHplmnSst =
            static_cast<V1_6::SliceServiceType>(info.mappedHplmnSst),
        .mappedHplmnSD = info.mappedHplmnSd,
...
static std::variant<bool, aidl::EpsQos, aidl::NrQos>
toAidl(const V1_6::Qos &qos) {
    if (qos.getDiscriminator() == V1_6::Qos::hidl_discriminator::eps)
        return toAidl(qos.eps());
    if (qos.getDiscriminator() == V1_6::Qos::hidl_discriminator::nr)
        return toAidl(qos.nr());
    return false;
}
...
static std::optional<aidl::PortRange> toAidl(const V1_6::MaybePort &opt) {
    if (opt.getDiscriminator() == V1_6::MaybePort::hidl_discriminator::noinit)
        return std::nullopt;
    return toAidl(opt.range());  // can't use MaybeX template - this field is not
                                 // named "value"
}
...
static int8_t toAidl(const V1_6::OptionalSscMode &opt) {
    if (opt.getDiscriminator() ==
        V1_6::OptionalSscMode::hidl_discriminator::noinit) {
...
static aidl::PdpProtocolType toAidl(const V1_6::OptionalPdpProtocolType &opt) {
    ...
    if (opt.getDiscriminator() == discriminator::noinit)
        return aidl::PdpProtocolType::UNKNOWN;
    return aidl::PdpProtocolType(opt.value());
}

aidl::RouteSelectionDescriptor
