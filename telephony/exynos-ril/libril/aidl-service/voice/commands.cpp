...
/**
  ...
  */
void IsVoNrEnabledCommand::onRequest(int serial) {
#if VDBG
...
int IsVoNrEnabledCommand::onResponse(int slotId,int responseType, int serial, RIL_Errno e,
    ...
    if (mCb) {
        ...
        mCb->isVoNrEnabledResponse(responseInfo, resp);
    } else {
 ...
 */
void SetVoNrEnabledCommand::onRequest(int32_t serial, bool enable) {
#if VDBG
    RLOGD("%s::%s: serial %d", TAG, __FUNCTION__, serial);
#endif
    mVendorFunctions.dispatchInts(allocateRequestInfo(serial), { BOOL_TO_INT(enable) });
}

int SetVoNrEnabledCommand::onResponse(int slotId,int responseType, int serial, RIL_Errno e,
        void *response, size_t responselen) {
#if VDBG
    RLOGD("%s::%s: serial %d", TAG, __FUNCTION__, serial);
#endif
    if (mCb) {
        aidl::RadioResponseInfo responseInfo = populateResponseInfo(serial, responseType, e);
        mCb->setVoNrEnabledResponse(responseInfo);
    } else {
        RLOGE("%s::%s: [%d] mCb == NULL", TAG, __FUNCTION__, slotId);
    }
    return 0;
