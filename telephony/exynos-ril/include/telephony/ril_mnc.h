...
static inline std::string decode(int mnc) {
    const std::string INVALID = std::to_string(INT_MAX);
    if (mnc == INT_MAX || mnc < 0) return INVALID;
    unsigned umnc = mnc;
    char mncNumDigits = (umnc >> (sizeof(int) * 8 - 4)) & 0xF;

    umnc = (umnc << 4) >> 4;
    if (umnc > 999) return INVALID;
...
static inline int test() {
    const std::string INVALID = std::to_string(INT_MAX);
    const struct mnc_strings { const char * in; std::string out; } mncs[] = {
        {"0001",INVALID},
        {"9999",INVALID},
        {"0",INVALID},
        {"9",INVALID},
    ...
    for (size_t i=0; i< sizeof(mncs) / sizeof(struct mnc_strings); i++) {
        if (decode(encode(mncs[i].in)).compare(mncs[i].out)) return 1;
    }

    const struct mnc_ints { const int in; std::string out; } legacy_mncs[] = {
        {INT_MAX, INVALID},
        {1, "1"},
        {11, "11"},
        {111, "111"},
    ...
    for (size_t i=0; i < sizeof(legacy_mncs) / sizeof(struct mnc_ints); i++) {
