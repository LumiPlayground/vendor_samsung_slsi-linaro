...
static inline std::string decode(int mcc) {
    char mccStr[4] = {0};
    if (mcc > 999 || mcc < 0) return std::to_string(INT_MAX);
...
static inline int test() {
    ...
    const struct mcc_ints { const int in; std::string out; } legacy_mccs[] = {
        {INT_MAX, INVALID},
        {1, "001"},
        {11, "011"},
        {111, "111"},
    ...
    for (size_t i=0; i < sizeof(legacy_mccs) / sizeof(struct mcc_ints); i++) {
