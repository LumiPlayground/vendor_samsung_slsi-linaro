...
#define RIL_CDMA_ALPHA_INFO_BUFFER_LENGTH 64
#define RIL_CDMA_NUMBER_INFO_BUFFER_LENGTH 81
...
#define MAX_SMS_CONFIG_INFO_NUM 50
#define MAX_SMS_CDMA_CONFIG_INFO_NUM 50
...
typedef enum {
    ...
    PDP_FAIL_PAYLOAD_NOT_FORWARDED = 0xA2,

    //RadioHal 1_6
    PDP_FAIL_RIL_SLICE_REJECTED = 0x8CC,
    PDP_MATCH_ALL_RULE_NOT_ALLOWED,
    PDP_ALL_MATCHING_RULES_FAILED,
...
typedef struct {
  char alpha_len;
  char alpha_buf[RIL_CDMA_ALPHA_INFO_BUFFER_LENGTH];
...
typedef struct {
  char len;
  char buf[RIL_CDMA_NUMBER_INFO_BUFFER_LENGTH];
  char number_type;
  char number_plan;
  char pi;
  char si;
...
#define RIL_SS_INFO_MAX 4
#define RIL_NUM_SERVICE_CLASSES 7
...
typedef struct {
  ...
  RIL_CallForwardInfo cfInfo[RIL_NUM_SERVICE_CLASSES]; /* This is the response data
...
typedef struct {
  ...
  union {
    int ssInfo[RIL_SS_INFO_MAX]; /* This is the response data for most of the SS GET/SET
...
typedef enum {
     ...
     */
    RIL_INDFILTER_SIGNAL_STRENGTH = 1 << 0,
    /**
     * When this bit is set, modem must invoke IRadioIndication.networkStateChanged() when any fie
     ...
     */
    RIL_INDFILTER_FULL_NETWORK_STATE = 1 << 1,
    /**
     ...
     */
    RIL_INDFILTER_DATA_CALL_DORMANCY_CHANGED = 1 << 2,
} RIL_IndicationFilter;
...
typedef enum {
    RIL_ONE_SHOT = 0x00, // Performs the scan only once
    RIL_PERIODIC = 0x01  // Performs the scan periodically until cancelled
} RIL_ScanType;

typedef enum {
    RIL_RAN_UNKNOWN = 0x0,
    RIL_RAN_GERAN = 0x01,   // GSM EDGE Radio Access Network
    RIL_RAN_UTRAN = 0x02,   // Universal Terrestrial Radio Access Network
    RIL_RAN_EUTRAN = 0x03,  // Evolved Universal Terrestrial Radio Access Network
 ...
 * "data" is an int *
 *
 * ((int *)data)[0] is a 32-bit bitmask of RIL_IndicationFilter
 *
 * "response" is NULL
 *
 * Valid errors:
...
/**
 * Addresses with optional "/" prefix length, e.g., "192.0.1.3" or
 * "192.0.1.11/16 2001:db8::1/64". If the
 ...
 */
    int IPv4_addresses[MAX_IPV4_ADDR_LEN];
    int IPv6_addresses[MAX_IPV6_ADDR_LEN];
} RIL_SetPdnIpAddressRequest;
...
/* for unit test */
typedef struct __RIL_TestToken {
    int serial;
    int requestId;
}RIL_TestToken;

typedef void (*RIL_ModemRspSend)(void *data, unsigned int datalen);
typedef void (*RIL_SitrilStateUpdate)(int radioState, int networkRegState, int cardState);
typedef void (*RIL_ExternRequestFunc) (int reqOemId, RIL_Token token, void *data, size_t datalen, RIL_SOCKET_ID socket_id);

typedef struct {
    int version;
    RIL_RequestFunc onRequest;
    RIL_RadioStateRequest onStateRequest;
    RIL_ModemRspSend onModemRspSend;
    RIL_SitrilStateUpdate onSitrilStateUpdate;
    RIL_ExternRequestFunc externOnRequest;
    RIL_RequestFunc onSapRequest;
} RIL_UnitTestFunctions;

#if NO_RIL_ENV_UNITTEST
#else
#ifdef RIL_SHLIB
struct RIL_EnvUnitTest : public RIL_Env {
