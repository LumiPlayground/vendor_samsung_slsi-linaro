...
#define ION_HEAP_TYPE_NONE INT_TYPE_MAX
...
static unsigned int ion_get_matched_heapmask(unsigned int legacy_heap_id) {
    ...
    for (heap_id = 0; heap_id < ION_NUM_HEAP_IDS; heap_id++) {
        ...
        if (!strcmp(ion_heap_list[heap_id].name, ion_heap_name[legacy_heap_id].name) &&
                (ion_heap_list[heap_id].namelen == ion_heap_name[legacy_heap_id].namelen))
            return 1 << heap_id;
    }

    printf("%s: unable to find heap '%s'(id %u)",
          __func__, ion_heap_name[legacy_heap_id].name, legacy_heap_id);
    printf("ION HEAP LIST");
    for (heap_id = 0; heap_id < ION_NUM_HEAP_IDS; heap_id++)
        if (ion_heap_list[heap_id].type != ION_HEAP_TYPE_NONE)
                printf("ID %d, TYPE %d, NAME %s", heap_id,
...
static int ion_alloc_legacy(int ion_fd, size_t len,
    ...
    if (ret < 0) {
        printf("%s(%d, %zu, %#x, %#x) ION_IOC_ALLOC failed: %s", __func__,
    ...
    if (ret < 0) {
        printf("%s(%d, %zu, %#x, %#x) ION_IOC_SHARE failed: %s", __func__,
              ion_fd, len, heap_mask, flags, strerror(errno));
        return -1;
    }
...
static int ion_alloc_modern(int ion_fd, size_t len,
    ...
    if (!data.heap_id_mask) {
        printf("%s: unable to find heaps of heap_mask %#x", __func__, legacy_heap_mask);
        return -1;
    }

    ret = ioctl(ion_fd, ION_IOC_ALLOC_MODERN, &data);
    if (ret < 0) {
        printf("%s(%d, %zu, %#x(%#x), %#x) failed: %s", __func__,
...
static void ion_query_heaps(int ion_fd) {
    ...
    if (ret < 0) {
        printf("%s: failed query heaps with ion_fd %d: %s",
...
int g_ion_version = ION_VERSION_UNKNOWN;

int ion_is_legacy(int ion_fd) {
    //int version = atomic_load_explicit(&g_ion_version, memory_order_acquire);
    int version = g_ion_version;
    if (version == ION_VERSION_UNKNOWN) {
        ...
        g_ion_version = version;
        if (version == ION_VERSION_MODERN)
            ion_query_heaps(ion_fd);
	//ALOGD("Found libion %s", (version == ION_VERSION_MODERN) ? "MODERN" : "LEGACY");
    }
...
int exynos_ion_open() {
    ...
    if (fd < 0)
        printf("open /dev/ion failed: %s", strerror(errno));
    return fd;
}
...
int exynos_ion_close(int fd) {
    int ret = close(fd);
    if (ret < 0)
        printf("closing fd %d of /dev/ion failed: %s", fd, strerror(errno));
    return ret;
}
...
int exynos_ion_import_handle(int ion_fd, int fd, int* handle) {
    ...
    if (!ion_is_legacy(ion_fd)) {
        printf("%s() is not supported by the modern ION", __func__);
	/*
	 * return success to the clients not to push them
	...
	*handle = 0;
    ...
    if (ret < 0) {
        printf("%s(%d, %d) failed: %s", __func__, ion_fd, fd, strerror(errno));
        return -1;
    }
...
int exynos_ion_free_handle(int ion_fd, int handle) {
    ...
    if (ret < 0) {
        printf("%s(%d, %d) failed: %s", __func__, ion_fd, handle, strerror(errno));
        return -1;
    }
    return 0;
}
...
int exynos_ion_sync_fd(int ion_fd, int fd) {
    ...
    return ioctl(ion_fd, ION_IOC_SYNC, &data);
}
...
int exynos_ion_sync_fd_partial(int ion_fd, int fd, off_t offset, size_t len) {
    ...
    return ioctl(ion_fd, ION_IOC_SYNC_PARTIAL, &data);
}
