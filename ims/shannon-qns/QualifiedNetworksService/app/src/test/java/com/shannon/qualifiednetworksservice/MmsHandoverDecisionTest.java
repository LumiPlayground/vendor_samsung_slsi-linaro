...
@Config(sdk = {32})
public class MmsHandoverDecisionTest extends TestCase {
    @BeforeClass
    ...
    private MmsHandoverDecision mHandoverDecision;
    private HandoverConditions handoverConditions;
    ...
    public void setUp() throws Exception {
        ...
        mHandoverDecision = new MmsHandoverDecision(handoverConditions);
    }
    ...
    public void testGetPreferredListWifiDuringHandover() {
        ...
        when(confMgr.isMmsOverWiFiSupported()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListCellularDuringHandover() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListWifiDuringIdleCase() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListCellularDuringIdleCase() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListNotWifiPreferred() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(cellular.isAllowedRat(any())).thenReturn(true);
    ...
    public void testGetPreferredListNotWifiPreferredWifiNotConnected() {
        ...
        when(confMgr.isMmsOverWiFiSupported()).thenReturn(false);
        when(condUserSetting.isWfcEnabled()).thenReturn(true);
        when(transport.getNetworkType(anyInt())).thenReturn(NETWORK_TYPE_LTE);
        when(cellular.isRoveOut()).thenReturn(true);
        when(wifi.isRoveIn()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListAirplaneModeOn() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(false);
        when(cellular.isAllowedRat(any())).thenReturn(true);
        when(cellular.isRegistered()).thenReturn(true);
        when(wifi.isConnected()).thenReturn(false);
        when(confMgr.isMmsOverWiFiSupported()).thenReturn(true);
        ...
        when(cellular.isRoveOut()).thenReturn(true);
        when(cellular.isAirplaneMode()).thenReturn(true);
        mHandoverDecision.setNewState(mHandoverDecision.mAttachState);
        when(wifi.isRoveIn()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListCellularRat() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListNotWifiPreferredNoValidRat() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(true);
    ...
    @Test
    public void testGetPreferredListNotWifiPreferredNotValidCellular() {
        //Given
        ConditionEvents event = EVENT_APN_CONNECTED;
        when(transport.isTransportAvailable(anyInt(), any())).thenReturn(true);
        when(confMgr.isWiFiPreferredForMms()).thenReturn(false);
